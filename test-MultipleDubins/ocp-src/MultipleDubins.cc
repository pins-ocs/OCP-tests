...
namespace MultipleDubinsDefine {
  ...
  */
  void
  MultipleDubins::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
  ...
  */
  void
  MultipleDubins::setupParameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "MultipleDubins::setupParameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in MultipleDubins::setup not all parameters are set!\n"
    );
  }

  void
  MultipleDubins::setupParameters( real_type const Pars[] ) {
  ...
  */
  void
  MultipleDubins::setupClasses( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  MultipleDubins::setupUserClasses( GenericContainer const & gc ) {
  }
  ...
  */
  void
  MultipleDubins::setupUserMappedFunctions( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("MappedObjects"),
      "MultipleDubins::setupClasses: Missing key `MappedObjects` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("diff2pi"),
      "in MultipleDubins::setupUserMappedFunctions(gc) missing key: ``diff2pi''\n"
    );
    diff2pi.setup( gc("diff2pi") );

    UTILS_ASSERT0(
      gc.exists("clip"),
      "in MultipleDubins::setupUserMappedFunctions(gc) missing key: ``clip''\n"
    );
    clip.setup( gc("clip") );
  }
  /* --------------------------------------------------------------------------
  ...
  */
  void
  MultipleDubins::setupControls( GenericContainer const & gc_data ) {
  ...
  */
  void
  MultipleDubins::setupPointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "MultipleDubins::setupPointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in MultipleDubins::setupPointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  MultipleDubins::infoClasses() const {
    int msg_level = 3;
    ostringstream mstr;
  ...
  */
  void
  MultipleDubins::get_names( GenericContainer & out ) const {
  ...
  void
  MultipleDubins::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
