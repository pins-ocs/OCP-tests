...
#ifndef MULTIPLEDUBINS_DOT_HH
#define MULTIPLEDUBINS_DOT_HH
...
#include <MechatronixCore/MechatronixCore.hh>
  ...
  #ifndef MULTIPLEDUBINS_API_DLL
    #ifdef MULTIPLEDUBINS_EXPORT
      #define MULTIPLEDUBINS_API_DLL __declspec(dllexport)
    #elif defined(MULTIPLEDUBINS_IMPORT)
      #define MULTIPLEDUBINS_API_DLL __declspec(dllimport)
    #else
      #define MULTIPLEDUBINS_API_DLL
    #endif
  #endif
...
namespace MultipleDubinsDefine {
  ...
  class MultipleDubins : public Mechatronix::Discretized_Indirect_OCP {
    ...
    Mechatronix::DiffModuleN diff2pi;
    ...
    MultipleDubins( MultipleDubins const & );
    MultipleDubins const & operator = ( MultipleDubins const & );
  ...
  public:
    ...
    MULTIPLEDUBINS_API_DLL virtual
    ~MultipleDubins() MECHATRONIX_OVERRIDE;

    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    MULTIPLEDUBINS_API_DLL virtual
    char const * model_name() const MECHATRONIX_OVERRIDE
    { return "MultipleDubins"; }
...
namespace MultipleDubinsLoad {
  using MultipleDubinsDefine::MultipleDubins;

}

#endif

// EOF: MultipleDubins.hh
