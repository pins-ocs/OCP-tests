...
namespace BrachiostocronaDefine {
  ...
  Brachiostocrona::g_eval(
    ...
    real_type t4   = ALIAS_vthetaControl_D_1(U__[0], -10, 10);
    result__[ 0   ] = t4 * P__[0] + L__[3];
    if ( m_debug )
  ...
  Brachiostocrona::DgDxlp_sparse(
    ...
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    result__[ 0   ] = 1;
    result__[ 1   ] = ALIAS_vthetaControl_D_1(U__[0], -10, 10);
    if ( m_debug )
  ...
  Brachiostocrona::DgDu_sparse(
    ...
    real_type const * L__ = NODE__.lambda;
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    real_type t3   = ALIAS_vthetaControl_D_1_1(U__[0], -10, 10);
    result__[ 0   ] = t3 * P__[0];
    if ( m_debug )
  ...
  Brachiostocrona::u_eval_analytic(
    ...
    real_type const * L__ = NODE__.lambda;
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    U__[ iU_vtheta ] = vthetaControl.solve(-L__[3] / P__[0], -10, 10);
  }
  ...
  Brachiostocrona::DuDxlp_full_analytic(
    ...
    DuDxlp(0, 5) = 0;
    DuDxlp(0, 6) = 0;
    DuDxlp(0, 7) = -vthetaControl.solve_rhs(-L__[3] / P__[0], -10, 10) / P__[0];
    DuDxlp(0, 8) = vthetaControl.solve_rhs(-L__[3] / P__[0], -10, 10) * L__[3] * pow(P__[0], -2);
  }
  ...
  bool
  Brachiostocrona::DuDxlp_full_numeric(
    NodeType2 const          & NODE__,
    P_const_pointer_type       P__,
    U_const_pointer_type       U__,
    MatrixWrapper<real_type> & DuDxlp
  ...
  Brachiostocrona::jump_eval(
    ...
    real_type t13  = ModelPars[3];
    result__[ 6   ] = -LL__[2] * t13 + LR__[2] * t13;
