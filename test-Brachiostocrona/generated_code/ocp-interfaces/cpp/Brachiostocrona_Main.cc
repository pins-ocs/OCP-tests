...
main() {
  ...
  try {
    Brachiostocrona * m_model = new Brachiostocrona("Brachiostocrona",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/Brachiostocrona_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/Brachiostocrona_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/Brachiostocrona_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
