/*-----------------------------------------------------------------------*\
 |  file: Brachiostocrona_dll_ruby.cc                                    |
 |                                                                       |
...
#include "Brachiostocrona_dll_ruby.hh"
#include <map>
...
namespace BrachiostocronaDefine {
  ...
  static map< string, Brachiostocrona_Problem * > problems;
  ...
  */
  ...
  Brachiostocrona_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, Brachiostocrona_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  BRACHIOSTOCRONA_API_DLL
  bool
  Brachiostocrona_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, Brachiostocrona_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "Brachiostocrona_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  Brachiostocrona_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("Brachiostocrona_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  Brachiostocrona_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, Brachiostocrona_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("Brachiostocrona_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: Brachiostocrona_dll_ruby.cc
