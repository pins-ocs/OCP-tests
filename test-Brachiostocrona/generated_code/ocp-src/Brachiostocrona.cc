...
namespace BrachiostocronaDefine {
  ...
  */
  void
  Brachiostocrona::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "Brachiostocrona::setup_parameters: Missing key `Parameters` in data\n"
    );
  ...
  void
  Brachiostocrona::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  Brachiostocrona::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  Brachiostocrona::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "Brachiostocrona::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  Brachiostocrona::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "Brachiostocrona::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in Brachiostocrona::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  Brachiostocrona::info_model( integer level ) const {
    ...
    m_console->message( vthetaControl.info(),level);

    // ==================================================================
    m_console->PINS_COLOR_TITLE( fmt::format( "+{:.^71}+\n", "Constraints type LT" ), level);
    m_console->message( LowBound.info(),level);
