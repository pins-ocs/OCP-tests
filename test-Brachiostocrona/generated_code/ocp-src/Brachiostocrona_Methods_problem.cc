...
namespace BrachiostocronaDefine {
  ...
  Brachiostocrona::continuation_step_0( real_type s ) {
    ...
    ModelPars[iM_mu] = pow_average__xo(s, ModelPars[iM_mu0], ModelPars[iM_mu1]);
  }
  ...
  Brachiostocrona::H_eval(
    ...
    real_type t9   = pow(X__[iX_x] * ModelPars[iM_slope_low] + t5 - X__[iX_y] + ModelPars[iM_y0_low], 2);
    ...
    real_type t21  = X__[iX_theta];
    real_type t22  = cos(t21);
    real_type t27  = sin(t21);
    real_type result__ = t22 * t20 * t18 * L__[iL_lambda1__xo] + t27 * t20 * t18 * L__[iL_lambda2__xo] - t27 * ModelPars[iM_g] * t18 * L__[iL_lambda3__xo] - t11 * t15 + t9 * ModelPars[iM_w_ARG] + L__[iL_lambda4__xo] * U__[iU_vtheta] + t14;
    if ( m_debug ) {
  ...
  Brachiostocrona::lagrange_target(
    ...
    real_type t14  = exp(-1.0 / t11 * t5);
    real_type t15  = log(t5);
    real_type result__ = -t11 * t15 + t9 * ModelPars[iM_w_ARG] + t14;
    if ( m_debug ) {
  ...
  integer Brachiostocrona::DmayerDxxp_numEqns() const { return 9; }
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Brachiostocrona::D2mayerD2xxp_numRows() const { return 9; }
  integer Brachiostocrona::D2mayerD2xxp_numCols() const { return 9; }
  integer Brachiostocrona::D2mayerD2xxp_nnz()     const { return 0; }

  void
  Brachiostocrona::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }
  ...
  Brachiostocrona::DlagrangeDxpu_eval(
    ...
    result__[ 4   ] = 0;
    result__[ 5   ] = 0;
    real_type t12  = ModelPars[iM_mu];
    real_type t13  = 1.0 / t12;
    real_type t15  = exp(-t13 * t5);
    result__[ 6   ] = t11 - t15 * t13 - 1.0 / t5 * t12;
    if ( m_debug )
  ...
  Brachiostocrona::D2lagrangeD2xpu_sparse(
    ...
    real_type t7   = ModelPars[iM_mu];
    real_type t8   = t7 * t7;
    real_type t10  = U__[iU_sz];
    real_type t13  = exp(-1.0 / t7 * t10);
    real_type t15  = t10 * t10;
    result__[ 8   ] = result__[4] + t13 / t8 + 1.0 / t15 * t7;
    if ( m_debug )
  ...
  integer Brachiostocrona::segmentLink_numEqns() const { return 0; }
  ...
  integer Brachiostocrona::jump_numEqns() const { return 8; }
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Brachiostocrona::DjumpDxlxlp_numRows() const { return 8; }
  integer Brachiostocrona::DjumpDxlxlp_numCols() const { return 17; }
  integer Brachiostocrona::DjumpDxlxlp_nnz()     const { return 16; }

  void
  Brachiostocrona::DjumpDxlxlp_pattern( integer iIndex[], integer jIndex[] ) const {
    iIndex[0 ] = 0   ; jIndex[0 ] = 0   ;
  ...
  integer Brachiostocrona::post_numEqns() const { return 2; }
  ...
  Brachiostocrona::post_eval(
    ...
    result__[ 1   ] = ModelPars[iM_slope_low] * X__[iX_x] + ModelPars[iM_y0_low];
    // do not check
  ...
  integer Brachiostocrona::integrated_post_numEqns() const { return 0; }
