...
main() {
  ...
  try {
    ...
    ROSS_Ball_Mizel  model("ROSS_Ball_Mizel",&console,&TP);
    ...
    // Auxiliary values
    real_type kappa = 0.99;
    real_type epsilon = max(0,4096/531441.0*kappa^12*(-kappa^3+1)*(13*kappa^3-7));
    integer InfoLevel = 4;
    ...
    gc_data["DumpFile"] = "ROSS_Ball_Mizel_dump";
...
ROSS_Ball_Mizel_data.Mesh["s0"] = 0;
ROSS_Ball_Mizel_data.Mesh["segments"][0]["n"] = 100;
ROSS_Ball_Mizel_data.Mesh["segments"][0]["length"] = 0.1;
ROSS_Ball_Mizel_data.Mesh["segments"][1]["n"] = 100;
ROSS_Ball_Mizel_data.Mesh["segments"][1]["length"] = 0.9;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/ROSS_Ball_Mizel_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/ROSS_Ball_Mizel_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
