...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libROSS_Ball_Mizel.#{DYLIB_EXT}"
...
module ROSS_Ball_Mizel
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libROSS_Ball_Mizel.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ROSS_Ball_Mizel.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ROSS_Ball_Mizel.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ROSS_Ball_Mizel.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ROSS_Ball_Mizel.OCP_write_ocp_solution( @id, fname )
    end
