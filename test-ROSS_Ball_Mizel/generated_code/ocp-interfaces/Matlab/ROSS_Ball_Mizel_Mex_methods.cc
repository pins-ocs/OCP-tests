...
#include "ROSS_Ball_Mizel_Mex.hh"
...
)
{
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format(
        "ROSS_Ball_Mizel_Mex('set_guess',...), error: {}",
        exc.what()
      ).c_str()
    );
  }
  catch (...) {
    mexErrMsgTxt( "ROSS_Ball_Mizel_Mex('set_guess',...), Unknown error\n" );
  }
  #undef CMD
}
...
)
{
  // per adesso non definisco niente
  #if 0
  #define CMD "Uguess = " MODEL_NAME "_Mex( 'DgDu', obj, x ): "
  UTILS_MEX_ASSERT0( guess_ok, CMD "use 'set_guess' before to use 'DgDu'" );
  CHECK_IN_OUT( 3, 1 );
...
)
{
  // per adesso non definisco niente
  #if 0
  #define CMD "Uguess = " MODEL_NAME "_Mex( 'DgDxpm', obj, x ): "
  UTILS_MEX_ASSERT0( guess_ok, CMD "use 'set_guess' before to use 'DgDxpm'" );
  CHECK_IN_OUT( 3, 1 );
...
)
{
  ...
  try {
    ...
    string msg = fmt::format( "ROSS_Ball_Mizel_Mex('eval_F',...) error: {}", exc.what() );
    mexWarnMsgTxt( msg.c_str() );
    ok = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "ROSS_Ball_Mizel_Mex('eval_F',...) unkown error\n" );
    ok = false;
  }
...
)
{
  ...
  try {
    ...
    mexWarnMsgTxt( fmt::format( "ROSS_Ball_Mizel_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
    ok_value = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "ROSS_Ball_Mizel_Mex('eval_JF',...) unkown error\n" );
    ok_value = false;
  }
...
)
{
  ...
  try {
    ...
    mexWarnMsgTxt( fmt::format( "ROSS_Ball_Mizel_Mex('eval_JF2',...) error: {}", exc.what() ).c_str() );
    ok_value = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "ROSS_Ball_Mizel_Mex('eval_JF2',...) unkown error\n" );
    ok_value = false;
  }
  int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
  UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
...
)
{
  ...
  UTILS_MEX_ASSERT0( setup_ok, CMD "use 'setup' before to use 'DgDu_pattern'" );
  CHECK_IN_OUT( 2, 1 );
...
)
{
  ...
  UTILS_MEX_ASSERT0( setup_ok, CMD "use 'setup' before to use 'DgDxpm_pattern'" );
  CHECK_IN_OUT( 2, 1 );
