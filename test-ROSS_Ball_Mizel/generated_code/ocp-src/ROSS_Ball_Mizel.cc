...
namespace ROSS_Ball_MizelDefine {
  ...
  */
  ROSS_Ball_Mizel::ROSS_Ball_Mizel(
    string const   & name,
  ...
  */
  void
  ROSS_Ball_Mizel::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "ROSS_Ball_Mizel::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  ROSS_Ball_Mizel::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "ROSS_Ball_Mizel::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in ROSS_Ball_Mizel::setup not all parameters are set!\n"
    );
  }

  void
  ROSS_Ball_Mizel::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  ROSS_Ball_Mizel::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  ROSS_Ball_Mizel::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  ROSS_Ball_Mizel::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  ROSS_Ball_Mizel::setup_controls( GenericContainer const & gc_data ) {
  ...
  */
  void
  ROSS_Ball_Mizel::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "ROSS_Ball_Mizel::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in ROSS_Ball_Mizel::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  ROSS_Ball_Mizel::info_model( integer level ) const {
  ...
  */
  void
  ROSS_Ball_Mizel::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("ROSS_Ball_Mizel::setup, Debug");
  ...
  */
  void
  ROSS_Ball_Mizel::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  ROSS_Ball_Mizel::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
