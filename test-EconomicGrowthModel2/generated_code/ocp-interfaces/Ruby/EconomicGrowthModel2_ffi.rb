#-----------------------------------------------------------------------#
#  file: EconomicGrowthModel2_run.rb                                    #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libEconomicGrowthModel2.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libEconomicGrowthModel2.#{DYLIB_EXT}"
end
...
module EconomicGrowthModel2
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libEconomicGrowthModel2.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = EconomicGrowthModel2.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = EconomicGrowthModel2.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = EconomicGrowthModel2.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      EconomicGrowthModel2.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: EconomicGrowthModel2_ffi.rb
