...
public:
  ...
  do_eval_JF(
    ...
    try {
      ...
      mexWarnMsgTxt( "EconomicGrowthModel2_Mex('eval_JF',...) unkown error\n" );
      ok_value = false;
    }
    int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
    MEX_ASSERT( ok == 0, CMD "failed the call sparse(...)" );
  ...
  do_explog(
    ...
    CHECK_IN_OUT( 5, 1 );
    mwSize N0, M0;
    real_type const * arg0 = getMatrixPointer( arg_in_2, N0, M0, CMD " xo__a" );
    mwSize N1, M1;
    real_type const * arg1 = getMatrixPointer( arg_in_3, N1, M1, CMD " xo__b" );
    MEX_CHECK_DIMS(N1,M1,N0,M0);
    mwSize N2, M2;
    real_type const * arg2 = getMatrixPointer( arg_in_4, N2, M2, CMD " xo__s" );
