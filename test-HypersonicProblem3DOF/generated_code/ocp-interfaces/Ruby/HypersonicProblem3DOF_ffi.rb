#-----------------------------------------------------------------------#
#  file: HypersonicProblem3DOF_run.rb                                   #
#                                                                       #
...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libHypersonicProblem3DOF.#{DYLIB_EXT}"
...
module HypersonicProblem3DOF
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libHypersonicProblem3DOF.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = HypersonicProblem3DOF.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = HypersonicProblem3DOF.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = HypersonicProblem3DOF.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      HypersonicProblem3DOF.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: HypersonicProblem3DOF_ffi.rb
