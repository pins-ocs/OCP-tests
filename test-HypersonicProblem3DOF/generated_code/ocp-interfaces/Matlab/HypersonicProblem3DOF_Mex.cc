...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"h_guess_D",&ProblemStorage::do_h_guess_D},
  {"theta_guess",&ProblemStorage::do_theta_guess},
  {"theta_guess_D",&ProblemStorage::do_theta_guess_D},
  {"phi_guess",&ProblemStorage::do_phi_guess},
  {"phi_guess_D",&ProblemStorage::do_phi_guess_D},
  {"V_guess",&ProblemStorage::do_V_guess},
  {"V_guess_D",&ProblemStorage::do_V_guess_D},
  {"G_guess",&ProblemStorage::do_G_guess},
  {"G_guess_D",&ProblemStorage::do_G_guess_D},
  {"psi_guess",&ProblemStorage::do_psi_guess},
  {"psi_guess_D",&ProblemStorage::do_psi_guess_D},
  {"G_bound",&ProblemStorage::do_G_bound},
  {"G_bound_D",&ProblemStorage::do_G_bound_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "HypersonicProblem3DOF_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
