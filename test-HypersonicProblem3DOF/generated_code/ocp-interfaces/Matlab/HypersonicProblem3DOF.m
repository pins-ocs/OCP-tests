...
classdef HypersonicProblem3DOF < handle
  ...
  methods
    ...
    function res = get_guess( self )
      ...
      %
      res = HypersonicProblem3DOF_Mex( 'get_guess', self.objectHandle );
    end
    %
    % ---------------------------------------------------------------------
    %
    function res = get_solution_as_guess( self )
      ...
      %
      res = HypersonicProblem3DOF_Mex( 'get_solution_as_guess', self.objectHandle );
    end
    ...
    function MU_U = guess_MU_U( self, Z )
      MU_U = HypersonicProblem3DOF_Mex( 'guess_MU_U', self.objectHandle, Z );
    end
    % ---------------------------------------------------------------------
    ...
    function MU_U = eval_MU_U( self, Z, u_guess )
      MU_U = HypersonicProblem3DOF_Mex( 'eval_MU_U', self.objectHandle, Z, u_guess );
    end
    % ---------------------------------------------------------------------
    ...
    function [F,ok] = eval_F( self, Z, varargin )
      ...
      [F,ok] = HypersonicProblem3DOF_Mex( 'eval_F', self.objectHandle, Z, MU_U );
    end
    % ---------------------------------------------------------------------
    function [JF,ok] = eval_JF( self, Z, varargin )
      ...
      [JF,ok] = HypersonicProblem3DOF_Mex( 'eval_JF', self.objectHandle, Z, MU_U );
    end
    % ---------------------------------------------------------------------
    ...
    function [JF,ok] = eval_JF2( self, Z, varargin )
      ...
      [JF,ok] = HypersonicProblem3DOF_Mex( 'eval_JF2', self.objectHandle, Z, MU_U );
    end
    % ---------------------------------------------------------------------
    function JF = eval_JF2_pattern( self )
      ...
      % system of the indirect methods.
      %
      JF = HypersonicProblem3DOF_Mex( 'eval_JF2_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function [ Z, MU, U ] = get_raw_solution( self )
      ...
      %
      [ Z, MU_U ] = HypersonicProblem3DOF_Mex( 'get_raw_solution', self.objectHandle );
    ...
    function set_raw_solution( self, Z, varargin )
      ...
      HypersonicProblem3DOF_Mex( 'set_raw_solution', self.objectHandle, Z, MU_U );
    end
    % ---------------------------------------------------------------------
    ...
    function check_jacobian( self, Z, varargin )
      ...
      HypersonicProblem3DOF_Mex( 'check_jacobian', self.objectHandle, Z, MU_U, varargin{end} );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    ...
    function [a,b,c] = eval_abc( self, L, R, pars, varargin )
      ...
      [a,b,c] = HypersonicProblem3DOF_Mex( 'abc', self.objectHandle, L, R, pars, MU_U );
    end
    % ---------------------------------------------------------------------
    function DabcDxlxlpu = eval_DabcDxlxlpu( self, L, R, pars, varargin )
      ...
      DabcDxlxlpu = HypersonicProblem3DOF_Mex( 'DabcDxlxlpu', self.objectHandle, L, R, pars, MU_U );
    end
    % ---------------------------------------------------------------------
    function [h,c] = eval_hc( self, L, R, pars )
      ...
      % <<FD3.jpg>>
      %
      [h,c] = HypersonicProblem3DOF_Mex( 'hc', self.objectHandle, L, R, pars );
    end
    % ---------------------------------------------------------------------
    function [Jh,Jc] = eval_DhcDxlxlop( self, L, R, pars )
      ...
      % <<FD4.jpg>>
      %
      [Jh,Jc] = HypersonicProblem3DOF_Mex( 'DhcDxlxlop', self.objectHandle, L, R, pars );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    ...
    function u = eval_u( self, L, R, pars )
      ...
      %
      if L.iseg == L.iseg
        u = HypersonicProblem3DOF_Mex( 'u', self.objectHandle, L, R, pars );
      else
    ...
    function [DmuDxlxlp, DuDxlxlp] = MU_U_eval_Dxlxlp( self, NODE, pars, MU, U )
      ...
      %
      [DmuDxlxlp, DuDxlxlp] = HypersonicProblem3DOF_Mex( 'MU_U_eval_Dxlxlp', self.objectHandle, NODE, pars, MU, U );
    end
    % ---------------------------------------------------------------------
    ...
    function rhs = eval_ode( self, S, pars, U, V )
      ...
      % S.x
      %
      rhs = HypersonicProblem3DOF_Mex( 'ode', self.objectHandle, S, pars, U, V );
    end
    % ---------------------------------------------------------------------
    %
    function J = eval_DodeDxpuv( self, S, pars, U, V )
      ...
      % respect to `x`.
      %
      J = HypersonicProblem3DOF_Mex( 'DodeDxpuv', self.objectHandle, S, pars, U, V );
    end
    % ---------------------------------------------------------------------
    function A = eval_A( self, S, pars )
      ...
      % respect to `pars`.
      %
      A = HypersonicProblem3DOF_Mex( 'A', self.objectHandle, S, pars );
    end
    % ---------------------------------------------------------------------
    function bc = eval_bc( self, L, R, pars )
      bc = HypersonicProblem3DOF_Mex( 'bc', self.objectHandle, L, R, pars );
    end
    % ---------------------------------------------------------------------
    function J = eval_DbcDxxp( self, L, R, pars )
      J = HypersonicProblem3DOF_Mex( 'DbcDxxp', self.objectHandle, L, R, pars );
    end
    % ---------------------------------------------------------------------
    function target = eval_lagrange_target( self, S, pars, U )
      target = HypersonicProblem3DOF_Mex( 'lagrange_target', self.objectHandle, S, pars, U );
    end
    % ---------------------------------------------------------------------
    function DlagrangeDxpu = eval_DlagrangeDxpu( self, S, pars, U )
      DlagrangeDxpu = HypersonicProblem3DOF_Mex( 'DlagrangeDxpu', self.objectHandle, S, pars, U );
    end
    % ---------------------------------------------------------------------
    function D2lagrangeD2xpu = eval_D2lagrangeD2xpu( self, S, pars, U )
      D2lagrangeD2xpu = HypersonicProblem3DOF_Mex( 'D2lagrangeD2xpu', self.objectHandle, S, pars, U );
    end
    %
    %   ____  _               _
    %  |  _ \(_)_ __ ___  ___| |_
    ...
    function J = eval_Dfd_BCDxlxlp( self, L, R, pars, omega )
      J = HypersonicProblem3DOF_Mex( 'Dfd_BCDxlxlp', self.objectHandle, L, R, pars, omega );
    end
    % ---------------------------------------------------------------------
    function target = eval_mayer_target( self, L, R, pars )
      target = HypersonicProblem3DOF_Mex( 'mayer_target', self.objectHandle, L, R, pars );
    end
    % ---------------------------------------------------------------------
    function DmayerDxxp = eval_DmayerDxxp( self, L, R, pars )
      DmayerDxxp = HypersonicProblem3DOF_Mex( 'DmayerDxxp', self.objectHandle, L, R, pars );
    end
    % ---------------------------------------------------------------------
    function D2mayerD2xxp = eval_D2mayerD2xxp( self, L, R, pars )
      D2mayerD2xxp = HypersonicProblem3DOF_Mex( 'D2mayerD2xxp', self.objectHandle, L, R, pars );
    end
    % ---------------------------------------------------------------------
    function c = eval_c( self, S, pars, U )
      ...
      %
      c = HypersonicProblem3DOF_Mex( 'LTargs', self.objectHandle, S, pars, U );
    end
    % ---------------------------------------------------------------------
    function Jc = eval_DcDxpu( self, S, pars, U )
      ...
      %
      Jc = HypersonicProblem3DOF_Mex( 'DLTargsDxpu', self.objectHandle, S, pars, U );
    end
    % ---------------------------------------------------------------------
    function Hc = eval_D2cD2xpu( self, S, pars, U, omega )
      ...
      %
      Hc = HypersonicProblem3DOF_Mex( 'D2LTargsD2xpu', self.objectHandle, S, pars, U, omega );
    end
    %
    %
    ...
    function Hxp = eval_Hxp( self, S, pars, MU, U, V )
      ...
      %
      Hxp = HypersonicProblem3DOF_Mex( 'Hxp', self.objectHandle, S, pars, MU, U, V );
    end
    % ---------------------------------------------------------------------
    function J = eval_DHxpDxpuv( self, S, pars, MU, U, V )
      ...
      %
      J = HypersonicProblem3DOF_Mex( 'DHxpDxpuv', self.objectHandle, S, pars, MU, U, V );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    function J = eval_penalties( self, S, pars, U )
      ...
      %
      J = HypersonicProblem3DOF_Mex( 'JP', self.objectHandle, S, pars, U );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    function J = eval_control_penalties( self, S, pars, U )
      ...
      %
      J = HypersonicProblem3DOF_Mex( 'JU', self.objectHandle, S, pars, U );
    end
    % ---------------------------------------------------------------------
    function JP = eval_JP( self, S, pars, U )
      JP = HypersonicProblem3DOF_Mex( 'JP', self.objectHandle, S, pars, U );
    end
    % ---------------------------------------------------------------------
    function JU = eval_JU( self, S, pars, U )
      JU = HypersonicProblem3DOF_Mex( 'JU', self.objectHandle, S, pars, U );
    end
    % ---------------------------------------------------------------------
    function LT = eval_LT( self, S, pars, U )
      LT = HypersonicProblem3DOF_Mex( 'LT', self.objectHandle, S, pars, U );
    end
    % ---------------------------------------------------------------------
    function LTargs = eval_LTargs( self, S, pars, U )
      LTargs = HypersonicProblem3DOF_Mex( 'LTargs', self.objectHandle, S, pars, U );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    function jmp = eval_jump( self, L, R, pars )
      jmp = HypersonicProblem3DOF_Mex( 'jump', self.objectHandle, L, R, pars );
    end
    % ---------------------------------------------------------------------
    function J = eval_DjumpDxlxlp( self, L, R, pars )
      J = HypersonicProblem3DOF_Mex( 'DjumpDxlxlp', self.objectHandle, L, R, pars );
    end
    % ---------------------------------------------------------------------
    function target = eval_q( self, S )
      ...
      % S.s
      %
      target = HypersonicProblem3DOF_Mex( 'mesh_functions', self.objectHandle, S );
    end
    % ---------------------------------------------------------------------
    ...
    function res = D2lagrangeD2xpu_pattern( self )
      res = HypersonicProblem3DOF_Mex('D2lagrangeD2xpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = D2mayerD2xxp_pattern( self )
      res = HypersonicProblem3DOF_Mex('D2mayerD2xxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = Dfd_BCDxlxlp_pattern( self )
      res = HypersonicProblem3DOF_Mex('Dfd_BCDxlxlp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DgDxpm_pattern( self )
      res = HypersonicProblem3DOF_Mex('DgDxpm_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DHxpDxpuv_pattern( self )
      res = HypersonicProblem3DOF_Mex('DHxpDxpuv_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DodeDxpuv_pattern( self )
      res = HypersonicProblem3DOF_Mex('DodeDxpuv_pattern', self.objectHandle );
    end
