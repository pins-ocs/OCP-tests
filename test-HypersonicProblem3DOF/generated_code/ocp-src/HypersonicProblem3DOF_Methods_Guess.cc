...
namespace HypersonicProblem3DOFDefine {
  ...
  void
  HypersonicProblem3DOF::p_guess_eval( P_p_type P__ ) const {
    P__[ iP_Tf ] = ModelPars[iM_Tf_guess];
    if ( m_debug )
      Mechatronix::check( P__.pointer(), "p_guess_eval", 1 );
  }

  void
  HypersonicProblem3DOF::xlambda_guess_eval(
    integer        i_segment,
    Q_const_p_type Q__,
    P_const_p_type P__,
    X_p_type       X__,
    ...
    { // open block to avoid temporary clash
      ...
      X__[ iX_h     ] = h_guess(t1);
      X__[ iX_theta ] = theta_guess(t1);
      X__[ iX_phi   ] = phi_guess(t1);
      X__[ iX_V     ] = V_guess(t1);
      X__[ iX_G     ] = G_guess(t1);
      X__[ iX_psi   ] = psi_guess(t1);
      X__[ iX_sigma ] = ModelPars[iM_sigma_i];
    }
    { // open block to avoid temporary clash
  ...
  bool
  HypersonicProblem3DOF::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  HypersonicProblem3DOF::xlambda_check_node(
    integer         ipos,
  ...
  bool
  HypersonicProblem3DOF::penalties_check(
    NodeQX const & NODE__,
    P_const_p_type P__
  ) const {
    ...
    ok = ok && G_bound_min.check_range(-0.314159265358979323846264338328e1 - t1);
    ok = ok && G_bound_max.check_range(t1 - 0.314159265358979323846264338328e1);
    return ok;
  }
  ...
  integer HypersonicProblem3DOF::u_guess_numEqns() const { return 9; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  HypersonicProblem3DOF::u_guess_eval(
    NodeQXL const & NODE__,
    P_const_p_type  P__,
    ...
    std::fill_n( UGUESS__.pointer(), 9, 0 );
    UGUESS__[ iU_alpha   ] = 0;
    UGUESS__[ iU_u2      ] = 0;
    UGUESS__[ iU_c_h     ] = 0;
    UGUESS__[ iU_c_theta ] = 0;
    UGUESS__[ iU_c_phi   ] = 0;
    UGUESS__[ iU_c_V     ] = 0;
    UGUESS__[ iU_c_G     ] = 0;
    UGUESS__[ iU_c_psi   ] = 0;
    UGUESS__[ iU_c_sigma ] = 0;
    if ( m_debug )
      Mechatronix::check_in_segment( UGUESS__.pointer(), "u_guess_eval", 9, i_segment );
  }
  ...
  bool
  HypersonicProblem3DOF::u_check_if_admissible(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__
