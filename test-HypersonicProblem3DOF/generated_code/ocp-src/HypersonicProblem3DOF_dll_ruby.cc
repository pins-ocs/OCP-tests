/*-----------------------------------------------------------------------*\
 |  file: HypersonicProblem3DOF_dll_ruby.cc                              |
 |                                                                       |
...
#include "HypersonicProblem3DOF_dll_ruby.hh"
#include <map>
...
namespace HypersonicProblem3DOFDefine {
  ...
  static map< string, HypersonicProblem3DOF_Problem * > problems;
  ...
  */
  ...
  HypersonicProblem3DOF_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, HypersonicProblem3DOF_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  HYPERSONICPROBLEM3DOF_API_DLL
  bool
  HypersonicProblem3DOF_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, HypersonicProblem3DOF_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "HypersonicProblem3DOF_ocp_guess" ) );
      return true;
    }
  }
  ...
  HypersonicProblem3DOF_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("HypersonicProblem3DOF_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  HypersonicProblem3DOF_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, HypersonicProblem3DOF_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("HypersonicProblem3DOF_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: HypersonicProblem3DOF_dll_ruby.cc
