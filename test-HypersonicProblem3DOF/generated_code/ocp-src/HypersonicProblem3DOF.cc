...
namespace HypersonicProblem3DOFDefine {
  ...
  */
  ...
  char const *namesPostProcess[numPostProcess+1] = {
    ...
    "longitude [deg]",
    "latitude [deg]",
    "velocity flight-path angle [deg]",
    "velocity heading angle [deg]",
    "bank angle [deg]",
    "CL",
    "CD",
    "G_PI",
  ...
  */
  ...
  , u2Control("u2Control")
  // Constraints LT
  , G_bound_min("G_bound_min")
  , G_bound_max("G_bound_max")
  // Constraints 1D
  // Constraints 2D
  // User classes
  {
  ...
  */
  void
  HypersonicProblem3DOF::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "HypersonicProblem3DOF::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  HypersonicProblem3DOF::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "HypersonicProblem3DOF::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in HypersonicProblem3DOF::setup not all parameters are set!\n"
    );
  }

  void
  HypersonicProblem3DOF::setup_parameters( real_type const Pars[] ) {
  ...
  */
  void
  HypersonicProblem3DOF::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "HypersonicProblem3DOF::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("G_bound_min"),
      "in HypersonicProblem3DOF::setup_classes(gc) missing key: ``G_bound_min''\n"
    );
    G_bound_min.setup( gc("G_bound_min") );

    UTILS_ASSERT0(
      gc.exists("G_bound_max"),
      "in HypersonicProblem3DOF::setup_classes(gc) missing key: ``G_bound_max''\n"
    );
    G_bound_max.setup( gc("G_bound_max") );
  ...
  */
  void
  HypersonicProblem3DOF::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  HypersonicProblem3DOF::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  HypersonicProblem3DOF::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "HypersonicProblem3DOF::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  HypersonicProblem3DOF::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "HypersonicProblem3DOF::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in HypersonicProblem3DOF::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  HypersonicProblem3DOF::info_classes() const {
    int msg_level = 3;

    m_console->message("\nControls\n",msg_level);
    m_console->message( u2Control.info(),msg_level);

    m_console->message("\nConstraints LT\n",msg_level);
    m_console->message( G_bound_min.info(),msg_level);
    m_console->message( G_bound_max.info(),msg_level);
  ...
  */
  void
  HypersonicProblem3DOF::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("HypersonicProblem3DOF::setup, Debug");
  ...
  */
  void
  HypersonicProblem3DOF::get_names( GenericContainer & out ) const {
  ...
  void
  HypersonicProblem3DOF::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
