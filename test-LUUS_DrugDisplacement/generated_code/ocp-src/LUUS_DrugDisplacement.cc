...
namespace LUUS_DrugDisplacementDefine {
  ...
  */
  LUUS_DrugDisplacement::LUUS_DrugDisplacement(
    string const   & name,
  ...
  */
  void
  LUUS_DrugDisplacement::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "LUUS_DrugDisplacement::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  LUUS_DrugDisplacement::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "LUUS_DrugDisplacement::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in LUUS_DrugDisplacement::setup not all parameters are set!\n"
    );
  }

  void
  LUUS_DrugDisplacement::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  LUUS_DrugDisplacement::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  LUUS_DrugDisplacement::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  LUUS_DrugDisplacement::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  LUUS_DrugDisplacement::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "LUUS_DrugDisplacement::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  LUUS_DrugDisplacement::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "LUUS_DrugDisplacement::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in LUUS_DrugDisplacement::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  LUUS_DrugDisplacement::info_model( integer level ) const {
  ...
  */
  void
  LUUS_DrugDisplacement::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("LUUS_DrugDisplacement::setup, Debug");
  ...
  */
  void
  LUUS_DrugDisplacement::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  LUUS_DrugDisplacement::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
