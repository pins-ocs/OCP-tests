/*-----------------------------------------------------------------------*\
 |  file: LUUS_DrugDisplacement_dll_ruby.cc                              |
 |                                                                       |
...
#include "LUUS_DrugDisplacement_dll_ruby.hh"
#include <map>
...
namespace LUUS_DrugDisplacementDefine {
  ...
  static map< string, LUUS_DrugDisplacement_Problem * > problems;
  ...
  */
  ...
  LUUS_DrugDisplacement_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, LUUS_DrugDisplacement_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  LUUS_DRUGDISPLACEMENT_API_DLL
  bool
  LUUS_DrugDisplacement_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, LUUS_DrugDisplacement_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "LUUS_DrugDisplacement_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  LUUS_DrugDisplacement_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("LUUS_DrugDisplacement_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  LUUS_DrugDisplacement_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, LUUS_DrugDisplacement_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("LUUS_DrugDisplacement_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: LUUS_DrugDisplacement_dll_ruby.cc
