...
namespace LUUS_DrugDisplacementDefine {
  ...
  void
  LUUS_DrugDisplacement::p_guess_eval( P_p_type P__ ) const {
    P__[ iP_T ] = 300;
    if ( m_debug )
  ...
  bool
  LUUS_DrugDisplacement::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  LUUS_DrugDisplacement::penalties_check_node(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__
  ) const {
  ...
  LUUS_DrugDisplacement::u_guess_eval(
    ...
    UGUESS__[ iU_u ] = uControl.solve(-0.2000000000e0 * (0.232e3 * t1 * MU__[0] + 5.0 * t8 * MU__[1]) * t3, 0, 8);
    if ( m_debug )
