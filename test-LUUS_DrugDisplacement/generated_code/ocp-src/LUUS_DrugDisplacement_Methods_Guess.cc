...
namespace LUUS_DrugDisplacementDefine {
  ...
  void
  LUUS_DrugDisplacement::p_guess_eval( P_pointer_type P__ ) const {
    P__[ iP_T ] = 300;
    if ( m_debug )
  ...
  void
  LUUS_DrugDisplacement::xlambda_guess_eval(
    integer              i_segment,
  ...
  bool
  LUUS_DrugDisplacement::p_check( P_const_pointer_type P__ ) const {
    return true;
  }
  ...
  bool
  LUUS_DrugDisplacement::xlambda_check_node(
    integer              ipos,
  ...
  bool
  LUUS_DrugDisplacement::xlambda_check_cell(
    integer              icell,
  ...
  integer LUUS_DrugDisplacement::u_guess_numEqns() const { return 1; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  LUUS_DrugDisplacement::u_guess_eval(
    NodeType2 const    & LEFT__,
    NodeType2 const    & RIGHT__,
    P_const_pointer_type P__,
    ...
    UGUESS__[ iU_u ] = uControl.solve(-0.2320000000e2 * t2 * t4 * t1 - 0.2320000000e2 * t8 * t10 * t1 - 0.5000000000e0 * t16 * t4 * t14 - 0.5000000000e0 * t20 * t10 * t14, 0, 8);
    if ( m_debug )
  ...
  bool
  LUUS_DrugDisplacement::u_check_if_admissible(
    NodeType2 const    & NODE__,
    ...
    ok = ok && uControl.check_range(U__[iU_u], 0, 8);
    return ok;
  }
