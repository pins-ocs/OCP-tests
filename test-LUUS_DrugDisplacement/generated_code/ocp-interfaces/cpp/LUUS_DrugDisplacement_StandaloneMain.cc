...
main() {
  ...
  try {
    ...
    LUUS_DrugDisplacement model("LUUS_DrugDisplacement",n_threads,&console);
    ...
    gc_data["DumpFile"] = "LUUS_DrugDisplacement_dump";
    ...
    // Continuation Parameters
    data_Parameters["pen_u_epsi0"] = pen_u_epsi0;
    data_Parameters["pen_u_epsi1"] = 1e-08;
    data_Parameters["pen_u_tol0"] = pen_u_tol0;
    data_Parameters["pen_u_tol1"] = 1e-08;
...
LUUS_DrugDisplacement_data.Mesh["segments"][0]["n"] = 200;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/LUUS_DrugDisplacement_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/LUUS_DrugDisplacement_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
