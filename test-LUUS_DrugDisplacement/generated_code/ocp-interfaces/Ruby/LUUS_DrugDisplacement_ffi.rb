#-----------------------------------------------------------------------#
#  file: LUUS_DrugDisplacement_run.rb                                   #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libLUUS_DrugDisplacement.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libLUUS_DrugDisplacement.#{DYLIB_EXT}"
end
...
module LUUS_DrugDisplacement
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libLUUS_DrugDisplacement.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = LUUS_DrugDisplacement.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = LUUS_DrugDisplacement.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = LUUS_DrugDisplacement.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      LUUS_DrugDisplacement.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: LUUS_DrugDisplacement_ffi.rb
