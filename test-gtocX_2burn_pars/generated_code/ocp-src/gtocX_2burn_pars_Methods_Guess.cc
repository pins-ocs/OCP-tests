...
namespace gtocX_2burn_parsDefine {
  ...
  void
  gtocX_2burn_pars::p_guess_eval( P_p_type P__ ) const {
    P__[ iP_p ] = p_guess(0);
    P__[ iP_h ] = h_guess(0);
    P__[ iP_k ] = k_guess(0);
    if ( m_debug )
  ...
  bool
  gtocX_2burn_pars::p_check( P_const_p_type P__ ) const {
    /* REMOVED */ Xoptima__check__pars__lt(0, P__[iP_p], Xoptima__message_pars_check_0);
    return true;
  }
  ...
  bool
  gtocX_2burn_pars::penalties_check(
    NodeQX const & NODE__,
    P_const_p_type P__
  ) const {
  ...
  integer gtocX_2burn_pars::u_guess_numEqns() const { return 0; }
