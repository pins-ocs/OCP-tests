...
namespace gtocX_2burn_parsDefine {
  ...
  */
  void
  gtocX_2burn_pars::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "gtocX_2burn_pars::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in gtocX_2burn_pars::setup not all parameters are set!\n"
    );
  }

  void
  gtocX_2burn_pars::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  gtocX_2burn_pars::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "gtocX_2burn_pars::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("ray_positive"),
      "in gtocX_2burn_pars::setup_classes(gc) missing key: ``ray_positive''\n"
    );
    ray_positive.setup( gc("ray_positive") );
  ...
  */
  void
  gtocX_2burn_pars::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  gtocX_2burn_pars::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  gtocX_2burn_pars::setup_controls( GenericContainer const & gc_data ) {
  ...
  */
  void
  gtocX_2burn_pars::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "gtocX_2burn_pars::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in gtocX_2burn_pars::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  gtocX_2burn_pars::info_model( integer level ) const {
    ...
    m_console->message( ray_positive.info(),level);
  ...
  */
  void
  gtocX_2burn_pars::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  gtocX_2burn_pars::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
