...
namespace BangBangFmoduleDefine {
  ...
  BangBangFmodule::g_eval(
    ...
    real_type t1   = L__[1];
    real_type t4   = ALIAS_controlP_D_1(U__[0], 0, ModelPars[1]);
    result__[ 0   ] = 1 + t1 + t4;
    real_type t7   = ALIAS_controlM_D_1(U__[1], 0, ModelPars[0]);
    result__[ 1   ] = 1 - t1 + t7;
  ...
  integer
  BangBangFmodule::DgDu_numRows() const
  { return 2; }

  integer
  BangBangFmodule::DgDu_numCols() const
  { return 2; }

  integer
  BangBangFmodule::DgDu_nnz() const
  { return 2; }
  ...
  BangBangFmodule::DgDu_sparse(
    ...
    real_type const * L__ = NODE__.lambda;
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    result__[ 0   ] = ALIAS_controlP_D_1_1(U__[0], 0, ModelPars[1]);
    result__[ 1   ] = ALIAS_controlM_D_1_1(U__[1], 0, ModelPars[0]);
    #ifdef MECHATRONIX_DEBUG
  ...
  BangBangFmodule::u_eval_analytic(
    ...
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    real_type t1   = L__[1];
    U__[ iU_Fp ] = controlP.solve(-1 - t1, 0, ModelPars[1]);
    U__[ iU_Fm ] = controlM.solve(-1 + t1, 0, ModelPars[0]);
  }
  ...
  BangBangFmodule::DuDxlp_full_analytic(
    ...
    DuDxlp(0, 2) = 0;
    DuDxlp(1, 2) = 0;
    DuDxlp(0, 3) = -controlP.solve_rhs(-1 - L__[1], 0, ModelPars[1]);
    DuDxlp(1, 3) = controlM.solve_rhs(-1 + L__[1], 0, ModelPars[0]);
  }
  ...
  bool
  BangBangFmodule::DuDxlp_full_numeric(
    NodeType2 const          & NODE__,
    P_const_pointer_type       P__,
    U_const_pointer_type       U__,
    MatrixWrapper<real_type> & DuDxlp
    ...
    integer info = alglin::gesv( 2, 4, DgDu.get_data(), 2, ipiv, DuDxlp.get_data(), 2 );
    return info == 0;
  }
