...
main() {
  ...
  try {
    BangBangFmodule * m_model = new BangBangFmodule("BangBangFmodule",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/BangBangFmodule_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/BangBangFmodule_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/BangBangFmodule_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
