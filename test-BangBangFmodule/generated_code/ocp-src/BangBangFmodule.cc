...
namespace BangBangFmoduleDefine {
  ...
  */
  void
  BangBangFmodule::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
  ...
  */
  void
  BangBangFmodule::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "BangBangFmodule::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in BangBangFmodule::setup not all parameters are set!\n"
    );
  }

  void
  BangBangFmodule::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  BangBangFmodule::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  BangBangFmodule::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  BangBangFmodule::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  BangBangFmodule::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "BangBangFmodule::setup_classes: Missing key `Controls` in data\n"
    );
    GenericContainer const & gc = gc_data("Controls");
    controlP.setup( gc("controlP") );
    controlM.setup( gc("controlM") );
  }
  ...
  */
  void
  BangBangFmodule::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "BangBangFmodule::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in BangBangFmodule::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  BangBangFmodule::info_model( integer level ) const {
    ...
    m_console->message( controlP.info(),level);
    m_console->message( controlM.info(),level);
  ...
  */
  void
  BangBangFmodule::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("BangBangFmodule::setup, Debug");
  ...
  */
  void
  BangBangFmodule::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  BangBangFmodule::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
