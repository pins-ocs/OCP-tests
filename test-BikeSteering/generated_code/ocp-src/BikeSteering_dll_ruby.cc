...
#include "BikeSteering_dll_ruby.hh"
#include <map>
...
namespace BikeSteeringDefine {
  ...
  */
  ...
  bool
  BikeSteering_ocp_setup( char const id[], GenericContainer & gc_data ) {
  ...
  BIKESTEERING_API_DLL
  bool
  BikeSteering_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, BikeSteering_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "BikeSteering_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  BikeSteering_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("BikeSteering_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
