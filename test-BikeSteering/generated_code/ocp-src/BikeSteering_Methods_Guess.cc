...
namespace BikeSteeringDefine {
  ...
  void
  BikeSteering::p_guess_eval( P_p_type P__ ) const {
  }
  ...
  bool
  BikeSteering::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  BikeSteering::penalties_check(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    bool ok = true;
    ok = ok && minimumTimeSize.check_range(-X__[iX_TimeSize]);
    return ok;
  }
  ...
  integer BikeSteering::u_guess_numEqns() const { return 1; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  BikeSteering::u_guess_eval(
    NodeQXL const & NODE__,
    P_const_p_type  P__,
    ...
    UGUESS__[ iU_Fy ] = FyControl.solve(MU__[1] * ModelPars[iM_h], -t4, t4);
    if ( m_debug )
  ...
  bool
  BikeSteering::u_check_if_admissible(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__
    ...
    ok = ok && FyControl.check_range(U__[iU_Fy], -t2, t2);
    return ok;
  }
