...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libBikeSteering.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libBikeSteering.#{DYLIB_EXT}"
end
...
module BikeSteering
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libBikeSteering.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = BikeSteering.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = BikeSteering.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = BikeSteering.OCP_solve( @id,
