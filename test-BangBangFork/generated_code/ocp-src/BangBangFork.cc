...
namespace BangBangForkDefine {
  ...
  */
  void
  BangBangFork::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "BangBangFork::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  BangBangFork::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "BangBangFork::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in BangBangFork::setup not all parameters are set!\n"
    );
  }

  void
  BangBangFork::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  BangBangFork::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "BangBangFork::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("Tbarrier"),
      "in BangBangFork::setup_classes(gc) missing key: ``Tbarrier''\n"
    );
    Tbarrier.setup( gc("Tbarrier") );
  ...
  */
  void
  BangBangFork::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  BangBangFork::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  BangBangFork::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "BangBangFork::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  BangBangFork::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "BangBangFork::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in BangBangFork::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  BangBangFork::info_model( integer level ) const {
    ...
    m_console->message( Tbarrier.info(),level);
  ...
  */
  void
  BangBangFork::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("BangBangFork::setup, Debug");
  ...
  */
  void
  BangBangFork::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  BangBangFork::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
