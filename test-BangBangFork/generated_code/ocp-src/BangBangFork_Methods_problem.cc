...
namespace BangBangForkDefine {
  ...
  void
  BangBangFork::continuation_step_1( real_type s ) {
    ModelPars[iM_WC] = ModelPars[iM_WC0] * (1 - s) + ModelPars[iM_WC1] * s;
  }
  /*\
   |   ___         _   _               _   _
   |  / __|___ _ _| |_(_)_ _ _  _ __ _| |_(_)___ _ _
  ...
  void
  BangBangFork::continuation_step_2( real_type s ) {
    ModelPars[iM_WU2] = 0;
    real_type t1   = 1 - s;
    uControl.update_epsilon(s * ModelPars[iM_u_epsi1] + ModelPars[iM_u_epsi0] * t1);
    uControl.update_tolerance(s * ModelPars[iM_u_tol1] + ModelPars[iM_u_tol0] * t1);
  }
  /*\
   |   ___         _   _               _   _
   |  / __|___ _ _| |_(_)_ _ _  _ __ _| |_(_)___ _ _
  ...
  void
  BangBangFork::continuation_step_3( real_type s ) {
    ModelPars[iM_kappa] = s;
  }
  ...
  BangBangFork::lagrange_target(
    ...
    real_const_ptr X__ = NODE__.x;
    MeshStd::SegmentClass const & segment = m_pMesh->get_segment_by_index(i_segment);
    real_type t5   = U__[iU_u2] * U__[iU_u2];
    real_type result__ = t5 * P__[iP_T] * ModelPars[iM_WC];
    if ( m_debug ) {
  ...
  integer BangBangFork::DmayerDxxp_numEqns() const { return 7; }
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer BangBangFork::D2mayerD2xxp_numRows() const { return 7; }
  integer BangBangFork::D2mayerD2xxp_numCols() const { return 7; }
  integer BangBangFork::D2mayerD2xxp_nnz()     const { return 0; }

  void
  BangBangFork::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }
  ...
  integer BangBangFork::DlagrangeDxpu_numEqns() const { return 6; }
  ...
  BangBangFork::DlagrangeDxpu_eval(
    ...
    result__[ 3   ] = t3 * t1;
    result__[ 4   ] = 0;
    result__[ 5   ] = 2 * t2 * P__[iP_T] * t1;
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "DlagrangeDxpu_eval", 6, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer BangBangFork::D2lagrangeD2xpu_numRows() const { return 6; }
  integer BangBangFork::D2lagrangeD2xpu_numCols() const { return 6; }
  integer BangBangFork::D2lagrangeD2xpu_nnz()     const { return 3; }
  ...
  integer BangBangFork::segmentLink_numEqns() const { return 0; }
  ...
  integer BangBangFork::jump_numEqns() const { return 6; }
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer BangBangFork::DjumpDxlxlp_numRows() const { return 6; }
  integer BangBangFork::DjumpDxlxlp_numCols() const { return 13; }
  integer BangBangFork::DjumpDxlxlp_nnz()     const { return 12; }

  void
  BangBangFork::DjumpDxlxlp_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  integer BangBangFork::post_numEqns() const { return 3; }
  ...
  integer BangBangFork::integrated_post_numEqns() const { return 0; }
...
// EOF: BangBangFork_Methods_problem.cc
