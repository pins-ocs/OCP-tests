...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libBangBangFork.#{DYLIB_EXT}"
...
module BangBangFork
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libBangBangFork.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = BangBangFork.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = BangBangFork.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = BangBangFork.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      BangBangFork.OCP_write_ocp_solution( @id, fname )
    end
