...
)
{
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format(
        "BangBangFork_Mex('set_guess',...), error: {}",
        exc.what()
      ).c_str()
    );
  }
  catch (...) {
    mexErrMsgTxt( "BangBangFork_Mex('set_guess',...), Unknown error\n" );
  }
  #undef CMD
}
...
)
{
  ...
  try {
    ...
    string msg = fmt::format( "BangBangFork_Mex('eval_F',...) error: {}", exc.what() );
    mexWarnMsgTxt( msg.c_str() );
    ok = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "BangBangFork_Mex('eval_F',...) unkown error\n" );
    ok = false;
  }
...
)
{
  ...
  try {
    ...
    mexWarnMsgTxt( fmt::format( "BangBangFork_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
    ok_value = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "BangBangFork_Mex('eval_JF',...) unkown error\n" );
    ok_value = false;
  }
...
)
{
  ...
  try {
    ...
    mexWarnMsgTxt( fmt::format( "BangBangFork_Mex('eval_JF2',...) error: {}", exc.what() ).c_str() );
    ok_value = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "BangBangFork_Mex('eval_JF2',...) unkown error\n" );
    ok_value = false;
  }
  int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
  UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
