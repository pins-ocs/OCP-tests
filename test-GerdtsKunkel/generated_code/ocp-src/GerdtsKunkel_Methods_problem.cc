...
namespace GerdtsKunkelDefine {
  ...
  void
  GerdtsKunkel::continuation_step_1( real_type s ) {
    real_type t1   = 1 - s;
    x1Limitation.update_epsilon(s * ModelPars[iM_epsilon_min] + ModelPars[iM_epsilon_max] * t1);
    x1Limitation.update_tolerance(s * ModelPars[iM_tolerance_min] + ModelPars[iM_tolerance_max] * t1);
  }
  ...
  GerdtsKunkel::H_eval(
    ...
    real_type t8   = t5 * t5;
    real_type result__ = MU__[0] * X__[iX_x2] + t5 * MU__[1] + t8 * MU__[2] / 2;
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer GerdtsKunkel::D2mayerD2xxp_numRows() const { return 6; }
  integer GerdtsKunkel::D2mayerD2xxp_numCols() const { return 6; }
  integer GerdtsKunkel::D2mayerD2xxp_nnz()     const { return 0; }

  void
  GerdtsKunkel::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  GerdtsKunkel::D2mayerD2xxp_sparse(
    NodeQX const & LEFT__,
    NodeQX const & RIGHT__,
    P_const_p_type P__,
    real_ptr       result__
  ...
  integer GerdtsKunkel::DsegmentLinkDxxp_numRows() const { return 0; }
  integer GerdtsKunkel::DsegmentLinkDxxp_numCols() const { return 0; }
  integer GerdtsKunkel::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  GerdtsKunkel::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  GerdtsKunkel::DsegmentLinkDxxp_sparse(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
  ...
  integer GerdtsKunkel::post_numEqns() const { return 1; }
...
// EOF: GerdtsKunkel_Methods_problem.cc
