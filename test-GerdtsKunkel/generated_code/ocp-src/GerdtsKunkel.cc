...
namespace GerdtsKunkelDefine {
  ...
  */
  void
  GerdtsKunkel::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "GerdtsKunkel::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  GerdtsKunkel::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "GerdtsKunkel::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in GerdtsKunkel::setup not all parameters are set!\n"
    );
  }

  void
  GerdtsKunkel::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  GerdtsKunkel::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "GerdtsKunkel::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("x1Limitation"),
      "in GerdtsKunkel::setup_classes(gc) missing key: ``x1Limitation''\n"
    );
    x1Limitation.setup( gc("x1Limitation") );
  ...
  */
  void
  GerdtsKunkel::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  GerdtsKunkel::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  GerdtsKunkel::setup_controls( GenericContainer const & gc_data ) {
  ...
  */
  void
  GerdtsKunkel::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "GerdtsKunkel::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in GerdtsKunkel::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  GerdtsKunkel::info_model( integer level ) const {
    ...
    m_console->message( x1Limitation.info(),level);
  ...
  */
  void
  GerdtsKunkel::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("GerdtsKunkel::setup, Debug");
  ...
  */
  void
  GerdtsKunkel::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  GerdtsKunkel::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
