...
main() {
  ...
  try {
    ...
    GerdtsKunkel     model("GerdtsKunkel",n_threads,&console);
    ...
    gc_data["DumpFile"] = "GerdtsKunkel_dump";
...
GerdtsKunkel_data.Mesh["segments"][0]["length"] = 1;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/GerdtsKunkel_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/GerdtsKunkel_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
