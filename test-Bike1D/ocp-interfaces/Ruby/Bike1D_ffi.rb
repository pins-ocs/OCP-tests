...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libBike1D.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libBike1D.#{DYLIB_EXT}"
end
...
module Bike1D
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libBike1D.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = Bike1D.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = Bike1D.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = Bike1D.OCP_solve( @id,
