...
namespace Bike1DDefine {
  ...
  void
  Bike1D::p_guess_eval( P_p_type P__ ) const {
  }

  void
  Bike1D::xlambda_guess_eval(
    integer        i_segment,
    Q_const_p_type Q__,
    P_const_p_type P__,
    X_p_type       X__,
  ...
  bool
  Bike1D::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  Bike1D::u_guess_eval(
    ...
    UGUESS__[ iU_mur ] = mufControl.solve(-t3, ModelPars[iM_mur_min], t8);
    UGUESS__[ iU_muf ] = mufControl.solve(-t3, ModelPars[iM_muf_min], 0);
    if ( m_debug )
