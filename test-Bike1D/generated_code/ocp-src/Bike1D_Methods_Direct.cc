...
namespace Bike1DDefine {
  ...
  integer Bike1D::fd_ode_numEqns() const { return 1; }
  ...
  integer Bike1D::Dfd_odeDxxup_nnz()     const { return 2; }
  ...
  Bike1D::Dfd_odeDxxup_sparse(
    ...
    V__[0] = __INV_DZETA*(XR__[0]-XL__[0]);
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t2   = 0.5e0 * V__[0];
    real_type t4   = __INV_DZETA * XM__[0];
    result__[ 0   ] = t2 - t4;
    result__[ 1   ] = t2 + t4;
    if ( m_debug )
