...
namespace Bike1DDefine {
  ...
  */
  ...
  BIKE1D_API_DLL
  bool
  Bike1D_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, Bike1D_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "Bike1D_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  Bike1D_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("Bike1D_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  Bike1D_write_ocp_solution( char const id[], char const fname[] ) {
    ...
    if ( it == problems.end() ) {
      pConsole->error("Bike1D_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
