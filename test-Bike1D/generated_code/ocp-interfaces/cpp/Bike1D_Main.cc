...
main() {
  ...
  try {
    Bike1D * m_model = new Bike1D("Bike1D",n_threads,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/Bike1D_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/Bike1D_Data.lua";
    #endif
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/Bike1D_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/Bike1D_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
