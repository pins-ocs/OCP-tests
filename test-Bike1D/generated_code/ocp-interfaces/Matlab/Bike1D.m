...
classdef Bike1D < handle
  ...
  methods
    ...
    function sol = pack_for_direct( self, X, U, Pars )
      ...
      %
      sol = Bike1D_Mex( 'pack_for_direct', self.objectHandle, X, U, Pars );
    end
    % ---------------------------------------------------------------------
    function [X, U, Pars] = unpack_for_direct( self, sol )
      ...
      %
      [X, U, Pars] = Bike1D_Mex( 'unpack_for_direct', self.objectHandle, sol );
    end
    % ---------------------------------------------------------------------
    function [Lambda,Omega] = estimate_multipliers( self, X, U, Pars, method )
      ...
      %
      [Lambda,Omega] = Bike1D_Mex( 'estimate_multipliers', self.objectHandle, X, U, Pars, method );
    end
    ...
    function U = init_U( self, Z, do_minimization )
      ...
      % Initialize `u`
      %
      U = Bike1D_Mex( 'init_U', self.objectHandle, Z, do_minimization );
    end
    % ---------------------------------------------------------------------
    ...
    function [ac] = eval_ac( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      ac = Bike1D_Mex( 'ac', self.objectHandle, ...
    ...
    function [h,c] = eval_hc( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      %
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      [h,c] = Bike1D_Mex( 'hc', self.objectHandle, ...
    ...
    function [Jh,Jc] = eval_DhcDxlxlop( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      %
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      [Jh,Jc] = Bike1D_Mex( 'DhcDxlxlop', self.objectHandle, ...
        iseg_L,  q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
      );
    end
    % ---------------------------------------------------------------------
    ...
    function u = eval_u( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        u = Bike1D_Mex( 'u', self.objectHandle, ...
          iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
        );
      else
    ...
    function DuDxlxlp = eval_DuDxlxlp( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        DuDxlxlp = Bike1D_Mex( 'DuDxlxlp', self.objectHandle, ...
          iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
        );
      else
    ...
    function rhs = eval_rhs_ode( self, iseg, q, x, u, pars )
      ...
      %
      rhs = Bike1D_Mex( 'rhs_ode', self.objectHandle, ...
        iseg, q, x, u, pars ...
      );
    end
    % ---------------------------------------------------------------------
    ...
    function A = eval_A( self, iseg, q, x, pars )
      ...
      % respect to `pars`.
      %
      A = Bike1D_Mex( 'A', self.objectHandle, iseg, q, x, pars );
    end
    % ---------------------------------------------------------------------
    ...
    function nu = eval_nu( self, iseg, q, x, V, pars )
      ...
      %
      nu = Bike1D_Mex( 'nu', self.objectHandle, iseg, q, x, V, pars );
    end
    % ---------------------------------------------------------------------
    function J = eval_DnuDxp( self, iseg, q, x, V, pars )
      ...
      % respect to `x` and `pars`.
      %
      J = Bike1D_Mex( 'DnuDxp', self.objectHandle, iseg, q, x, V, pars );
    end
    % ---------------------------------------------------------------------
    ...
    function J = eval_penalties( self, iseg, q, x, u, pars )
      ...
      %
      J = Bike1D_Mex( 'JP', self.objectHandle, iseg, q, x, u, pars );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    function J = eval_control_penalties( self, iseg, q, x, u, pars )
      ...
      %
      J = Bike1D_Mex( 'JU', self.objectHandle, iseg, q, x, u, pars );
    end
    % ---------------------------------------------------------------------
    ...
    function target = eval_q( self, i_segment, s )
      target = Bike1D_Mex( 'mesh_functions', self.objectHandle, i_segment, s );
    end
    % ---------------------------------------------------------------------
    ...
    function res = D2bcD2xxp_pattern( self )
      res = Bike1D_Mex('D2bcD2xxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = Drhs_odeDxpu_pattern( self )
      res = Bike1D_Mex('Drhs_odeDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DsegmentLinkDxp_pattern( self )
      res = Bike1D_Mex('DsegmentLinkDxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DHxpDxpu_pattern( self )
      res = Bike1D_Mex('DHxpDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DJPDxpu_pattern( self )
      res = Bike1D_Mex('DJPDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DLTDxpu_pattern( self )
      res = Bike1D_Mex('DLTDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DJUDxpu_pattern( self )
      res = Bike1D_Mex('DJUDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DnuDxp_pattern( self )
      res = Bike1D_Mex('DnuDxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DgDxlxlp_pattern( self )
      res = Bike1D_Mex('DgDxlxlp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DmDuu_pattern( self )
      res = Bike1D_Mex('DmDuu_pattern', self.objectHandle );
    end
