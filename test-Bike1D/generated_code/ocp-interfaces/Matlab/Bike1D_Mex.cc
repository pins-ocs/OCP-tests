...
"Bike1D_Mex('command', arguments ):\n"
"%===========================================================================%\n"
;

#define MODEL_NAME  "Bike1D"
#define MODEL_CLASS Bike1D
...
public:
  ...
  )
  {
    ...
    try {
      ...
      mexErrMsgTxt(
        fmt::format(
          "Bike1D_Mex('set_guess',...), error: {}",
          exc.what()
        ).c_str()
      );
    }
    catch (...) {
      mexErrMsgTxt( "Bike1D_Mex('set_guess',...), Unknown error\n" );
    }
    #undef CMD
  }
  ...
  )
  {
    ...
    try {
      ...
      string msg = fmt::format( "Bike1D_Mex('eval_F',...) error: {}", exc.what() );
      mexWarnMsgTxt( msg.c_str() );
      ok = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "Bike1D_Mex('eval_F',...) unkown error\n" );
      ok = false;
    }
  ...
  )
  {
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "Bike1D_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
      ok_value = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "Bike1D_Mex('eval_JF',...) unkown error\n" );
      ok_value = false;
    }
    int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
    UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
  ...
  do_Tmax_normalized(
    ...
    #define CMD MODEL_NAME "_Mex('Tmax_normalized', obj, xo__v ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
    ...
    for ( mwSize ii = 0; ii < N0*M0; ++ii )
      res[ii] = this->Tmax_normalized(arg0[ii]);
    #undef CMD
  }

  void
  do_Tmax_normalized_D(
    int nlhs, mxArray       *plhs[],
    int nrhs, mxArray const *prhs[]
  )
  {
    #define CMD MODEL_NAME "_Mex('Tmax_normalized_D', obj, xo__v ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
    real_const_ptr arg0 = Utils::mex_matrix_pointer( arg_in_2, N0, M0, CMD " xo__v" );
    ...
    real_ptr res = Utils::mex_create_matrix_value( arg_out_0, N0, M0 );
    for ( mwSize ii = 0; ii < N0*M0; ++ii )
      res[ii] = this->Tmax_normalized_D(arg0[ii]);
    #undef CMD
  }
  void
  do_Tmax_normalized_DD(
    int nlhs, mxArray       *plhs[],
    int nrhs, mxArray const *prhs[]
  ) {
    #define CMD MODEL_NAME "_Mex('Tmax_normalized_DD', obj, xo__v ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
    real_const_ptr arg0 = Utils::mex_matrix_pointer( arg_in_2, N0, M0, CMD " xo__v" );
    ...
    real_ptr res = Utils::mex_create_matrix_value( arg_out_0, N0, M0 );
    for ( mwSize ii = 0; ii < N0*M0; ++ii )
      res[ii] = this->Tmax_normalized_DD(arg0[ii]);
    #undef CMD
  }
...
do_Tmax_normalized(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('Tmax_normalized',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_Tmax_normalized( nlhs, plhs, nrhs, prhs );
}
...
do_Tmax_normalized_D(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('Tmax_normalized_D',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_Tmax_normalized_D( nlhs, plhs, nrhs, prhs );
}
...
do_Tmax_normalized_DD(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('Tmax_normalized_DD',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_Tmax_normalized_DD( nlhs, plhs, nrhs, prhs );
}
...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"Tmax_normalized",do_Tmax_normalized},
  {"Tmax_normalized_D",do_Tmax_normalized_D},
  {"Tmax_normalized_DD",do_Tmax_normalized_DD},
  {"new",do_new},
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt( fmt::format( "Bike1D_Mex, unknown command: {}", exc.what() ).c_str() );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt( fmt::format( "Bike1D_Mex, error: {}", exc.what() ).c_str() );
  }
  catch (...) {
