...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"Tmax_normalized",&ProblemStorage::do_Tmax_normalized},
  {"Tmax_normalized_D",&ProblemStorage::do_Tmax_normalized_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "Bike1D_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
