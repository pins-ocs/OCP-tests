...
DO_COMMAND_REMAP_TO_CLASS_METHOD(Tmax_normalized);
DO_COMMAND_REMAP_TO_CLASS_METHOD(Tmax_normalized_D);
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format( "Bike1D_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "Bike1D_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
