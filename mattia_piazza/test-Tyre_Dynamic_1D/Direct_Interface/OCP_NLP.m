...
%         res  = res + self.lagrange( nsg, nk, nk1, Z(idx), Z(idx1), Z(idu), Z(idp) ) ;
        res  = res + self.lagrange( nsg, nk, nk1, Z(idx), Z(idx1), Z(idu), Z(idp) ) * (nk1 - nk) ;
        idu  = idu + self.nu ;
        idx  = idx1 ;
      end
    ...
    function C = NLP_constraints( self, Z )
      ...
      idxc = 1:(self.nx+self.npc) ;

      C = zeros( (self.N-1)*self.nx + (self.N-1)*self.npc + self.nbc, 1 ) ;

      nsg = 0 ; % segment number
      for k=1:self.N-1
        nk     = self.nodes(k) ;
        nk1    = self.nodes(k+1) ;
        idxc1  = idxc + self.nx+self.npc;
        idx1   = idx + self.nx ;
        C(idxc) = self.ds( nsg, nk, nk1, Z(idx), Z(idx1), Z(idu), Z(idp) ) ;
        idx    = idx1 ;
        idxc   = idxc1;
      ...
      idx   = 1:self.nx ;
      id    = ((self.N-1)*self.nx + (self.N-1)*self.npc)+(1:self.nbc) ;
    ...
    function Jac = NLP_constraints_jacobian( self, Z )
      ...
      dim  = (self.N-1)*self.nx+(self.N-1)*self.npc+self.nbc ;
      ...
      imap = [ (self.N-1)*self.nx+(self.N-1)*self.npc + (1:self.nbc) ] ;
      jmap = [ idx, idx1, idp ] ;
    ...
    function Jac = NLP_constraints_jacobian_pattern( self )
      ...
      dimC = (self.N-1)*self.nx+(self.N-1)*self.npc+self.nbc ;
      dimZ = totx+totu+self.np ;
      Jac  = sparse( dimC, dimZ ) ;
      J    = ones(self.nx+self.npc,2*self.nx+self.nu+self.np) ;
      idp  = totx + totu + (1:self.np) ;
