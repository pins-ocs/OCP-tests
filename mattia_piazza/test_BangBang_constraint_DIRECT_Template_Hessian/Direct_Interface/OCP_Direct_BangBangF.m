...
%
  ...
  methods
    ...
    function hessL = lagrange_hessian( this, nseg, tL, tR, XL, XR, UC, P )
      ...
      qM     = this.pins.eval_q(nseg,tM);
      %
      hess_XMUXP = full(this.pins.eval_D2lagrangeD2xup(nseg,qM,XM,UC,P));
    ...
    %
    function CH = ds_hessian( this, nseg, tL, tR, XL, XR, UC, P, L )
      XM      = ( XR + XL ) / 2;
      tM      = (tR+tL)/2;
      qM      = this.pins.eval_q(nseg,tM);
      qL      = this.pins.eval_q(nseg,tL);
      ...
      CH_InC_XMUCP = full(this.pins.eval_D2cD2xup( nseg, qM, XM, UC, P, L_InC ));
      % Matrix can be decomposed in 
      % [ A | B ]
      % [ --|-- ]
      % [ B'| C ]
      ...
      % [ 1/2*B'| 1/2*B'|     C ]
      %
      A  = CH_InC_XMUCP(1:this.nx,1:this.nx);
      B  = CH_InC_XMUCP(1:this.nx,(this.nx+1):end);
      BT = CH_InC_XMUCP((this.nx+1):end,1:this.nx);
      C  = CH_InC_XMUCP((this.nx+1):end,(this.nx+1):end);
      %
      CH_InC =  [ 1/4*A , 1/4*A , 1/2*B ; ...
                  1/4*A , 1/4*A , 1/2*B ; ...
                  1/2*B', 1/2*B',     C ];
      %
      CH = CH_coll + CH_InC;
