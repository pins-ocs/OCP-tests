...
classdef OCP_Direct_AlpRider < OCP_NLP 
  ...
  methods
    ...
    function x0 = standard_Guess(self)
      ...
      y1_g      = y1_i + zeta*(y1_f-y1_i);
      y2_g      = y2_i + zeta*(y2_f-y2_i);
      y3_g      = y3_i + zeta*(y3_f-y3_i);
      y4_g      = y4_i + zeta*(y4_f-y4_i);
    ...
    function plot( self )
      ...
      plot( nodes, self.pins.q(nodes)   , 'Linewidth', 2 );
      legend('\Sigma ^2', 'Constaint') ;
      
      subplot( 4, 1, 3 );   
      plot( nodes, self.pins.C1(y1,y2,y3,y4,nodes), 'Linewidth', 2 );
      legend('Constraint  Value (C>=0)') ;
...
%       L = 1/(VM) ;
    ...
    function C = ds_inequality_Constraint( self, nseg, tL, tR, XL, XR, UC, ~ )
      ...
      XM = (XR+XL)./2 ;
      qM = self.eval_q(tM);
      C = self.pins.C1(XM(1),XM(2),XM(3),XM(4),tM);
    end
...
%       DRHSDUC = full(self.pins.eval_Drhs_odeDu(nseg, qM, XM, UC, [] ));
    ...
    function C = ds_inequality_Constraint_jacobian(self, nseg, tL, tR, XL, XR, UC)
      ...
      CJXM(1) = self.pins.C1_D_1(XM(1),XM(2),XM(3),XM(4),tM);
      CJXM(2) = self.pins.C1_D_2(XM(1),XM(2),XM(3),XM(4),tM);
      CJXM(3) = self.pins.C1_D_3(XM(1),XM(2),XM(3),XM(4),tM);
      CJXM(4) = self.pins.C1_D_4(XM(1),XM(2),XM(3),XM(4),tM);
