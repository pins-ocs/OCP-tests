...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libHyperSensitive.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libHyperSensitive.#{DYLIB_EXT}"
end
...
module HyperSensitive
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libHyperSensitive.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = HyperSensitive.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = HyperSensitive.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = HyperSensitive.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      HyperSensitive.OCP_write_ocp_solution( @id, fname )
    end
