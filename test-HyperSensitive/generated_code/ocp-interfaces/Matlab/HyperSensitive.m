...
classdef HyperSensitive < handle
  ...
  methods
    ...
    function res = get_dim_Q( self )
      HyperSensitive_Mex( 'dim_Q', self.objectHandle );
      res = HyperSensitive_Mex( 'dim_Q', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_X( self )
      HyperSensitive_Mex( 'dim_X', self.objectHandle );
      res = HyperSensitive_Mex( 'dim_X', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_Pars( self )
      HyperSensitive_Mex( 'dim_Pars', self.objectHandle );
      res = HyperSensitive_Mex( 'dim_Pars', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_BC( self )
      HyperSensitive_Mex( 'dim_BC', self.objectHandle );
      res = HyperSensitive_Mex( 'dim_BC', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_Post( self )
      HyperSensitive_Mex( 'dim_Post', self.objectHandle );
      res = HyperSensitive_Mex( 'dim_Post', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_Ipost( self )
      HyperSensitive_Mex( 'dim_Ipost', self.objectHandle );
      res = HyperSensitive_Mex( 'dim_Ipost', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_num_active_BC( self )
      HyperSensitive_Mex( 'num_active_BC', self.objectHandle );
      res = HyperSensitive_Mex( 'num_active_BC', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_num_nodes( self )
      HyperSensitive_Mex( 'num_nodes', self.objectHandle );
      res = HyperSensitive_Mex( 'num_nodes', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_num_equations( self )
      HyperSensitive_Mex( 'num_equations', self.objectHandle );
      res = HyperSensitive_Mex( 'num_equations', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_num_segments( self )
      HyperSensitive_Mex( 'num_segments', self.objectHandle );
      res = HyperSensitive_Mex( 'num_segments', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_full_bc( self )
      HyperSensitive_Mex( 'dim_full_bc', self.objectHandle );
      res = HyperSensitive_Mex( 'dim_full_bc', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_ineq( self )
      HyperSensitive_Mex( 'dim_ineq', self.objectHandle );
      res = HyperSensitive_Mex( 'dim_ineq', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    % ---------------------------------------------------------------------
    function info( self )
      HyperSensitive_Mex( 'info', self.objectHandle );
    end
    ...
    function set_info_level( self, infoLvl )
      ...
      %
      HyperSensitive_Mex( 'set_info_level', self.objectHandle, infoLvl );
    end
    %
    % DEPRECATED
    %
    ...
    function set_max_threads( self, nt )
      ...
      %
      HyperSensitive_Mex( 'set_max_threads', self.objectHandle, nt );
    end
    ...
    function remesh( self, new_mesh )
      ...
      %
      HyperSensitive_Mex( 'remesh', self.objectHandle, new_mesh );
    end
    ...
    function ok = solve( self, varargin )
      ...
      %
      ok = HyperSensitive_Mex( 'solve', self.objectHandle, varargin{:} );
    end
    %
    % ---------------------------------------------------------------------
    %
    function update_continuation( self, n, old_s, s )
      ...
      % the continuation.
      %
      HyperSensitive_Mex( 'update_continuation', self.objectHandle, n, old_s, s );
    end
    ...
    function sol = solution_by_group( self )
      ...
      sol = HyperSensitive_Mex( 'get_solution2', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function sol = solution_by_group_and_names( self )
      ...
      % res.data.control_penalties
      %
      sol = HyperSensitive_Mex( 'get_solution3', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function sol = pack_for_direct( self, X, U, Pars )
      ...
      %
      sol = HyperSensitive_Mex( 'pack_for_direct', self.objectHandle, X, U, Pars );
    end
    % ---------------------------------------------------------------------
    function [X, U, Pars] = unpack_for_direct( self, sol )
      ...
      %
      [X, U, Pars] = HyperSensitive_Mex( 'unpack_for_direct', self.objectHandle, sol );
    end
    % ---------------------------------------------------------------------
    function [Lambda,Omega] = estimate_multipliers( self, X, U, Pars, method )
      ...
      %
      [Lambda,Omega] = HyperSensitive_Mex( 'estimate_multipliers', self.objectHandle, X, U, Pars, method );
    end
    ...
    function res = y( self )
      ...
      %
      res = HyperSensitive_Mex( 'get_solution', self.objectHandle, 'y' );
    end
    ...
    function res = lambda1( self )
      ...
      %
      res = HyperSensitive_Mex( 'get_solution', self.objectHandle, 'lambda1__xo' );
    end
    ...
    function res = u( self )
      ...
      %
      res = HyperSensitive_Mex( 'get_solution', self.objectHandle, 'u' );
    end
    ...
    function U = init_U( self, Z, do_minimization )
      ...
      % Initialize `u`
      %
      U = HyperSensitive_Mex( 'init_U', self.objectHandle, Z, do_minimization );
    end
    % ---------------------------------------------------------------------
    function U = eval_U( self, Z, u_guess )
      ...
      %
      U = HyperSensitive_Mex( 'eval_U', self.objectHandle, Z, u_guess );
    end
    % ---------------------------------------------------------------------
    function [F,ok] = eval_F( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      [F,ok] = HyperSensitive_Mex( 'eval_F', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    function [JF,ok] = eval_JF( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      [JF,ok] = HyperSensitive_Mex( 'eval_JF', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    ...
    function [Z,U] = get_raw_solution( self )
      ...
      %
      [Z,U] = HyperSensitive_Mex( 'get_raw_solution', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function set_raw_solution( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      HyperSensitive_Mex( 'set_raw_solution', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    function ok = check_raw_solution( self, Z )
      ...
      % admissible regions.
      %
      ok = HyperSensitive_Mex( 'check_raw_solution', self.objectHandle, Z );
    end
    % ---------------------------------------------------------------------
    function check_jacobian( self, Z, U, epsi )
      ...
      %
      HyperSensitive_Mex( 'check_jacobian', self.objectHandle, Z, U, epsi );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    ...
    function [ac] = eval_ac( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      ac = HyperSensitive_Mex( 'ac', self.objectHandle, ...
    ...
    function [ DacDxlxlp, DacDu ] = ...
      ...
      %
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      [DacDxlxlp, DacDu] = HyperSensitive_Mex( ...
    ...
    function [h,c] = eval_hc( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      %
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      [h,c] = HyperSensitive_Mex( 'hc', self.objectHandle, ...
    ...
    function [Jh,Jc] = eval_DhcDxlxlop( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      %
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      [Jh,Jc] = HyperSensitive_Mex( 'DhcDxlxlop', self.objectHandle, ...
        iseg_L,  q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
      );
    end
    % ---------------------------------------------------------------------
    ...
    function u = eval_u( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        u = HyperSensitive_Mex( 'u', self.objectHandle, ...
          iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
        );
      else
    ...
    function DuDxlxlp = eval_DuDxlxlp( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        DuDxlxlp = HyperSensitive_Mex( 'DuDxlxlp', self.objectHandle, ...
          iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
        );
      else
    ...
    function rhs = eval_rhs_ode( self, iseg, q, x, u, pars )
      ...
      %
      rhs = HyperSensitive_Mex( 'rhs_ode', self.objectHandle, ...
        iseg, q, x, u, pars ...
      );
    end
    % ---------------------------------------------------------------------
    ...
    function A = eval_A( self, iseg, q, x, pars )
      ...
      % respect to `pars`.
      %
      A = HyperSensitive_Mex( 'A', self.objectHandle, iseg, q, x, pars );
    end
    % ---------------------------------------------------------------------
    ...
    function nu = eval_nu( self, iseg, q, x, V, pars )
      ...
      %
      nu = HyperSensitive_Mex( 'nu', self.objectHandle, iseg, q, x, V, pars );
    end
    % ---------------------------------------------------------------------
    function J = eval_DnuDxp( self, iseg, q, x, V, pars )
      ...
      % respect to `x` and `pars`.
      %
      J = HyperSensitive_Mex( 'DnuDxp', self.objectHandle, iseg, q, x, V, pars );
    end
    % ---------------------------------------------------------------------
    function bc = eval_bc( self, iseg_L, t_L, x_L, iseg_R, t_R, x_R, pars )
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      bc = HyperSensitive_Mex( ...
    ...
    function Dfd_odeDxxpu = eval_Dfd_odeDxxpu( self, iseg_L, t_L, x_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        Dfd_odeDxxpu = HyperSensitive_Mex( ...
    ...
    function D2fd_odeD2xxpu = eval_D2fd_odeD2xxpu( self, iseg_L, t_L, x_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        D2fd_odeD2xxpu = HyperSensitive_Mex( ...
    ...
    function D2mayerD2xxp = eval_D2mayerD2xxp( self, iseg_L, t_L, x_L, ...
                                                     ...
                                                     pars )
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      D2mayerD2xxp = HyperSensitive_Mex( ...
    ...
    function J = eval_penalties( self, iseg, q, x, u, pars )
      ...
      %
      J = HyperSensitive_Mex( 'JP', self.objectHandle, iseg, q, x, u, pars );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    ...
    function target = eval_q( self, i_segment, s )
      target = HyperSensitive_Mex( 'mesh_functions', self.objectHandle, i_segment, s );
    end
    % ---------------------------------------------------------------------
    function nodes = get_nodes( self )
      nodes = HyperSensitive_Mex( 'nodes', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function node_to_segment = get_node_to_segment( self )
      node_to_segment = HyperSensitive_Mex( 'node_to_segment', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = A_pattern( self )
      res = HyperSensitive_Mex('A_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DbcDxxp_pattern( self )
      res = HyperSensitive_Mex('DbcDxxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = D2bcD2xxp_pattern( self )
      res = HyperSensitive_Mex('D2bcD2xxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = Drhs_odeDxpu_pattern( self )
      res = HyperSensitive_Mex('Drhs_odeDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DsegmentLinkDxp_pattern( self )
      res = HyperSensitive_Mex('DsegmentLinkDxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DjumpDxlxlp_pattern( self )
      res = HyperSensitive_Mex('DjumpDxlxlp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DHxpDxpu_pattern( self )
      res = HyperSensitive_Mex('DHxpDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DJPDxpu_pattern( self )
      res = HyperSensitive_Mex('DJPDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DLTDxpu_pattern( self )
      res = HyperSensitive_Mex('DLTDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DJUDxpu_pattern( self )
      res = HyperSensitive_Mex('DJUDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = D2JPD2xpu_pattern( self )
      res = HyperSensitive_Mex('D2JPD2xpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = D2LTD2xpu_pattern( self )
      res = HyperSensitive_Mex('D2LTD2xpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = D2JUD2xpu_pattern( self )
      res = HyperSensitive_Mex('D2JUD2xpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DLTargsDxpu_pattern( self )
      res = HyperSensitive_Mex('DLTargsDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = D2LTargsD2xpu_pattern( self )
      res = HyperSensitive_Mex('D2LTargsD2xpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DnuDxp_pattern( self )
      res = HyperSensitive_Mex('DnuDxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DetaDxp_pattern( self )
      res = HyperSensitive_Mex('DetaDxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DgDxlxlp_pattern( self )
      res = HyperSensitive_Mex('DgDxlxlp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DgDu_pattern( self )
      res = HyperSensitive_Mex('DgDu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DmDuu_pattern( self )
      res = HyperSensitive_Mex('DmDuu_pattern', self.objectHandle );
    end
