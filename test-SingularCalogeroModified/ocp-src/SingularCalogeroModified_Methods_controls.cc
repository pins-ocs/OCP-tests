...
namespace SingularCalogeroModifiedDefine {
  ...
  SingularCalogeroModified::DgDu_sparse(
    ...
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    real_type t4   = Q__[0] * Q__[0];
    real_type t8   = pow(t4 * ModelPars[0] + X__[0] - 1, 2);
  ...
  bool
  SingularCalogeroModified::DuDxlp_full_numeric(
    NodeType2 const          & NODE__,
    P_const_pointer_type       P__,
    U_const_pointer_type       U__,
    MatrixWrapper<real_type> & DuDxlp
  ...
  integer
  SingularCalogeroModified::DjumpDxlp_numRows() const
  { return 4; }

  integer
  SingularCalogeroModified::DjumpDxlp_numCols() const
  { return 8; }

  integer
  SingularCalogeroModified::DjumpDxlp_nnz() const
  { return 8; }

  void
  SingularCalogeroModified::DjumpDxlp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
    iIndex[0 ] = 0   ; jIndex[0 ] = 0   ;
  ...
  void
  SingularCalogeroModified::DjumpDxlp_sparse(
    NodeType2 const    & LEFT__,
    NodeType2 const    & RIGHT__,
    P_const_pointer_type P__,
    real_type            result__[]
