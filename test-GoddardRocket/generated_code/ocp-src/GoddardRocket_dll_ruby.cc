/*-----------------------------------------------------------------------*\
 |  file: GoddardRocket_dll_ruby.cc                                      |
 |                                                                       |
...
#include "GoddardRocket_dll_ruby.hh"
#include <map>
...
namespace GoddardRocketDefine {
  ...
  static map< string, GoddardRocket_Problem * > problems;
  ...
  */
  ...
  GoddardRocket_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, GoddardRocket_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  GODDARDROCKET_API_DLL
  bool
  GoddardRocket_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, GoddardRocket_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "GoddardRocket_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  GoddardRocket_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("GoddardRocket_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  GoddardRocket_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, GoddardRocket_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("GoddardRocket_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
