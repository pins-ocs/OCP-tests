...
namespace GoddardRocketDefine {
  ...
  void
  GoddardRocket::p_guess_eval( P_p_type P__ ) const {
  ...
  bool
  GoddardRocket::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  GoddardRocket::u_guess_eval(
    ...
    UGUESS__[ iU_T ] = TControl.solve(-1.0 / t3 / t6 * (t3 * MU__[1] - t6 * MU__[2]) * P__[iP_TimeSize], 0, ModelPars[iM_Tmax]);
    if ( m_debug )
