...
namespace GoddardRocketDefine {
  ...
  GoddardRocket::H_eval(
    ...
    real_type t15  = gg(t9);
    real_type result__ = t4 * t2 * MU__[0] + (1.0 / X__[iX_m] * (t8 - t10) - t15) * t2 * MU__[1] - 1.0 / ModelPars[iM_c] * t8 * t2 * MU__[2];
    if ( m_debug ) {
  ...
  integer GoddardRocket::DsegmentLinkDxxp_numRows() const { return 0; }
  integer GoddardRocket::DsegmentLinkDxxp_numCols() const { return 0; }
  integer GoddardRocket::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  GoddardRocket::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  GoddardRocket::DsegmentLinkDxxp_sparse(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
