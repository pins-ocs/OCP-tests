...
public:
  ...
  do_eval_F(
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "GoddardRocket_Mex('eval_F',...) error: {}", exc.what() ).c_str() );
      ok = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "GoddardRocket_Mex('eval_F',...) unkown error\n" );
      ok = false;
    }
  ...
  do_eval_JF(
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "GoddardRocket_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
      ok_value = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "GoddardRocket_Mex('eval_JF',...) unkown error\n" );
      ok_value = false;
    }
    int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
    MEX_ASSERT( ok == 0, CMD "failed the call sparse(...)" );
  ...
  do_DD(
    ...
    CHECK_IN_OUT( 4, 1 );
    mwSize N0, M0;
    real_const_ptr arg0 = getMatrixPointer( arg_in_2, N0, M0, CMD " xo__h" );
    mwSize N1, M1;
    real_const_ptr arg1 = getMatrixPointer( arg_in_3, N1, M1, CMD " xo__v" );
  ...
  do_explog(
    ...
    mwSize N1, M1;
    real_const_ptr arg1 = getMatrixPointer( arg_in_3, N1, M1, CMD " xo__a" );
    MEX_CHECK_DIMS(N1,M1,N0,M0);
    mwSize N2, M2;
    real_const_ptr arg2 = getMatrixPointer( arg_in_4, N2, M2, CMD " xo__b" );
