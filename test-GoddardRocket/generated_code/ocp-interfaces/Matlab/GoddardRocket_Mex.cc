...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"DD",&ProblemStorage::do_DD},
  {"DD_D_1",&ProblemStorage::do_DD_D_1},
  ...
  {"T",&ProblemStorage::do_T},
  {"T_D",&ProblemStorage::do_T_D},
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "GoddardRocket_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
