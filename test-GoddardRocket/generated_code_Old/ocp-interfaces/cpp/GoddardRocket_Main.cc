...
main() {
  ...
  try {
    GoddardRocket * m_model = new GoddardRocket("GoddardRocket",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/GoddardRocket_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/GoddardRocket_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/GoddardRocket_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
