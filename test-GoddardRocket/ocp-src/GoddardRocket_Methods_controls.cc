...
namespace GoddardRocketDefine {
  ...
  GoddardRocket::g_eval(
    ...
    real_type t3   = ALIAS_TControl_D_1(UM__[0], 0, ModelPars[iM_Tmax]);
    real_type t5   = P__[iP_TimeSize];
    result__[ 0   ] = t3 + 1.0 / XM__[2] * t5 * LM__[1] - 1.0 / ModelPars[iM_c] * t5 * LM__[2];
    if ( m_debug )
  ...
  GoddardRocket::u_eval_analytic(
    ...
    real_type t6   = XM__[2];
    U__[ iU_T ] = TControl.solve(1.0 / t3 / t6 * (-t3 * LM__[1] + t6 * LM__[2]) * P__[iP_TimeSize], 0, ModelPars[iM_Tmax]);
    if ( m_debug )
