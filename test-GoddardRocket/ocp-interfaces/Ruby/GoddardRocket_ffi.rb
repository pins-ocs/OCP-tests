...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libGoddardRocket.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libGoddardRocket.#{DYLIB_EXT}"
end
...
module GoddardRocket
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libGoddardRocket.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = GoddardRocket.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = GoddardRocket.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = GoddardRocket.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      GoddardRocket.OCP_write_ocp_solution( @id, fname )
    end
