...
namespace vanDerPolDefine {
  ...
  vanDerPol::Hx_eval(
    ...
    result__[ 0   ] = 2 * t3 * t1 + 2 * t1 + (-2 * t8 * t1 - 1) * t7;
    real_type t17  = t1 * t1;
    result__[ 1   ] = 2 * t3 * t8 + 2 * t8 + L__[iL_lambda1__xo] + (-t17 + 1) * t7;
    if ( m_debug )
  ...
  vanDerPol::DHxDx_sparse(
    ...
    result__[ 0   ] = -2 * X__[iX_x2] * t3 + 2 * t2 + 2;
    result__[ 1   ] = -2 * X__[iX_x1] * t3;
  ...
  vanDerPol::Hu_eval(
    ...
    result__[ 0   ] = t8 * (t2 + t4 + ModelPars[iM_epsilon]) + L__[iL_lambda2__xo];
    if ( m_debug )
