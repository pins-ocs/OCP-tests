...
namespace vanDerPolDefine {
  ...
  vanDerPol::g_fun_eval(
    ...
    real_type result__ = t2 + t4 + t8 * (t2 + t4 + ModelPars[iM_epsilon]) + t3 * MU__[0] + (t3 * (-t2 + 1) - t1 + t7) * MU__[1];
    if ( m_debug ) {
  ...
  vanDerPol::g_eval(
    ...
    result__[ 0   ] = t8 * (t2 + t4 + ModelPars[iM_epsilon]) + MU__[1];
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 1, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer vanDerPol::DgDxpm_numRows() const { return 1; }
  integer vanDerPol::DgDxpm_numCols() const { return 4; }
  integer vanDerPol::DgDxpm_nnz()     const { return 3; }

  void
  vanDerPol::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  vanDerPol::u_eval_analytic(
    ...
    U__[ iU_u ] = uControl.solve(-1.0 / (t3 + t5 + ModelPars[iM_epsilon]) * MU__[1], -1, 1);
    if ( m_debug )
