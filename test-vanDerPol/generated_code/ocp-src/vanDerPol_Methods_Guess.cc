...
namespace vanDerPolDefine {
  ...
  void
  vanDerPol::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  vanDerPol::xlambda_guess_eval(
    integer              i_segment,
  ...
  bool
  vanDerPol::p_check( P_const_pointer_type P__ ) const {
    return true;
  }
  ...
  integer vanDerPol::u_guess_numEqns() const { return 1; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  vanDerPol::u_guess_eval(
    NodeType2 const    & LEFT__,
    NodeType2 const    & RIGHT__,
    P_const_pointer_type P__,
    ...
    UGUESS__[ iU_u ] = uControl.solve(-2 / (t3 + t5 + t7 + t9 + 2 * ModelPars[iM_epsilon]) * L__[iL_lambda2__xo], -1, 1);
    if ( m_debug )
  ...
  bool
  vanDerPol::u_check_if_admissible(
    NodeType2 const    & NODE__,
