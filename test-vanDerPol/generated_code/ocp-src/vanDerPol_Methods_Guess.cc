...
namespace vanDerPolDefine {
  ...
  void
  vanDerPol::p_guess_eval( P_p_type P__ ) const {
  }
  ...
  bool
  vanDerPol::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  vanDerPol::penalties_check_node(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__
  ) const {
  ...
  vanDerPol::u_guess_eval(
    ...
    UGUESS__[ iU_u ] = uControl.solve(-1.0 / (t3 + t5 + ModelPars[iM_epsilon]) * MU__[1], -1, 1);
    if ( m_debug )
