...
main() {
  ...
  try {
    ...
    vanDerPol        model("vanDerPol",n_threads,&console);
    ...
    gc_data["DumpFile"] = "vanDerPol_dump";
...
vanDerPol_data.Mesh["s0"] = 0;
vanDerPol_data.Mesh["segments"][0]["n"] = 400;
vanDerPol_data.Mesh["segments"][0]["length"] = 4;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/vanDerPol_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/vanDerPol_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
