...
main() {
  ...
  try {
    ...
    SingularLuus02   model("SingularLuus02",&TP,&console);
    ...
    gc_data["DumpFile"] = "SingularLuus02_dump";
    ...
    // Model Parameters
    data_Parameters["epsi_x"] = epsi_x;

    // Guess Parameters

    // Boundary Conditions
    data_Parameters["x_i"] = 0;
    data_Parameters["y_i"] = 1;
    ...
    data_uControl["type"]      = ;
    data_uControl["epsilon"]   = 0.001;
    data_uControl["tolerance"] = 0.001;
...
SingularLuus02_data.Mesh["s0"] = 0;
SingularLuus02_data.Mesh["segments"][0]["n"] = 400;
SingularLuus02_data.Mesh["segments"][0]["length"] = Tf;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/SingularLuus02_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/SingularLuus02_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
