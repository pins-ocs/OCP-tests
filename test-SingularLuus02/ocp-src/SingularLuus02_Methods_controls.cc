...
namespace SingularLuus02Define {
  ...
  SingularLuus02::g_eval(
    ...
    real_type t5   = ModelPars[0] * ModelPars[0];
    real_type t8   = ALIAS_uControl_D_1(U__[0], -1, 1);
    result__[ 0   ] = L__[1] + t8 * (t3 + t5);
    if ( m_debug )
  ...
  SingularLuus02::DgDu_sparse(
    ...
    real_type t4   = ModelPars[0] * ModelPars[0];
  ...
  SingularLuus02::u_eval_analytic(
    ...
    real_type t3   = X__[0] * X__[0];
    real_type t5   = ModelPars[0] * ModelPars[0];
    U__[ iU_u ] = uControl.solve(-1.0 / (t3 + t5) * L__[1], -1, 1);
  }
  ...
  SingularLuus02::DuDxlp_full_analytic(
    ...
    real_type const * X__ = NODE__.x;
    real_type const * L__ = NODE__.lambda;
    DuDxlp(0, 0) = 2 * uControl.solve_rhs(-L__[1] / (X__[0] * X__[0] + ModelPars[0] * ModelPars[0]), -1, 1) * L__[1] * pow(X__[0] * X__[0] + ModelPars[0] * ModelPars[0], -2) * X__[0];
    DuDxlp(0, 1) = 0;
    DuDxlp(0, 2) = 0;
    DuDxlp(0, 3) = -uControl.solve_rhs(-L__[1] / (X__[0] * X__[0] + ModelPars[0] * ModelPars[0]), -1, 1) / (X__[0] * X__[0] + ModelPars[0] * ModelPars[0]);
  }
  ...
  bool
  SingularLuus02::DuDxlp_full_numeric(
    NodeType2 const          & NODE__,
    P_const_pointer_type       P__,
    U_const_pointer_type       U__,
    MatrixWrapper<real_type> & DuDxlp
    ...
    integer info = alglin::gesv( 1, 4, DgDu.data(), 1, ipiv, DuDxlp.data(), 1 );
    return info == 0;
  }
  ...
  integer
  SingularLuus02::segmentLink_numEqns() const
  { return 0; }

  void
  SingularLuus02::segmentLink_eval(
    NodeType const     & L,
    NodeType const     & R,
  ...
  integer
  SingularLuus02::DsegmentLinkDxp_numRows() const
  { return 0; }

  integer
  SingularLuus02::DsegmentLinkDxp_numCols() const
  { return 0; }

  integer
  SingularLuus02::DsegmentLinkDxp_nnz() const
  { return 0; }

  void
  SingularLuus02::DsegmentLinkDxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  SingularLuus02::DsegmentLinkDxp_sparse(
    NodeType const     & L,
    NodeType const     & R,
    P_const_pointer_type p,
  ...
  integer
  SingularLuus02::jump_numEqns() const
  { return 4; }

  void
  SingularLuus02::jump_eval(
    NodeType2 const    & LEFT__,
    NodeType2 const    & RIGHT__,
    P_const_pointer_type P__,
    real_type            result__[]
