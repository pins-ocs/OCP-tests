...
main() {
  ...
  try {
    ...
    stirred_tank     model("stirred_tank",&console,&TP);
    ...
    gc_data["DumpFile"] = "stirred_tank_dump";
    ...
    data_BoundaryConditions["initial_x5"] = SET;
    data_BoundaryConditions["initial_x6"] = SET;
    data_BoundaryConditions["initial_x7"] = SET;
    data_BoundaryConditions["initial_x8"] = SET;
    ...
    data_Parameters["epsi_min"] = 0.001;
    ...
    data_u1Control["epsilon"]   = epsi_max;
    data_u1Control["tolerance"] = tol_max;


    GenericContainer & data_u2Control = data_Controls["u2Control"];
    data_u2Control["type"]      = "COS_LOGARITHMIC";
    data_u2Control["epsilon"]   = epsi_max;
    data_u2Control["tolerance"] = tol_max;


    GenericContainer & data_u3Control = data_Controls["u3Control"];
    data_u3Control["type"]      = "COS_LOGARITHMIC";
    data_u3Control["epsilon"]   = epsi_max;
    data_u3Control["tolerance"] = tol_max;


    GenericContainer & data_u4Control = data_Controls["u4Control"];
    data_u4Control["type"]      = "COS_LOGARITHMIC";
    data_u4Control["epsilon"]   = epsi_max;
    data_u4Control["tolerance"] = tol_max;
...
stirred_tank_data.Mesh["segments"][0]["length"] = tf;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/stirred_tank_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/stirred_tank_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
