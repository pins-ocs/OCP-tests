...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libstirred_tank.#{DYLIB_EXT}"
...
module Stirred_tank
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libstirred_tank.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = Stirred_tank.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = Stirred_tank.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = Stirred_tank.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      Stirred_tank.OCP_write_ocp_solution( @id, fname )
    end
