...
)
{
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format(
        "stirred_tank_Mex('set_guess',...), error: {}",
        exc.what()
      ).c_str()
    );
  }
  catch (...) {
    mexErrMsgTxt( "stirred_tank_Mex('set_guess',...), Unknown error\n" );
  }
  #undef CMD
}
...
)
{
  ...
  try {
    ...
    string msg = fmt::format( "stirred_tank_Mex('eval_F',...) error: {}", exc.what() );
    mexWarnMsgTxt( msg.c_str() );
    ok = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "stirred_tank_Mex('eval_F',...) unkown error\n" );
    ok = false;
  }
...
)
{
  ...
  try {
    ...
    mexWarnMsgTxt( fmt::format( "stirred_tank_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
    ok_value = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "stirred_tank_Mex('eval_JF',...) unkown error\n" );
    ok_value = false;
  }
...
)
{
  ...
  //mwSize nL;
  //L_const_p_type lambda(Utils::mex_vector_pointer( arg_in_10, nL, CMD " argument lambda" ));
  //UTILS_MEX_ASSERT(
  //  nL == this->dim_X(),
