...
#include "stirred_tank_dll_ruby.hh"
#include <map>
...
namespace stirred_tankDefine {
  ...
  static map< string, stirred_tank_Problem * > problems;
  ...
  */
  ...
  bool
  stirred_tank_ocp_setup( char const id[], GenericContainer & gc_data ) {
    if ( pConsole == nullptr ) pConsole = new Console(&std::cout,4);
    if ( pTP      == nullptr ) pTP      = new ThreadPool1(std::thread::hardware_concurrency());

    map< string, stirred_tank_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  STIRRED_TANK_API_DLL
  bool
  stirred_tank_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, stirred_tank_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "stirred_tank_ocp_guess" ) );
      return true;
    }
  }
  ...
  STIRRED_TANK_API_DLL
  bool
  stirred_tank_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("stirred_tank_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  STIRRED_TANK_API_DLL
  void
  stirred_tank_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, stirred_tank_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("stirred_tank_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
