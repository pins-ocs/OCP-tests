...
namespace stirred_tankDefine {
  ...
  */
  void
  stirred_tank::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "stirred_tank::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  stirred_tank::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "stirred_tank::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in stirred_tank::setup not all parameters are set!\n"
    );
  }

  void
  stirred_tank::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  stirred_tank::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  stirred_tank::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  stirred_tank::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  stirred_tank::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "stirred_tank::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  stirred_tank::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "stirred_tank::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in stirred_tank::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  stirred_tank::info_classes() const {
    int msg_level = 3;

    m_console->message("\nControls\n",msg_level);
    m_console->message( u1Control.info(),msg_level);
    m_console->message( u2Control.info(),msg_level);
    m_console->message( u3Control.info(),msg_level);
    m_console->message( u4Control.info(),msg_level);
  ...
  */
  void
  stirred_tank::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("stirred_tank::setup, Debug");
  ...
  */
  void
  stirred_tank::get_names( GenericContainer & out ) const {
  ...
  void
  stirred_tank::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
