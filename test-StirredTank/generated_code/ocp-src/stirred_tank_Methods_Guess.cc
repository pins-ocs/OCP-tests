...
namespace stirred_tankDefine {
  ...
  void
  stirred_tank::p_guess_eval( P_p_type P__ ) const {
  }

  void
  stirred_tank::xlambda_guess_eval(
    integer        i_segment,
    Q_const_p_type Q__,
    P_const_p_type P__,
    X_p_type       X__,
    ...
    if ( m_debug ) {
      Mechatronix::check( X__.pointer(), "xlambda_guess_eval (x part)", 8 );
      Mechatronix::check( L__.pointer(), "xlambda_guess_eval (lambda part)", 8 );
    }
  }
  ...
  bool
  stirred_tank::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  stirred_tank::xlambda_check_node(
    integer         ipos,
  ...
  bool
  stirred_tank::penalties_check_node(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__
  ) const {
  ...
  integer stirred_tank::u_guess_numEqns() const { return 4; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  stirred_tank::u_guess_eval(
    NodeQXL const & NODE__,
    P_const_p_type  P__,
  ...
  bool
  stirred_tank::u_check_if_admissible(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__
    ...
    ok = ok && u1Control.check_range(U__[iU_u1], 0, 20);
    ok = ok && u2Control.check_range(U__[iU_u2], 0, 6);
    ok = ok && u3Control.check_range(U__[iU_u3], 0, 4);
    ok = ok && u4Control.check_range(U__[iU_u4], 0, 20);
    return ok;
  }
