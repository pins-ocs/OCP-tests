...
namespace BangBangFtminDefine {
  ...
  BangBangFtmin::g_fun_eval(
    ...
    real_type result__ = t2 * t1 * MU__[1] + X__[iX_v] * t1 * MU__[0] + t3 * t1;
    if ( m_debug ) {
  ...
  BangBangFtmin::g_eval(
    ...
    result__[ 0   ] = t3 * t1 + t1 * MU__[1];
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 1, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer BangBangFtmin::DgDxpm_numRows() const { return 1; }
  integer BangBangFtmin::DgDxpm_numCols() const { return 5; }
  integer BangBangFtmin::DgDxpm_nnz()     const { return 2; }

  void
  BangBangFtmin::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
