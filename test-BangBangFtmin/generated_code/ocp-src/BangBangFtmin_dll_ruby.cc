...
#include "BangBangFtmin_dll_ruby.hh"
#include <map>
...
namespace BangBangFtminDefine {
  ...
  static map< string, BangBangFtmin_Problem * > problems;
  ...
  */
  ...
  BangBangFtmin_ocp_guess( char const id[], GenericContainer & gc_data ) {
    ...
    if ( it == problems.end() ) {
      ...
      it->second->guess( gc_data("Guess", "BangBangFtmin_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  BangBangFtmin_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("BangBangFtmin_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
