...
namespace BangBangFworkDefine {
  ...
  void
  BangBangFwork::p_guess_eval( P_p_type P__ ) const {
  }
  ...
  bool
  BangBangFwork::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  BangBangFwork::penalties_check_node(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__
  ) const {
  ...
  BangBangFwork::u_guess_eval(
    ...
    UGUESS__[ iU_F ] = Fcontrol.solve(-MU__[1], -t3, t3);
    if ( m_debug )
