...
namespace BangBangFworkDefine {
  ...
  BangBangFwork::g_fun_eval(
    ...
    real_type result__ = t1 * MU__[1] + MU__[0] * X__[iX_v] + t4;
    if ( m_debug ) {
  ...
  BangBangFwork::g_eval(
    ...
    result__[ 0   ] = t4 + MU__[1];
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 1, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer BangBangFwork::DgDxpm_numRows() const { return 1; }
  integer BangBangFwork::DgDxpm_numCols() const { return 4; }
  integer BangBangFwork::DgDxpm_nnz()     const { return 1; }

  void
  BangBangFwork::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  BangBangFwork::u_eval_analytic(
    ...
    MeshStd::SegmentClass const & segment = m_pMesh->get_segment_by_index(i_segment);
    real_type t3   = fMax(Q__[iQ_zeta]);
    U__[ iU_F ] = Fcontrol.solve(-MU__[1], -t3, t3);
    if ( m_debug )
