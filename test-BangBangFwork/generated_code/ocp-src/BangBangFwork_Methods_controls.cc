...
namespace BangBangFworkDefine {
  ...
  BangBangFwork::g_fun_eval(
    ...
    real_type t1   = LM__[0];
    real_type t5   = UM__[0];
    real_type t9   = fMax(QM__[0]);
    real_type t10  = Fcontrol(t5, -t9, t9);
    real_type result__ = XL__[iX_v] * t1 + XR__[iX_v] * t1 + 2 * t5 * LM__[1] + 2 * t10;
    if ( m_debug ) {
  ...
  BangBangFwork::g_eval(
    ...
    LM__[1] = (LL__[1]+LR__[1])/2;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t4   = fMax(QM__[0]);
    real_type t5   = ALIAS_Fcontrol_D_1(UM__[0], -t4, t4);
    result__[ 0   ] = 2 * LM__[1] + 2 * t5;
    if ( m_debug )
  ...
  BangBangFwork::DgDu_sparse(
    ...
    real_type t4   = ALIAS_Fcontrol_D_1_1(UM__[0], -t3, t3);
    result__[ 0   ] = 2 * t4;
    if ( m_debug )
