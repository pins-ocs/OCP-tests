...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"fMax",&ProblemStorage::do_fMax},
  {"fMax_D",&ProblemStorage::do_fMax_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "BangBangFwork_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
    ...
    mexErrMsgTxt(
      fmt::format( "BangBangFwork_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "BangBangFwork_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
