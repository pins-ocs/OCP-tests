...
using namespace BangBangFworkLoad;
...
"BangBangFwork_Mex('command', arguments ):\n"
"%===========================================================================%\n"
;

#define MODEL_NAME  "BangBangFwork"
#define MODEL_CLASS BangBangFwork
...
public:
  ...
  )
  {
    ...
    try {
      ...
      mexErrMsgTxt(
        fmt::format(
          "BangBangFwork_Mex('set_guess',...), error: {}",
          exc.what()
        ).c_str()
      );
    }
    catch (...) {
      mexErrMsgTxt( "BangBangFwork_Mex('set_guess',...), Unknown error\n" );
    }
    #undef CMD
  }
  ...
  )
  {
    ...
    try {
      ...
      string msg = fmt::format( "BangBangFwork_Mex('eval_F',...) error: {}", exc.what() );
      mexWarnMsgTxt( msg.c_str() );
      ok = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "BangBangFwork_Mex('eval_F',...) unkown error\n" );
      ok = false;
    }
  ...
  )
  {
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "BangBangFwork_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
      ok_value = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "BangBangFwork_Mex('eval_JF',...) unkown error\n" );
      ok_value = false;
    }
    int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
    UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
...
do_fMax(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('fMax',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_fMax( nlhs, plhs, nrhs, prhs );
}
...
do_fMax_D(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('fMax_D',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_fMax_D( nlhs, plhs, nrhs, prhs );
}
...
do_fMax_DD(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('fMax_DD',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_fMax_DD( nlhs, plhs, nrhs, prhs );
}
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt( fmt::format( "BangBangFwork_Mex, unknown command: {}", exc.what() ).c_str() );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt( fmt::format( "BangBangFwork_Mex, error: {}", exc.what() ).c_str() );
  }
  catch (...) {
