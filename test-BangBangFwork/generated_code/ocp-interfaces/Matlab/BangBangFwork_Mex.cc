...
public:
  ...
  do_eval_F(
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "BangBangFwork_Mex('eval_F',...) error: {}", exc.what() ).c_str() );
      ok = false;
    } catch ( ... ) {
  ...
  do_fMax(
    ...
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
    real_type const * arg0 = getMatrixPointer( arg_in_2, N0, M0, CMD " xo__zeta" );
...
do_fMax(
  ...
  convertMat2Ptr<ProblemStorage>(arg_in_1)->do_fMax( nlhs, plhs, nrhs, prhs );
}
...
do_fMax_D(
  ...
  convertMat2Ptr<ProblemStorage>(arg_in_1)->do_fMax_D( nlhs, plhs, nrhs, prhs );
}
...
do_fMax_DD(
  ...
  convertMat2Ptr<ProblemStorage>(arg_in_1)->do_fMax_DD( nlhs, plhs, nrhs, prhs );
}
