...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libBangBangFwork.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libBangBangFwork.#{DYLIB_EXT}"
end
...
module BangBangFwork
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libBangBangFwork.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = BangBangFwork.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = BangBangFwork.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = BangBangFwork.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      BangBangFwork.OCP_write_ocp_solution( @id, fname )
    end
