...
namespace SingularArcDefine {
  ...
  */
  void
  SingularArc::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "SingularArc::setup_parameters: Missing key `Parameters` in data\n"
    );
  ...
  void
  SingularArc::setup_parameters( real_type const Pars[] ) {
  ...
  */
  void
  SingularArc::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "SingularArc::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("tfbound"),
      "in SingularArc::setup_classes(gc) missing key: ``tfbound''\n"
    );
  ...
  */
  void
  SingularArc::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  SingularArc::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  SingularArc::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "SingularArc::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  SingularArc::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "SingularArc::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in SingularArc::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  SingularArc::info_classes() const {
    int msg_level = 3;
    ostringstream mstr;
