...
namespace SingularArcDefine {
  ...
  SingularArc::u_eval_analytic(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    U__[ iU_u ] = uControl.solve(-P__[iP_T] * (LL__[iL_lambda1__xo] + LR__[iL_lambda1__xo]) / 2, -2, 2);
    if ( m_debug )
  ...
  SingularArc::DuDxlxlp_full_analytic(
    ...
    real_type t4   = LL__[iL_lambda1__xo] + LR__[iL_lambda1__xo];
    real_type t7   = uControl.solve_rhs(-t4 * t1 / 2, -2, 2);
  ...
  SingularArc::DmDu_eval(
    ...
    real_type t2   = t1 * t1;
    real_type t3   = U__[iU_u];
    real_type t9   = ALIAS_uControl_D_1(t3, -2, 2);
    result__[ 0   ] = -2 * V__[0] * t1 + 2 * t3 * t2 + t9;
    if ( m_debug )
