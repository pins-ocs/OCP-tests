...
namespace SingularArcDefine {
  ...
  integer
  SingularArc::DgDxlxlp_numRows() const
  { return 1; }

  integer
  SingularArc::DgDxlxlp_numCols() const
  { return 13; }

  integer
  SingularArc::DgDxlxlp_nnz() const
  { return 3; }

  void
  SingularArc::DgDxlxlp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
  ...
  void
  SingularArc::DuDxlxlp_full_analytic(
    NodeType2 const &          LEFT__,
    NodeType2 const &          RIGHT__,
    P_const_pointer_type       P__,
    U_const_pointer_type       UM__,
  ...
  SingularArc::m_eval(
    ...
    real_type t14  = pow(-t1 * t11 + V__[1], 2);
    real_type t16  = sin(t10);
    real_type t19  = pow(-t1 * t16 + V__[2], 2);
