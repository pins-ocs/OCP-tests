...
namespace SingularArcDefine {
  ...
  */
  void
  SingularArc::updateContinuation( integer phase, real_type s ) {
    UTILS_ASSERT(
      s >= 0 && s <= 1,
      "SingularArc::updateContinuation( phase number = {}, s = {}) "
  ...
  */
  void
  SingularArc::setupParameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "SingularArc::setupParameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in SingularArc::setup not all parameters are set!\n"
    );
  }

  void
  SingularArc::setupParameters( real_type const Pars[] ) {
  ...
  */
  void
  SingularArc::setupClasses( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "SingularArc::setupClasses: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("tfbound"),
      "in SingularArc::setupClasses(gc) missing key: ``tfbound''\n"
    );
  ...
  */
  void
  SingularArc::setupUserClasses( GenericContainer const & gc ) {
  }
  ...
  */
  void
  SingularArc::setupUserMappedFunctions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  SingularArc::setupControls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "SingularArc::setupClasses: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  SingularArc::setupPointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "SingularArc::setupPointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in SingularArc::setupPointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  SingularArc::setup( GenericContainer const & gc ) {

    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("SingularArc::setup, Debug");
  ...
  */
  void
  SingularArc::get_names( GenericContainer & out ) const {
  ...
  */
  void
  SingularArc::diagnostic( GenericContainer const & gc ) {
  ...
  void
  SingularArc::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
