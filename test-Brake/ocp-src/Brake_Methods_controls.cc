...
namespace BrakeDefine {
  ...
  Brake::g_eval(
    ...
    real_type t5   = ALIAS_aControl_D_1(U__[0], -1, 1);
    result__[ 0   ] = L__[1] * P__[0] + t5;
  ...
  Brake::DgDu_sparse(
    ...
    real_type const * L__ = NODE__.lambda;
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    result__[ 0   ] = ALIAS_aControl_D_1_1(U__[0], -1, 1);
    #ifdef MECHATRONIX_DEBUG
  ...
  Brake::u_eval_analytic(
    ...
    real_type const * L__ = NODE__.lambda;
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    U__[ iU_a ] = aControl.solve(-L__[1] * P__[0], -1, 1);
  }
  ...
  Brake::DuDxlp_full_analytic(
    ...
    DuDxlp(0, 1) = 0;
    DuDxlp(0, 2) = 0;
    DuDxlp(0, 3) = -aControl.solve_rhs(-L__[1] * P__[0], -1, 1) * P__[0];
    DuDxlp(0, 4) = -aControl.solve_rhs(-L__[1] * P__[0], -1, 1) * L__[1];
  }
  ...
  integer
  Brake::DjumpDxlp_numRows() const
  { return 4; }

  integer
  Brake::DjumpDxlp_numCols() const
  { return 9; }

  integer
  Brake::DjumpDxlp_nnz() const
  { return 8; }

  void
  Brake::DjumpDxlp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
    iIndex[ 0  ] = 0   ; jIndex[ 0  ] = 0   ;
