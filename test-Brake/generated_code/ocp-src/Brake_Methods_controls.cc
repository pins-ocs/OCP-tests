...
namespace BrakeDefine {
  ...
  Brake::g_fun_eval(
    ...
    real_type t13  = Tpositive(-t2);
    real_type result__ = 2 * t8 * t2 * LM__[1] + XL__[iX_v] * t3 + XR__[iX_v] * t3 + 2 * t11 + 2 * t13;
    if ( m_debug ) {
  ...
  Brake::g_eval(
    ...
    LM__[1] = (LL__[1]+LR__[1])/2;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t5   = ALIAS_aControl_D_1(UM__[0], -1, 1);
    result__[ 0   ] = 2 * LM__[1] * P__[iP_T] + 2 * t5;
    if ( m_debug )
  ...
  Brake::DgDu_sparse(
    ...
    LM__[1] = (LL__[1]+LR__[1])/2;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t2   = ALIAS_aControl_D_1_1(UM__[0], -1, 1);
    result__[ 0   ] = 2 * t2;
    if ( m_debug )
  ...
  Brake::u_eval_analytic(
    ...
    integer i_segment = LEFT__.i_segment;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    U__[ iU_a ] = aControl.solve(-LM__[1] * P__[iP_T], -1, 1);
    if ( m_debug )
  ...
  Brake::DmDu_eval(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t1   = U__[iU_a];
    real_type t2   = ALIAS_aControl_D_1(t1, -1, 1);
    real_type t3   = P__[iP_T];
    result__[ 0   ] = t2 - 2 * t3 * (-t1 * t3 + V__[1]);
    if ( m_debug )
