...
namespace BrakeDefine {
  ...
  Brake::mayer_target(
    ...
    real_type t4   = log(P__[iP_T]);
    real_type result__ = -t4 * ModelPars[iM_epsilon2] + XR__[iX_x];
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Brake::D2mayerD2xxp_numRows() const { return 5; }
  integer Brake::D2mayerD2xxp_numCols() const { return 5; }
  integer Brake::D2mayerD2xxp_nnz()     const { return 1; }

  void
  Brake::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    iIndex[0 ] = 4   ; jIndex[0 ] = 4   ;
  }


  void
  Brake::D2mayerD2xxp_sparse(
    NodeQX const & LEFT__,
    NodeQX const & RIGHT__,
    P_const_p_type P__,
    real_ptr       result__
    ...
    result__[ 0   ] = 1.0 / t3 * ModelPars[iM_epsilon2];
    if ( m_debug )
  ...
  integer Brake::DlagrangeDxpu_numEqns() const { return 4; }

  void
  Brake::DlagrangeDxpu_eval(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Brake::D2lagrangeD2xpu_numRows() const { return 4; }
  integer Brake::D2lagrangeD2xpu_numCols() const { return 4; }
  integer Brake::D2lagrangeD2xpu_nnz()     const { return 3; }

  void
  Brake::D2lagrangeD2xpu_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  integer Brake::DsegmentLinkDxxp_numRows() const { return 0; }
  integer Brake::DsegmentLinkDxxp_numCols() const { return 0; }
  integer Brake::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  Brake::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
