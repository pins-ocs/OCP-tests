...
#define MODEL_NAME  "Brake"
#define MODEL_CLASS Brake
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt( fmt::format( "Brake_Mex, unknown command: {}", exc.what() ).c_str() );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt( fmt::format( "Brake_Mex, error: {}", exc.what() ).c_str() );
  }
  catch (...) {
