...
main() {
  ...
  try {
    ...
    BangBangF        model("BangBangF",n_threads,&console);
    ...
    gc_data["DumpFile"] = "BangBangF_dump";
    ...
    GenericContainer & data_C1_constr = data_Constraints["C1_constr"];
    data_C1_constr["subType"]   = "PENALTY_REGULAR";
    data_C1_constr["epsilon"]   = 0.001;
    data_C1_constr["tolerance"] = 0.025;
    data_C1_constr["active"]    = true;
    // Constraint1D: none defined
    // Constraint2D: none defined

    // User defined classes initialization
    // User defined classes: M E S H
BangBangF_data.Mesh["s0"] = 0;
BangBangF_data.Mesh["segments"][0]["length"] = 1;
BangBangF_data.Mesh["segments"][0]["n"] = 100;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/BangBangF_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/BangBangF_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
