...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"C1",&ProblemStorage::do_C1},
  {"C1_D",&ProblemStorage::do_C1_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "BangBangF_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
    ...
    mexErrMsgTxt(
      fmt::format( "BangBangF_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "BangBangF_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
