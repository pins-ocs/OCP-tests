...
namespace BangBangFDefine {
  ...
  BangBangF::g_fun_eval(
    ...
    real_type t8   = C1(t2);
    real_type t9   = C1_constr(t8);
    real_type result__ = t2 * LM__[0] + t5 * LM__[1] + t7 + t9;
    if ( m_debug ) {
  ...
  BangBangF::DgDu_sparse(
    ...
    LM__[1] = (LL__[1]+LR__[1])/2;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    result__[ 0   ] = ALIAS_FControl_D_1_1(UM__[0], -1, 1);
    if ( m_debug )
  ...
  BangBangF::m_eval(
    ...
    real_type t8   = pow(V__[0] - t3, 2);
    real_type t11  = pow(V__[1] - t1, 2);
    real_type result__ = t2 + t5 + t8 + t11;
    if ( m_debug ) {
