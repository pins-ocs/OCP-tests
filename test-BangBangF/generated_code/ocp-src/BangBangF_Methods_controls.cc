...
namespace BangBangFDefine {
  ...
  BangBangF::g_fun_eval(
    ...
    real_type t4   = C1(t3);
    real_type t5   = C1_constr(t4);
    real_type result__ = t1 * MU__[1] + t3 * MU__[0] + t2 + t5;
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer BangBangF::DgDxpm_numRows() const { return 1; }
  integer BangBangF::DgDxpm_numCols() const { return 4; }
  integer BangBangF::DgDxpm_nnz()     const { return 1; }

  void
  BangBangF::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  BangBangF::u_eval_analytic(
    ...
    integer i_segment = NODE__.i_segment;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    U__[ iU_F ] = FControl.solve(-MU__[1], -1, 1);
    if ( m_debug )
