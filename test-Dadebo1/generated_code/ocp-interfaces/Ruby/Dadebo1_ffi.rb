...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libDadebo1.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libDadebo1.#{DYLIB_EXT}"
end
...
module Dadebo1
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libDadebo1.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = Dadebo1.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = Dadebo1.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = Dadebo1.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      Dadebo1.OCP_write_ocp_solution( @id, fname )
    end
