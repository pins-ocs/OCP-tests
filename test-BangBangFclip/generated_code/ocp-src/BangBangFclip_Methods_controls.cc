...
namespace BangBangFclipDefine {
  ...
  BangBangFclip::g_fun_eval(
    ...
    real_type t1   = LM__[0];
    real_type t4   = LM__[1];
    real_type t6   = ModelPars[iM_minClip];
    real_type t7   = ModelPars[iM_maxClip];
    real_type t8   = clip(XL__[iX_F], t6, t7);
    real_type t11  = UM__[0];
    real_type t14  = ModelPars[iM_vFmax];
    real_type t15  = controlForce(t11, -t14, t14);
    real_type t20  = clip(XR__[iX_F], t6, t7);
    real_type result__ = XL__[iX_v] * t1 + XR__[iX_v] * t1 + 2 * t11 * LM__[2] + t20 * t4 + t8 * t4 + 2 * t15;
    if ( m_debug ) {
  ...
  BangBangFclip::g_eval(
    ...
    real_type t4   = ALIAS_controlForce_D_1(UM__[0], -t3, t3);
    result__[ 0   ] = 2 * LM__[2] + 2 * t4;
    if ( m_debug )
  ...
  BangBangFclip::DgDu_sparse(
    ...
    real_type t3   = ALIAS_controlForce_D_1_1(UM__[0], -t2, t2);
    result__[ 0   ] = 2 * t3;
    if ( m_debug )
  ...
  BangBangFclip::u_eval_analytic(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t2   = ModelPars[iM_vFmax];
    U__[ iU_vF ] = controlForce.solve(-LM__[2], -t2, t2);
    if ( m_debug )
