...
namespace BangBangFclipDefine {
  ...
  BangBangFclip::g_fun_eval(
    ...
    real_type result__ = t1 * MU__[2] + t11 * MU__[1] + MU__[0] * X__[iX_v] + t3;
    if ( m_debug ) {
  ...
  BangBangFclip::g_eval(
    ...
    result__[ 0   ] = t3 + MU__[2];
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 1, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer BangBangFclip::DgDxpm_numRows() const { return 1; }
  integer BangBangFclip::DgDxpm_numCols() const { return 6; }
  integer BangBangFclip::DgDxpm_nnz()     const { return 1; }

  void
  BangBangFclip::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  BangBangFclip::u_eval_analytic(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t2   = ModelPars[iM_vFmax];
    U__[ iU_vF ] = controlForce.solve(-MU__[2], -t2, t2);
    if ( m_debug )
