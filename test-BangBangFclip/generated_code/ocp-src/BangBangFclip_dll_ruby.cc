...
#include "BangBangFclip_dll_ruby.hh"
#include <map>
...
namespace BangBangFclipDefine {
  ...
  static map< string, BangBangFclip_Problem * > problems;
  ...
  */
  ...
  BangBangFclip_ocp_guess( char const id[], GenericContainer & gc_data ) {
    ...
    if ( it == problems.end() ) {
      ...
      it->second->guess( gc_data("Guess", "BangBangFclip_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  BangBangFclip_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("BangBangFclip_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
