...
namespace BangBangFclipDefine {
  ...
  void
  BangBangFclip::p_guess_eval( P_p_type P__ ) const {
  }
  ...
  bool
  BangBangFclip::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  integer BangBangFclip::u_guess_numEqns() const { return 1; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  BangBangFclip::u_guess_eval(
    NodeQXL const & NODE__,
    P_const_p_type  P__,
    ...
    UGUESS__[ iU_vF ] = controlForce.solve(-MU__[2], -t2, t2);
    if ( m_debug )
  ...
  bool
  BangBangFclip::u_check_if_admissible(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__
