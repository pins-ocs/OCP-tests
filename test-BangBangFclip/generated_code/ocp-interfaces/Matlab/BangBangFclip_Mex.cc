...
#define MODEL_NAME  "BangBangFclip"
#define MODEL_CLASS BangBangFclip
...
public:
  ...
  do_eval_F(
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "BangBangFclip_Mex('eval_F',...) error: {}", exc.what() ).c_str() );
      ok = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "BangBangFclip_Mex('eval_F',...) unkown error\n" );
      ok = false;
    }
  ...
  do_eval_JF(
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "BangBangFclip_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
      ok_value = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "BangBangFclip_Mex('eval_JF',...) unkown error\n" );
      ok_value = false;
    }
    int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
    MEX_ASSERT( ok == 0, CMD "failed the call sparse(...)" );
  ...
  do_fun(
    ...
    #define CMD MODEL_NAME "_Mex('fun', obj, xo__x ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
  ...
  do_fun_D(
    ...
    #define CMD MODEL_NAME "_Mex('fun_D', obj, xo__x ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
    real_type const * arg0 = getMatrixPointer( arg_in_2, N0, M0, CMD " xo__x" );
  ...
  do_fun_DD(
    ...
    #define CMD MODEL_NAME "_Mex('fun_DD', obj, xo__x ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
    real_type const * arg0 = getMatrixPointer( arg_in_2, N0, M0, CMD " xo__x" );
...
do_fun(
  ...
  MEX_ASSERT2(
    nrhs >= 2,
    MODEL_NAME "_Mex('fun',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  convertMat2Ptr<ProblemStorage>(arg_in_1)->do_fun( nlhs, plhs, nrhs, prhs );
}
...
do_fun_D(
  ...
  MEX_ASSERT2(
    nrhs >= 2,
    MODEL_NAME "_Mex('fun_D',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  convertMat2Ptr<ProblemStorage>(arg_in_1)->do_fun_D( nlhs, plhs, nrhs, prhs );
}
...
do_fun_DD(
  ...
  MEX_ASSERT2(
    nrhs >= 2,
    MODEL_NAME "_Mex('fun_DD',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  convertMat2Ptr<ProblemStorage>(arg_in_1)->do_fun_DD( nlhs, plhs, nrhs, prhs );
}
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt( fmt::format( "BangBangFclip_Mex, unknown command: {}", exc.what() ).c_str() );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt( fmt::format( "BangBangFclip_Mex, error: {}", exc.what() ).c_str() );
  }
  catch (...) {
