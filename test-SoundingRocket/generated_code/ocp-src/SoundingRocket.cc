...
namespace SoundingRocketDefine {
  ...
  */
  void
  SoundingRocket::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "SoundingRocket::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  SoundingRocket::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "SoundingRocket::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in SoundingRocket::setup not all parameters are set!\n"
    );
  }

  void
  SoundingRocket::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  SoundingRocket::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  SoundingRocket::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  SoundingRocket::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  SoundingRocket::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "SoundingRocket::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  SoundingRocket::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "SoundingRocket::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in SoundingRocket::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  SoundingRocket::info_model( integer level ) const {
  ...
  */
  void
  SoundingRocket::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("SoundingRocket::setup, Debug");
  ...
  */
  void
  SoundingRocket::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  SoundingRocket::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
