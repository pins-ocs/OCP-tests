...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libSoundingRocket.#{DYLIB_EXT}"
...
module SoundingRocket
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libSoundingRocket.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = SoundingRocket.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = SoundingRocket.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = SoundingRocket.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      SoundingRocket.OCP_write_ocp_solution( @id, fname )
    end
