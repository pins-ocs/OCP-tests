...
main() {
  ...
  try {
    ...
    SlidingMode      model("SlidingMode",&TP,&console);
    ...
    gc_data["DumpFile"] = "SlidingMode_dump";
...
SlidingMode_data.Mesh["segments"][0]["length"] = 3;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/SlidingMode_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/SlidingMode_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
