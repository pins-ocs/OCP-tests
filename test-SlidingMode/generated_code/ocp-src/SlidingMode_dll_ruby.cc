...
#include "SlidingMode_dll_ruby.hh"
#include <map>
...
namespace SlidingModeDefine {
  ...
  static map< string, SlidingMode_Problem * > problems;
  ...
  */
  ...
  SlidingMode_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, SlidingMode_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  SLIDINGMODE_API_DLL
  bool
  SlidingMode_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, SlidingMode_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "SlidingMode_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  SlidingMode_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("SlidingMode_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  SlidingMode_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, SlidingMode_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("SlidingMode_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
