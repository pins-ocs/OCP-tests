...
namespace SlidingModeDefine {
  ...
  void
  SlidingMode::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  SlidingMode::xlambda_guess_eval(
    integer              i_segment,
  ...
  bool
  SlidingMode::p_check( P_const_pointer_type P__ ) const {
    return true;
  }
  ...
  bool
  SlidingMode::xlambda_check_node(
    integer              ipos,
  ...
  bool
  SlidingMode::xlambda_check_cell(
    integer              icell,
  ...
  integer SlidingMode::u_guess_numEqns() const { return 1; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  SlidingMode::u_guess_eval(
    NodeType2 const    & LEFT__,
    NodeType2 const    & RIGHT__,
    P_const_pointer_type P__,
  ...
  bool
  SlidingMode::u_check_if_admissible(
    NodeType2 const    & NODE__,
...
// EOF: SlidingMode_Methods_Guess.cc
