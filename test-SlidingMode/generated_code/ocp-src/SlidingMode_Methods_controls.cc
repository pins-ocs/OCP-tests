...
namespace SlidingModeDefine {
  ...
  SlidingMode::g_fun_eval(
    ...
    real_type result__ = t1 * MU__[0] + t7 * MU__[1] + t2;
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer SlidingMode::DgDxpm_numRows() const { return 1; }
  integer SlidingMode::DgDxpm_numCols() const { return 4; }
  integer SlidingMode::DgDxpm_nnz()     const { return 1; }

  void
  SlidingMode::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
