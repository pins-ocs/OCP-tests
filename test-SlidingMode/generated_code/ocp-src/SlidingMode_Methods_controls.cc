...
namespace SlidingModeDefine {
  ...
  integer
  SlidingMode::DgDxlp_numRows() const
  { return 1; }

  integer
  SlidingMode::DgDxlp_numCols() const
  { return 4; }

  integer
  SlidingMode::DgDxlp_nnz() const
  { return 1; }

  void
  SlidingMode::DgDxlp_pattern(
    integer_ptr iIndex,
    integer_ptr jIndex
  ) const {
  ...
  void
  SlidingMode::DuDxlxlp_full_analytic(
    NodeType2 const &          LEFT__,
    NodeType2 const &          RIGHT__,
    P_const_pointer_type       P__,
    U_const_pointer_type       U__,
  ...
  SlidingMode::m_eval(
    ...
    real_type t10  = pow(-t7 + V__[1], 2);
    real_type result__ = t2 + t5 + t10;
    if ( m_debug ) {
...
// EOF: SlidingMode_Methods_controls.cc
