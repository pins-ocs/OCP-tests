...
namespace SlidingModeDefine {
  ...
  SlidingMode::H_eval(
    ...
    real_type result__ = t3 * MU__[1] + MU__[0] * U__[iU_u];
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer SlidingMode::D2mayerD2xxp_numRows() const { return 4; }
  integer SlidingMode::D2mayerD2xxp_numCols() const { return 4; }
  integer SlidingMode::D2mayerD2xxp_nnz()     const { return 0; }

  void
  SlidingMode::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  SlidingMode::D2mayerD2xxp_sparse(
    NodeQX const & LEFT__,
    NodeQX const & RIGHT__,
    P_const_p_type P__,
    real_ptr       result__
  ...
  integer SlidingMode::DlagrangeDxpu_numEqns() const { return 3; }

  void
  SlidingMode::DlagrangeDxpu_eval(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer SlidingMode::D2lagrangeD2xpu_numRows() const { return 3; }
  integer SlidingMode::D2lagrangeD2xpu_numCols() const { return 3; }
  integer SlidingMode::D2lagrangeD2xpu_nnz()     const { return 0; }

  void
  SlidingMode::D2lagrangeD2xpu_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  SlidingMode::D2lagrangeD2xpu_sparse(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  integer SlidingMode::DsegmentLinkDxxp_numRows() const { return 0; }
  integer SlidingMode::DsegmentLinkDxxp_numCols() const { return 0; }
  integer SlidingMode::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  SlidingMode::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  SlidingMode::DsegmentLinkDxxp_sparse(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
  ...
  integer SlidingMode::post_numEqns() const { return 1; }
...
// EOF: SlidingMode_Methods_problem.cc
