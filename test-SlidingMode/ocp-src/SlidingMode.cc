...
namespace SlidingModeDefine {
  ...
  */
  void
  SlidingMode::updateContinuation( integer phase, real_type s ) {
  }
  ...
  */
  ...
  SlidingMode::setupParameters( GenericContainer const & gc_data ) {
    ...
    UTILS_ASSERT0(
      allfound, "in SlidingMode::setup not all parameters are set!\n"
    );
  }
  ...
  */
  void
  SlidingMode::setup( GenericContainer const & gc ) {
  ...
  */
  void
  SlidingMode::get_names( GenericContainer & out ) const {
  ...
  */
  void
  SlidingMode::diagnostic( GenericContainer const & gc ) {
  ...
  void
  SlidingMode::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
