/*-----------------------------------------------------------------------*\
 |  file: MinimumFuelOrbitRaising_dll_pins.cc                            |
 |                                                                       |
...
#include "MinimumFuelOrbitRaising_dll_pins.hh"
#include <map>
...
#define MAP_PROBLEM map< string, MinimumFuelOrbitRaising_Problem * >
...
namespace MinimumFuelOrbitRaisingDefine {
  ...
  EXTERN_C
  MINIMUMFUELORBITRAISING_API_DLL
  mrb_value
  mrb_MinimumFuelOrbitRaising_ocp_setup( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "MinimumFuelOrbitRaising_ocp_setup, missing first parameter or is nota a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "MinimumFuelOrbitRaising_ocp_setup, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      problems[id] = new MinimumFuelOrbitRaising_Problem( pTP, pConsole );
  ...
  MINIMUMFUELORBITRAISING_API_DLL
  mrb_value
  mrb_MinimumFuelOrbitRaising_ocp_guess( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "MinimumFuelOrbitRaising_ocp_guess, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "MinimumFuelOrbitRaising_ocp_guess, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "MinimumFuelOrbitRaising_ocp_guess, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      it->second->guess(
        gc_data(
          "Guess",
          "mrb_MinimumFuelOrbitRaising_ocp_guess Missing field `Guess`"
        )
      );
    }
  ...
  MINIMUMFUELORBITRAISING_API_DLL
  mrb_value
  mrb_MinimumFuelOrbitRaising_ocp_solve( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "MinimumFuelOrbitRaising_ocp_solve, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "MinimumFuelOrbitRaising_ocp_solve, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "MinimumFuelOrbitRaising_ocp_solve, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      if ( !ok )
        pConsole->error( "MinimumFuelOrbitRaising_ocp_solve, solution failed!\n" );
  ...
  MINIMUMFUELORBITRAISING_API_DLL
  mrb_value
  mrb_MinimumFuelOrbitRaising_write_ocp_solution( mrb_state *mrb, mrb_value self ) {
    mrb_value a_string;
    ...
    MAP_PROBLEM::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error( "MinimumFuelOrbitRaising_ocp_write_to_file, no solution found!\n" );
    } else {
  ...
  MINIMUMFUELORBITRAISING_API_DLL
  mrb_value
  mrb_MinimumFuelOrbitRaising_ocp_solution_range( mrb_state *mrb, mrb_value self ) {
  ...
  MINIMUMFUELORBITRAISING_API_DLL
  mrb_value
  mrb_MinimumFuelOrbitRaising_ocp_eval_solution( mrb_state *mrb, mrb_value self ) {
  ...
  MINIMUMFUELORBITRAISING_API_DLL
  void
  mrb_libMinimumFuelOrbitRaising_gem_init(mrb_state* mrb) {
    ...
    mrb_define_const(
      ...
      ocp_class,
      "NAME",
      mrb_str_new_cstr(mrb, "MinimumFuelOrbitRaising")
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "setup",
      mrb_MinimumFuelOrbitRaising_ocp_setup,
      MRB_ARGS_NONE()
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "guess",
      mrb_MinimumFuelOrbitRaising_ocp_guess,
      MRB_ARGS_NONE()
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "solve",
      mrb_MinimumFuelOrbitRaising_ocp_solve,
      MRB_ARGS_NONE()
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "write_ocp_solution",
      mrb_MinimumFuelOrbitRaising_write_ocp_solution,
      MRB_ARGS_REQ(1)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "eval_solution",
      mrb_MinimumFuelOrbitRaising_ocp_eval_solution,
      MRB_ARGS_REQ(2)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "solution_range",
      mrb_MinimumFuelOrbitRaising_ocp_solution_range,
      MRB_ARGS_REQ(1)
    );
  ...
  MINIMUMFUELORBITRAISING_API_DLL
  void
  mrb_libMinimumFuelOrbitRaising_gem_final(mrb_state* mrb)
  {
  }
}

// EOF: MinimumFuelOrbitRaising_dll_pins.cc
