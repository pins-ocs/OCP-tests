...
namespace MinimumFuelOrbitRaisingDefine {
  ...
  integer
  MinimumFuelOrbitRaising::DgDxlxlp_numRows() const
  { return 1; }

  integer
  MinimumFuelOrbitRaising::DgDxlxlp_numCols() const
  { return 12; }

  integer
  MinimumFuelOrbitRaising::DgDxlxlp_nnz() const
  { return 4; }

  void
  MinimumFuelOrbitRaising::DgDxlxlp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
  ...
  void
  MinimumFuelOrbitRaising::DgDxlxlp_sparse(
    NodeType2 const &    LEFT__,
    NodeType2 const &    RIGHT__,
    U_const_pointer_type UM__,
    P_const_pointer_type P__,
  ...
  MinimumFuelOrbitRaising::DmDu_eval(
    ...
    result__[ 0   ] = -2 * (V__[1] - t6 * t3 * t2 + 1.0 / t8 - t10 * t19) * t10 * t18 * t22 + 2 * t10 * (-t10 * t18 * t22 + t2 * t6 * X__[iX_x2] + V__[2]) * t19;
    if ( m_debug )
