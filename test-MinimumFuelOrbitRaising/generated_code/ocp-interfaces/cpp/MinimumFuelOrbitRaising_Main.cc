...
main() {
  ...
  try {
    MinimumFuelOrbitRaising * m_model = new MinimumFuelOrbitRaising("MinimumFuelOrbitRaising",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/MinimumFuelOrbitRaising_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/MinimumFuelOrbitRaising_Data.lua";
    #endif
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/MinimumFuelOrbitRaising_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/MinimumFuelOrbitRaising_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
