...
namespace TwoStageCSTRDefine {
  ...
  */
  void
  TwoStageCSTR::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
  ...
  */
  void
  TwoStageCSTR::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "TwoStageCSTR::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in TwoStageCSTR::setup not all parameters are set!\n"
    );
  }

  void
  TwoStageCSTR::setup_parameters( real_type const Pars[] ) {
  ...
  */
  void
  TwoStageCSTR::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  TwoStageCSTR::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  TwoStageCSTR::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  TwoStageCSTR::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "TwoStageCSTR::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  TwoStageCSTR::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "TwoStageCSTR::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in TwoStageCSTR::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  TwoStageCSTR::info_classes() const {
    int msg_level = 3;
  ...
  */
  void
  TwoStageCSTR::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("TwoStageCSTR::setup, Debug");
  ...
  */
  void
  TwoStageCSTR::get_names( GenericContainer & out ) const {
  ...
  void
  TwoStageCSTR::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
