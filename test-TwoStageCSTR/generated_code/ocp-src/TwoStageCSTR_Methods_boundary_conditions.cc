...
namespace TwoStageCSTRDefine {
  ...
  integer TwoStageCSTR::bc_numEqns() const { return 4; }

  void
  TwoStageCSTR::bc_eval(
    NodeQX const & LEFT__,
    ...
    result__[ 3   ] = XL__[iX_x4] - ModelPars[iM_x4_i];
    if ( m_debug )
      Mechatronix::check_in_segment2( result__, "bc_eval", 4, i_segment_left, i_segment_right );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer TwoStageCSTR::DbcDxxp_numRows() const { return 4; }
  integer TwoStageCSTR::DbcDxxp_numCols() const { return 8; }
  integer TwoStageCSTR::DbcDxxp_nnz()     const { return 4; }

  void
  TwoStageCSTR::DbcDxxp_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer TwoStageCSTR::D2bcD2xxp_numRows() const { return 8; }
  integer TwoStageCSTR::D2bcD2xxp_numCols() const { return 8; }
  integer TwoStageCSTR::D2bcD2xxp_nnz()     const { return 0; }

  void
  TwoStageCSTR::D2bcD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  TwoStageCSTR::D2bcD2xxp_sparse(
    NodeQX const &          LEFT__,
    NodeQX const &          RIGHT__,
    P_const_p_type          P__,
  ...
  integer TwoStageCSTR::fd_BC_numEqns() const { return 8; }

  void
  TwoStageCSTR::fd_BC_eval(
    ...
    result__[ 5   ] = -LR__[iL_lambda2__xo];
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer TwoStageCSTR::Dfd_BCDxlxlp_numRows() const { return 8; }
  integer TwoStageCSTR::Dfd_BCDxlxlp_numCols() const { return 16; }
  integer TwoStageCSTR::Dfd_BCDxlxlp_nnz()     const { return 8; }

  void
  TwoStageCSTR::Dfd_BCDxlxlp_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  void
  TwoStageCSTR::Dfd_BCDxlxlp_sparse(
    NodeQXL const &         LEFT__,
    NodeQXL const &         RIGHT__,
    P_const_p_type          P__,
    OMEGA_full_const_p_type OMEGA__,
