...
namespace TwoStageCSTRDefine {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer TwoStageCSTR::DHxpDxpuv_numRows() const { return 4; }
  integer TwoStageCSTR::DHxpDxpuv_numCols() const { return 10; }
  integer TwoStageCSTR::DHxpDxpuv_nnz()     const { return 10; }

  void
  TwoStageCSTR::DHxpDxpuv_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  TwoStageCSTR::DHxpDxpuv_sparse(
    ...
    result__[ 9   ] = -t12;
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "DHxpDxpuv_sparse", 10, i_segment );
  }
