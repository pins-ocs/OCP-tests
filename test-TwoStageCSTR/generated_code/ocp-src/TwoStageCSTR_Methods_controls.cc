...
namespace TwoStageCSTRDefine {
  ...
  TwoStageCSTR::g_eval(
    ...
    real_type t9   = LM__[3];
    real_type t14  = ALIAS_u1Control_D_1(t2, -0.5e0, 0.5e0);
    result__[ 0   ] = -t7 * ModelPars[iM_tau] * t9 + 2 * t2 * t1 + t7 * LM__[1] + t14;
    real_type t15  = UM__[1];
    real_type t21  = ALIAS_u2Control_D_1(t15, -0.5e0, 0.5e0);
    result__[ 1   ] = 2 * t15 * t1 + (-XM__[3] - 0.25e0) * t9 + t21;
    if ( m_debug )
