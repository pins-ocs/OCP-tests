...
namespace TwoPhaseSchwartzDefine {
  ...
  TwoPhaseSchwartz::m_eval(
    ...
    real_type t20  = t1 * t1;
    real_type t25  = pow(V__[1] - 1.0 * t13 + 0.2e0 * t5 * t20 + 0.1e0 * t5, 2);
    real_type t26  = ModelPars[iM_T2];
    real_type t27  = X__[iX_x4];
    real_type t31  = pow(-t27 * t26 + V__[2], 2);
    real_type t34  = X__[iX_x3] * X__[iX_x3];
    real_type t43  = pow(t26 * (U__[iU_u2] - 0.2e0 * t27 * t34 - 0.1e0 * t27) - 1.0 * V__[3], 2);
  ...
  TwoPhaseSchwartz::DmDu_eval(
    ...
    result__[ 0   ] = t2 - 2 * V__[1] + 2 * t1 - 0.4e0 * t8 * t7 - 0.2e0 * t8;
    real_type t12  = ModelPars[iM_T2];
    real_type t15  = X__[iX_x3] * X__[iX_x3];
    real_type t16  = X__[iX_x4];
    result__[ 1   ] = 2.0 * (t12 * (U__[iU_u2] - 0.2e0 * t16 * t15 - 0.1e0 * t16) - 1.0 * V__[3]) * t12;
    if ( m_debug )
