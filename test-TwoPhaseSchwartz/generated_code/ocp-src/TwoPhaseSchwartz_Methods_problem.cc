...
namespace TwoPhaseSchwartzDefine {
  ...
  TwoPhaseSchwartz::H_eval(
    ...
    real_type t16  = X__[iX_x3] * X__[iX_x3];
    real_type t19  = X__[iX_x4];
    real_type t21  = U__[iU_u2];
    real_type t30  = t1 * t1;
    real_type t38  = t21 * t21;
    real_type result__ = t10 + t12 + t14 + ModelPars[iM_T2] * (L__[iL_lambda4__xo] * (t19 * (-0.2e0 * t16 - 0.1e0) + t21) + t19 * L__[iL_lambda3__xo]) + L__[iL_lambda2__xo] * (t5 * (-0.2e0 * t30 - 0.1e0) + t13) + t38 * ModelPars[iM_epsilon] + t5 * L__[iL_lambda1__xo];
    return result__;
  }
#else
  real_type
  ...
  integer
  TwoPhaseSchwartz::DmayerDx_numEqns() const
  { return 8; }

  void
  TwoPhaseSchwartz::DmayerDx_eval(
    NodeType const     & LEFT__,
    NodeType const     & RIGHT__,
    P_const_pointer_type P__,
    real_ptr             result__
  ...
  integer
  TwoPhaseSchwartz::DmayerDp_numEqns() const
  { return 0; }

  void
  TwoPhaseSchwartz::DmayerDp_eval(
    NodeType const     & LEFT__,
    NodeType const     & RIGHT__,
    P_const_pointer_type P__,
    real_ptr             result__
  ...
  TwoPhaseSchwartz::post_eval(
    ...
    real_const_ptr L__ = NODE__.lambda;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    result__[ 0   ] = Q__[iQ_zeta] * ModelPars[iM_T2] + 1;
