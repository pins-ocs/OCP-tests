...
namespace TwoPhaseSchwartzDefine {
  ...
  */
  void
  TwoPhaseSchwartz::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "TwoPhaseSchwartz::setup_parameters: Missing key `Parameters` in data\n"
    );
  ...
  void
  TwoPhaseSchwartz::setup_parameters( real_type const Pars[] ) {
  ...
  */
  void
  TwoPhaseSchwartz::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "TwoPhaseSchwartz::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("bound1"),
      "in TwoPhaseSchwartz::setup_classes(gc) missing key: ``bound1''\n"
    );
    bound1.setup( gc("bound1") );

    UTILS_ASSERT0(
      gc.exists("bound2"),
      "in TwoPhaseSchwartz::setup_classes(gc) missing key: ``bound2''\n"
    );
  ...
  */
  void
  TwoPhaseSchwartz::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  TwoPhaseSchwartz::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  TwoPhaseSchwartz::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "TwoPhaseSchwartz::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  TwoPhaseSchwartz::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "TwoPhaseSchwartz::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in TwoPhaseSchwartz::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  TwoPhaseSchwartz::info_classes() const {
    int msg_level = 3;
    ...
    m_console->message("\nConstraints LT\n",msg_level);
    m_console->message( bound1.info(),msg_level);
    m_console->message( bound2.info(),msg_level);
  ...
  */
  ...
  TwoPhaseSchwartz::setup( GenericContainer const & gc ) {
    ...
    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("TwoPhaseSchwartz::setup, Debug");
