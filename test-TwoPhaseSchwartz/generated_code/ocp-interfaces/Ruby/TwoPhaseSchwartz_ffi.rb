...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libTwoPhaseSchwartz.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libTwoPhaseSchwartz.#{DYLIB_EXT}"
end
...
module TwoPhaseSchwartz
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libTwoPhaseSchwartz.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = TwoPhaseSchwartz.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = TwoPhaseSchwartz.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = TwoPhaseSchwartz.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      TwoPhaseSchwartz.OCP_write_ocp_solution( @id, fname )
    end
