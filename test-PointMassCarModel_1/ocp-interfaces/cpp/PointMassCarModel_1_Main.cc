...
main() {
  ...
  try {
    PointMassCarModel_1 * m_model = new PointMassCarModel_1("PointMassCarModel_1",&TP,&console);

    // user defined Object instances (external)
    Road2D           road( "road" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/PointMassCarModel_1_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/PointMassCarModel_1_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/PointMassCarModel_1_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
