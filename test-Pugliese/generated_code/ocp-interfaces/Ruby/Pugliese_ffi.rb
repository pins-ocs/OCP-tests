...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libPugliese.#{DYLIB_EXT}"
...
module Pugliese
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libPugliese.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = Pugliese.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = Pugliese.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = Pugliese.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      Pugliese.OCP_write_ocp_solution( @id, fname )
    end
