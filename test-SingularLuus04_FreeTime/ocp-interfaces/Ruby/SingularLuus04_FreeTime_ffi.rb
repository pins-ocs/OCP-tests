#-----------------------------------------------------------------------#
#  file: SingularLuus04_FreeTime_run.rb                                 #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libSingularLuus04_FreeTime.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libSingularLuus04_FreeTime.#{DYLIB_EXT}"
end
...
module SingularLuus04_FreeTime
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libSingularLuus04_FreeTime.#{DYLIB_EXT}"
  ...
  attach_function :OCP_setup,
                  :SingularLuus04_FreeTime_ocp_setup,
                  [ :string, :buffer_in ], :bool

  attach_function :OCP_guess,
                  :SingularLuus04_FreeTime_ocp_guess,
                  [ :string, :buffer_in ], :bool

  attach_function :OCP_solve,
                  :SingularLuus04_FreeTime_ocp_solve,
                  [ :string, :buffer_in, :buffer_out ], :bool

  attach_function :OCP_write_ocp_solution,
                  :SingularLuus04_FreeTime_write_ocp_solution,
                  [ :string, :string ], :void
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = SingularLuus04_FreeTime.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = SingularLuus04_FreeTime.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = SingularLuus04_FreeTime.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      SingularLuus04_FreeTime.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: SingularLuus04_FreeTime_ffi.rb
