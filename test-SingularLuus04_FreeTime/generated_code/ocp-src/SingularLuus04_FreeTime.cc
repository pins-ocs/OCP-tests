...
namespace SingularLuus04_FreeTimeDefine {
  ...
  */
  SingularLuus04_FreeTime::SingularLuus04_FreeTime(
    string const   & name,
  ...
  */
  void
  SingularLuus04_FreeTime::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "SingularLuus04_FreeTime::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  SingularLuus04_FreeTime::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "SingularLuus04_FreeTime::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in SingularLuus04_FreeTime::setup not all parameters are set!\n"
    );
  }

  void
  SingularLuus04_FreeTime::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  SingularLuus04_FreeTime::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  SingularLuus04_FreeTime::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  SingularLuus04_FreeTime::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  SingularLuus04_FreeTime::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "SingularLuus04_FreeTime::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  SingularLuus04_FreeTime::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "SingularLuus04_FreeTime::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in SingularLuus04_FreeTime::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  SingularLuus04_FreeTime::info_model( integer level ) const {
  ...
  */
  void
  SingularLuus04_FreeTime::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("SingularLuus04_FreeTime::setup, Debug");
  ...
  */
  void
  SingularLuus04_FreeTime::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  SingularLuus04_FreeTime::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
...
// EOF: SingularLuus04_FreeTime.cc
