...
# 
# Minimum time problem for single track with road description
# Author: Francesco Biral
# Date:        13/07/2016
# 
# Optimal control with road and fixed space (free time)
# Solve the minimum time problem (free time) given the road geometry. Vehicle is a single track model with tyre ellipse of adherence. 
# Vehicle power is constrained. 
# Longitudinal force is split between front and rear and traction is applied to rear only.
# Road porperties functions are mapped to methods of Mechatronics library C++ road class. 
# What you can learn from this tutorial?
# 1. Change of coordinate: from time domain to space domain (curvlinear path)
# 2. Define mesh and road geometry mapping to methods of C++ road class
...
qvars := [Curv(s)        = isoCurvature(s),
          leftWidth(s)   = leftWidth(s),
          rightWidth(s)  = rightWidth(s),
          #xCoM(s,n)     = isoX(s,n),  #**** not working
          #yCoM(s,n)     = isoY(s,n),  #**** not working
          xLane(s)       = isoX(s), 
          yLane(s)       = isoY(s),
          theta(s)       = isoAngle(s)]:
...
addBoundaryConditions(initial=[n=0,xi=0,u=u0,v=0,Omega=0, alpha__r=0, alpha__f=0, delta=0, S = 0], 
                      final  =[n=0,xi=0]);
...
# Road boundaries
# Right road boarder
addUnilateralConstraint( n(zeta)-vehHalfWidth >= -rightWidth(zeta),
                         roadRightLateralBoundaries,
                         epsilon   = epsi0, 
                         tolerance = 0.01,    
                         scale     = pen_scale );
# Left road boarder
addUnilateralConstraint( n(zeta)+vehHalfWidth <= leftWidth(zeta), 
...
xLEdge  := xLane(zeta)-sin(theta(zeta))*(leftWidth(zeta)):
yLEdge  := yLane(zeta)+cos(theta(zeta))*(leftWidth(zeta)):
xREdge  := xLane(zeta)+sin(theta(zeta))*(rightWidth(zeta)):
yREdge  := yLane(zeta)-cos(theta(zeta))*(rightWidth(zeta)):
...
cont := [[wT = 0.01*(1-s)+1*s,
          ...
          ["roadRightLateralBoundaries","epsilon"] = epsi,
          ["roadLeftLateralBoundaries", "epsilon"] = epsi,
...
project_name := "SingleTrack_Road";  # project name
;
