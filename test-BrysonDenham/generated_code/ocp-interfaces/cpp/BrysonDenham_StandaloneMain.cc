...
main() {
  ...
  try {
    ...
    BrysonDenham     model("BrysonDenham",&TP,&console);
    ...
    gc_data["DumpFile"] = "BrysonDenham_dump";
    ...
    data_X1bound["subType"]   = "PENALTY_REGULAR";
    data_X1bound["epsilon"]   = epsi;
    data_X1bound["tolerance"] = tol;
...
BrysonDenham_data.Mesh["segments"][0]["length"] = 1;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/BrysonDenham_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/BrysonDenham_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
