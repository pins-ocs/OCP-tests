...
namespace BrysonDenhamDefine {
  ...
  BrysonDenham::g_fun_eval(
    ...
    real_type t2   = t1 * t1;
    real_type t6   = X1bound(X__[iX_x] - 1.0 / 9.0);
    real_type result__ = t2 / 2 + t6 + MU__[0] * X__[iX_v] + t1 * MU__[1];
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer BrysonDenham::DgDxpm_numRows() const { return 1; }
  integer BrysonDenham::DgDxpm_numCols() const { return 4; }
  integer BrysonDenham::DgDxpm_nnz()     const { return 1; }

  void
  BrysonDenham::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
