...
#include "PointMassCarModel_3_dll_ruby.hh"
#include <map>
...
namespace PointMassCarModel_3Define {
  ...
  */
  ...
  POINTMASSCARMODEL_3_API_DLL
  bool
  PointMassCarModel_3_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, PointMassCarModel_3_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "PointMassCarModel_3_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  PointMassCarModel_3_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("PointMassCarModel_3_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  PointMassCarModel_3_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, PointMassCarModel_3_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("PointMassCarModel_3_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: PointMassCarModel_3_dll_ruby.cc
