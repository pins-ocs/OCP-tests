...
namespace PointMassCarModel_3Define {
  ...
  void
  PointMassCarModel_3::continuation_step_2( real_type s ) {
  ...
  PointMassCarModel_3::H_eval(
    ...
    real_type t10  = ALIAS_Kappa(X__[iX_s]);
    real_type t11  = zeta__dot(t6, t7, X__[iX_n], t10);
    real_type t16  = sin(t7);
    real_type t28  = t6 * t6;
    real_type result__ = t3 * ModelPars[iM_wT] + t3 * t11 * MU__[0] + t3 * t16 * t6 * MU__[1] + (-t10 * t11 + X__[iX_Omega]) * t3 * MU__[2] + (-t28 * ModelPars[iM_kD] + X__[iX_fx]) * t3 * MU__[3] + t3 * ModelPars[iM_v__Omega__max] * MU__[4] * U__[iU_v__Omega] + t3 * ModelPars[iM_v__fx__max] * MU__[5] * U__[iU_v__fx];
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer PointMassCarModel_3::D2mayerD2xxp_numRows() const { return 14; }
  integer PointMassCarModel_3::D2mayerD2xxp_numCols() const { return 14; }
  integer PointMassCarModel_3::D2mayerD2xxp_nnz()     const { return 0; }

  void
  PointMassCarModel_3::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  PointMassCarModel_3::D2mayerD2xxp_sparse(
    NodeQX const & LEFT__,
    NodeQX const & RIGHT__,
    P_const_p_type P__,
    real_ptr       result__
  ...
  integer PointMassCarModel_3::DlagrangeDxpu_numEqns() const { return 9; }

  void
  PointMassCarModel_3::DlagrangeDxpu_eval(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer PointMassCarModel_3::D2lagrangeD2xpu_numRows() const { return 9; }
  integer PointMassCarModel_3::D2lagrangeD2xpu_numCols() const { return 9; }
  integer PointMassCarModel_3::D2lagrangeD2xpu_nnz()     const { return 1; }

  void
  PointMassCarModel_3::D2lagrangeD2xpu_pattern( integer iIndex[], integer jIndex[] ) const {
    iIndex[0 ] = 6   ; jIndex[0 ] = 6   ;
  }


  void
  PointMassCarModel_3::D2lagrangeD2xpu_sparse(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  integer PointMassCarModel_3::DsegmentLinkDxxp_numRows() const { return 0; }
  integer PointMassCarModel_3::DsegmentLinkDxxp_numCols() const { return 0; }
  integer PointMassCarModel_3::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  PointMassCarModel_3::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  PointMassCarModel_3::DsegmentLinkDxxp_sparse(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
