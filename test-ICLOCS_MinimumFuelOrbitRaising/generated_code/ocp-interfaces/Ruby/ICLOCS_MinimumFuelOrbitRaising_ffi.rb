...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libICLOCS_MinimumFuelOrbitRaising.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libICLOCS_MinimumFuelOrbitRaising.#{DYLIB_EXT}"
end
...
module ICLOCS_MinimumFuelOrbitRaising
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libICLOCS_MinimumFuelOrbitRaising.#{DYLIB_EXT}"
  ...
  attach_function :OCP_setup,
                  :ICLOCS_MinimumFuelOrbitRaising_ocp_setup,
                  [ :string, :buffer_in ], :bool

  attach_function :OCP_guess,
                  :ICLOCS_MinimumFuelOrbitRaising_ocp_guess,
                  [ :string, :buffer_in ], :bool

  attach_function :OCP_solve,
                  :ICLOCS_MinimumFuelOrbitRaising_ocp_solve,
                  [ :string, :buffer_in, :buffer_out ], :bool

  attach_function :OCP_write_ocp_solution,
                  :ICLOCS_MinimumFuelOrbitRaising_write_ocp_solution,
                  [ :string, :string ], :void
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ICLOCS_MinimumFuelOrbitRaising.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ICLOCS_MinimumFuelOrbitRaising.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ICLOCS_MinimumFuelOrbitRaising.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ICLOCS_MinimumFuelOrbitRaising.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: ICLOCS_MinimumFuelOrbitRaising_ffi.rb
