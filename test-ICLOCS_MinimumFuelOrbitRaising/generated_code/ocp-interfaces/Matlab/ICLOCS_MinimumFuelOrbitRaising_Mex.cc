...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"mass",&ProblemStorage::do_mass},
  {"mass_D",&ProblemStorage::do_mass_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "ICLOCS_MinimumFuelOrbitRaising_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
    ...
    mexErrMsgTxt(
      fmt::format( "ICLOCS_MinimumFuelOrbitRaising_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "ICLOCS_MinimumFuelOrbitRaising_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
