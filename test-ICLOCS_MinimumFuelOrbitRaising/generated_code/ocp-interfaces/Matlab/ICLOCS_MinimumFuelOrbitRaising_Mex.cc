...
"ICLOCS_MinimumFuelOrbitRaising_Mex('command', arguments ):\n"
"%===========================================================================%\n"
;

#define MODEL_NAME  "ICLOCS_MinimumFuelOrbitRaising"
#define MODEL_CLASS ICLOCS_MinimumFuelOrbitRaising
...
public:
  ...
  do_eval_F(
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "ICLOCS_MinimumFuelOrbitRaising_Mex('eval_F',...) error: {}", exc.what() ).c_str() );
      ok = false;
    } catch ( ... ) {
...
do_mass(
  ...
  MEX_ASSERT2(
    nrhs >= 2,
    MODEL_NAME "_Mex('mass',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  convertMat2Ptr<ProblemStorage>(arg_in_1)->do_mass( nlhs, plhs, nrhs, prhs );
}
...
do_mass_D(
  ...
  MEX_ASSERT2(
    nrhs >= 2,
    MODEL_NAME "_Mex('mass_D',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  convertMat2Ptr<ProblemStorage>(arg_in_1)->do_mass_D( nlhs, plhs, nrhs, prhs );
}
...
do_mass_DD(
  ...
  MEX_ASSERT2(
    nrhs >= 2,
    MODEL_NAME "_Mex('mass_DD',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  convertMat2Ptr<ProblemStorage>(arg_in_1)->do_mass_DD( nlhs, plhs, nrhs, prhs );
}
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt( fmt::format( "ICLOCS_MinimumFuelOrbitRaising_Mex, unknown command: {}", exc.what() ).c_str() );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt( fmt::format( "ICLOCS_MinimumFuelOrbitRaising_Mex, error: {}", exc.what() ).c_str() );
  }
  catch (...) {
