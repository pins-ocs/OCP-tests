...
main() {
  ...
  try {
    ICLOCS_MinimumFuelOrbitRaising * m_model = new ICLOCS_MinimumFuelOrbitRaising("ICLOCS_MinimumFuelOrbitRaising",n_threads,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/ICLOCS_MinimumFuelOrbitRaising_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/ICLOCS_MinimumFuelOrbitRaising_Data.lua";
    #endif
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/ICLOCS_MinimumFuelOrbitRaising_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/ICLOCS_MinimumFuelOrbitRaising_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
