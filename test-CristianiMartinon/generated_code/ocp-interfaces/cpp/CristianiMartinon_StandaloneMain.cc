/*-----------------------------------------------------------------------*\
 |  file: CristianiMartinon_Main.cc                                      |
 |                                                                       |
...
main() {
  ...
  try {
    ...
    CristianiMartinon model("CristianiMartinon",&console,&TP);
    ...
    gc_data["DumpFile"] = "CristianiMartinon_dump";
    ...
    // Model Parameters
    data_Parameters["WY"] = 1;

    // Guess Parameters
    data_Parameters["Tguess"] = 1;

    // Boundary Conditions
    data_Parameters["x_f"] = 3;
    data_Parameters["x_i"] = -2.5;
    data_Parameters["y_f"] = 0;
    data_Parameters["y_i"] = 0;

    // Post Processing Parameters

    // User Function Parameters
    data_Parameters["ymax_guess"] = 1.48001;
    ...
    GenericContainer & data_pos = gc_MappedObjects["pos"];
    data_pos["h"] = 0.0001;
    ...
    data_Tpositive["subType"]   = "PENALTY_PIECEWISE";
    data_Tpositive["epsilon"]   = T_epsi;
    data_Tpositive["tolerance"] = T_tol;
...
CristianiMartinon_data.Mesh["s0"] = 0;
CristianiMartinon_data.Mesh["segments"][0]["length"] = 1;
CristianiMartinon_data.Mesh["segments"][0]["n"] = 100;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/CristianiMartinon_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/CristianiMartinon_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
