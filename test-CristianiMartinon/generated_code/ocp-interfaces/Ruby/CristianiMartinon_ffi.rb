...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libCristianiMartinon.#{DYLIB_EXT}"
...
module CristianiMartinon
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libCristianiMartinon.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = CristianiMartinon.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = CristianiMartinon.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = CristianiMartinon.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      CristianiMartinon.OCP_write_ocp_solution( @id, fname )
    end
