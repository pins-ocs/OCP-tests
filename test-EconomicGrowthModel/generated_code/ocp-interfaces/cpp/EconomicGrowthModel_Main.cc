...
main() {
  ...
  try {
    EconomicGrowthModel * m_model = new EconomicGrowthModel("EconomicGrowthModel",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/EconomicGrowthModel_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/EconomicGrowthModel_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/EconomicGrowthModel_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
