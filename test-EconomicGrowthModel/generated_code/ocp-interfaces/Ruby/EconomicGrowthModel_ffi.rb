#-----------------------------------------------------------------------#
#  file: EconomicGrowthModel_run.rb                                     #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libEconomicGrowthModel.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libEconomicGrowthModel.#{DYLIB_EXT}"
end
...
module EconomicGrowthModel

  extend FFI::Library

  LIB_PATH = MECHATRONIX_LIB_PATH+"/libEconomicGrowthModel.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = EconomicGrowthModel.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = EconomicGrowthModel.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = EconomicGrowthModel.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      EconomicGrowthModel.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: EconomicGrowthModel_ffi.rb
