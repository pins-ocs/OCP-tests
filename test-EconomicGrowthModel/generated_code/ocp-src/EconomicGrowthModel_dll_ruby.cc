/*-----------------------------------------------------------------------*\
 |  file: EconomicGrowthModel_dll_ruby.cc                                |
 |                                                                       |
...
#include "EconomicGrowthModel_dll_ruby.hh"
#include <map>
...
namespace EconomicGrowthModelDefine {
  ...
  static map< string, EconomicGrowthModel_Problem * > problems;
  ...
  */
  ...
  EconomicGrowthModel_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, EconomicGrowthModel_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  ECONOMICGROWTHMODEL_API_DLL
  bool
  EconomicGrowthModel_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, EconomicGrowthModel_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "EconomicGrowthModel_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  EconomicGrowthModel_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("EconomicGrowthModel_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  EconomicGrowthModel_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, EconomicGrowthModel_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("EconomicGrowthModel_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: EconomicGrowthModel_dll_ruby.cc
