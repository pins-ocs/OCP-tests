...
namespace EconomicGrowthModelDefine {
  ...
  EconomicGrowthModel::g_fun_eval(
    ...
    real_type result__ = t3 * t1 + t5 + t11 * t2 * MU__[0] + t11 * (1 - t2) * MU__[1];
    if ( m_debug ) {
  ...
  EconomicGrowthModel::g_eval(
    ...
    result__[ 0   ] = t1 * t8 * MU__[0] - t1 * t8 * MU__[1] + t3 * t1;
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 1, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer EconomicGrowthModel::DgDxpm_numRows() const { return 1; }
  integer EconomicGrowthModel::DgDxpm_numCols() const { return 6; }
  integer EconomicGrowthModel::DgDxpm_nnz()     const { return 5; }

  void
  EconomicGrowthModel::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  void
  EconomicGrowthModel::DgDxpm_sparse(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__,
  ...
  EconomicGrowthModel::u_eval_analytic(
    ...
    U__[ iU_u ] = uControl.solve(-t4 * MU__[0] + t4 * MU__[1], 0, 1);
    if ( m_debug )
