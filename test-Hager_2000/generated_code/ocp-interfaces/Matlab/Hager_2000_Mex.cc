...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"x_exact",&ProblemStorage::do_x_exact},
  {"x_exact_D",&ProblemStorage::do_x_exact_D},
  {"u_exact",&ProblemStorage::do_u_exact},
  {"u_exact_D",&ProblemStorage::do_u_exact_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "Hager_2000_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
