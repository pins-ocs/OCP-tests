...
DO_COMMAND_REMAP_TO_CLASS_METHOD(x_exact);
DO_COMMAND_REMAP_TO_CLASS_METHOD(x_exact_D);
DO_COMMAND_REMAP_TO_CLASS_METHOD(u_exact);
DO_COMMAND_REMAP_TO_CLASS_METHOD(u_exact_D);
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format( "Hager_2000_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "Hager_2000_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
