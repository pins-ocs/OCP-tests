...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libHager_2000.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libHager_2000.#{DYLIB_EXT}"
end
...
module Hager_2000
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libHager_2000.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = Hager_2000.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = Hager_2000.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = Hager_2000.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      Hager_2000.OCP_write_ocp_solution( @id, fname )
    end
