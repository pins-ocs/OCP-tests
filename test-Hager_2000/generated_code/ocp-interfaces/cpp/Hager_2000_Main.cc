...
main() {
  ...
  try {
    Hager_2000 * m_model = new Hager_2000("Hager_2000",n_threads,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/Hager_2000_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/Hager_2000_Data.lua";
    #endif
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/Hager_2000_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/Hager_2000_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
