...
namespace Hager_2000Define {
  ...
  Hager_2000::H_eval(
    ...
    real_type t4   = t3 * t3;
    real_type result__ = t2 + 2 * t4 + (t3 / 2 + t1) * MU__[0];
    if ( m_debug ) {
  ...
  integer Hager_2000::DsegmentLinkDxxp_numRows() const { return 0; }
  integer Hager_2000::DsegmentLinkDxxp_numCols() const { return 0; }
  integer Hager_2000::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  Hager_2000::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  Hager_2000::DsegmentLinkDxxp_sparse(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
