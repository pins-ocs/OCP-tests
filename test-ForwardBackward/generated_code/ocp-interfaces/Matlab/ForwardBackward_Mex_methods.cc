...
)
{
  ...
  UTILS_MEX_ASSERT0( m_gc_data.exists("Trajectory"), "missing key: ``Trajectory'' in m_gc_data\n" );
  m_trajectory.setup( m_gc_data("Trajectory") );
  ptrs[ "pTrajectory" ] = &m_trajectory;
...
)
{
  ...
  CHECK_IN_OUT( 3, 0 );
  GenericContainer & gc_mesh = m_gc_data["Trajectory"];
  GenericContainer & ptrs    = m_gc_data["Pointers"];
  Path2D * pTrajectory = ptrs("pTrajectory").get_pointer<Path2D*>();
...
)
{
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format(
        "ForwardBackward_Mex('set_guess',...), error: {}",
        exc.what()
      ).c_str()
    );
  }
  catch (...) {
    mexErrMsgTxt( "ForwardBackward_Mex('set_guess',...), Unknown error\n" );
  }
  #undef CMD
}
...
)
{
  ...
  try {
    ...
    string msg = fmt::format( "ForwardBackward_Mex('eval_F',...) error: {}", exc.what() );
    mexWarnMsgTxt( msg.c_str() );
    ok = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "ForwardBackward_Mex('eval_F',...) unkown error\n" );
    ok = false;
  }
...
)
{
  ...
  try {
    ...
    mexWarnMsgTxt( fmt::format( "ForwardBackward_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
    ok_value = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "ForwardBackward_Mex('eval_JF',...) unkown error\n" );
    ok_value = false;
  }
...
)
{
  ...
  try {
    ...
    mexWarnMsgTxt( fmt::format( "ForwardBackward_Mex('eval_JF2',...) error: {}", exc.what() ).c_str() );
    ok_value = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "ForwardBackward_Mex('eval_JF2',...) unkown error\n" );
    ok_value = false;
  }
  int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
  UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
