...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"LimitV",&ProblemStorage::do_LimitV},
  {"LimitV_D",&ProblemStorage::do_LimitV_D},
  {"LimitA",&ProblemStorage::do_LimitA},
  {"LimitA_D",&ProblemStorage::do_LimitA_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "ForwardBackward_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
