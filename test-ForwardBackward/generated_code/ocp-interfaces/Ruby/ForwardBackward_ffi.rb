...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libForwardBackward.#{DYLIB_EXT}"
...
module ForwardBackward
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libForwardBackward.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ForwardBackward.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ForwardBackward.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ForwardBackward.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ForwardBackward.OCP_write_ocp_solution( @id, fname )
    end
