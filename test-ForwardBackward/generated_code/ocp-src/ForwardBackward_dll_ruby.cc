/*-----------------------------------------------------------------------*\
 |  file: ForwardBackward_dll_ruby.cc                                    |
 |                                                                       |
...
#include "ForwardBackward_dll_ruby.hh"
#include <map>
...
namespace ForwardBackwardDefine {
  ...
  static map< string, ForwardBackward_Problem * > problems;
  ...
  */
  ...
  ForwardBackward_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, ForwardBackward_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  FORWARDBACKWARD_API_DLL
  bool
  ForwardBackward_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, ForwardBackward_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "ForwardBackward_ocp_guess" ) );
      return true;
    }
  }
  ...
  ForwardBackward_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("ForwardBackward_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  ForwardBackward_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, ForwardBackward_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("ForwardBackward_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: ForwardBackward_dll_ruby.cc
