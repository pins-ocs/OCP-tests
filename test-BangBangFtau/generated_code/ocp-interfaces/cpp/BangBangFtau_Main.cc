...
main() {
  ...
  try {
    BangBangFtau * m_model = new BangBangFtau("BangBangFtau",n_threads,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/BangBangFtau_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/BangBangFtau_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/BangBangFtau_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
