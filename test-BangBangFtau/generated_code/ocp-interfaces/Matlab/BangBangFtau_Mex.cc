...
DO_COMMAND_REMAP_TO_CLASS_METHOD(vsTBInterval);
DO_COMMAND_REMAP_TO_CLASS_METHOD(vsTBInterval_D);
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format( "BangBangFtau_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "BangBangFtau_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
