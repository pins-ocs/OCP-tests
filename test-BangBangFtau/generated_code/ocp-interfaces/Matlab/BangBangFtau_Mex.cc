...
"BangBangFtau_Mex('command', arguments ):\n"
"%===========================================================================%\n"
;

#define MODEL_NAME  "BangBangFtau"
#define MODEL_CLASS BangBangFtau
...
public:
  ...
  )
  {
    ...
    try {
      ...
      mexErrMsgTxt(
        fmt::format(
          "BangBangFtau_Mex('set_guess',...), error: {}",
          exc.what()
        ).c_str()
      );
    }
    catch (...) {
      mexErrMsgTxt( "BangBangFtau_Mex('set_guess',...), Unknown error\n" );
    }
    #undef CMD
  }
  ...
  )
  {
    ...
    try {
      ...
      string msg = fmt::format( "BangBangFtau_Mex('eval_F',...) error: {}", exc.what() );
      mexWarnMsgTxt( msg.c_str() );
      ok = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "BangBangFtau_Mex('eval_F',...) unkown error\n" );
      ok = false;
    }
  ...
  )
  {
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "BangBangFtau_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
      ok_value = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "BangBangFtau_Mex('eval_JF',...) unkown error\n" );
      ok_value = false;
    }
    int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
    UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
  ...
  do_vsTBInterval(
    ...
    #define CMD MODEL_NAME "_Mex('vsTBInterval', obj, xo___V ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
  ...
  {
    #define CMD MODEL_NAME "_Mex('vsTBInterval_D', obj, xo___V ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
    real_const_ptr arg0 = Utils::mex_matrix_pointer( arg_in_2, N0, M0, CMD " xo___V" );
  ...
  do_vsTBInterval_DD(
    ...
    #define CMD MODEL_NAME "_Mex('vsTBInterval_DD', obj, xo___V ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
    real_const_ptr arg0 = Utils::mex_matrix_pointer( arg_in_2, N0, M0, CMD " xo___V" );
...
do_vsTBInterval(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('vsTBInterval',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_vsTBInterval( nlhs, plhs, nrhs, prhs );
}
...
do_vsTBInterval_D(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('vsTBInterval_D',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_vsTBInterval_D( nlhs, plhs, nrhs, prhs );
}
...
do_vsTBInterval_DD(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('vsTBInterval_DD',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_vsTBInterval_DD( nlhs, plhs, nrhs, prhs );
}
...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"vsTBInterval",do_vsTBInterval},
  {"vsTBInterval_D",do_vsTBInterval_D},
  {"vsTBInterval_DD",do_vsTBInterval_DD},
  {"new",do_new},
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt( fmt::format( "BangBangFtau_Mex, unknown command: {}", exc.what() ).c_str() );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt( fmt::format( "BangBangFtau_Mex, error: {}", exc.what() ).c_str() );
  }
  catch (...) {
