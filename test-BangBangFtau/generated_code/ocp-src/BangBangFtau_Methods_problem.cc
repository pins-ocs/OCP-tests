...
namespace BangBangFtauDefine {
  ...
  BangBangFtau::H_eval(
    ...
    real_type result__ = (t3 + t5) * ModelPars[iM_epsiTB] + MU__[0] * X__[iX_v] + t17 * MU__[1] - 1.0 / ModelPars[iM_tauT] * (t12 - t2) * MU__[2] - 1.0 / ModelPars[iM_tauB] * (t13 - t4) * MU__[3];
    if ( m_debug ) {
  ...
  integer BangBangFtau::DsegmentLinkDxxp_numRows() const { return 0; }
  integer BangBangFtau::DsegmentLinkDxxp_numCols() const { return 0; }
  integer BangBangFtau::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  BangBangFtau::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  BangBangFtau::DsegmentLinkDxxp_sparse(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
