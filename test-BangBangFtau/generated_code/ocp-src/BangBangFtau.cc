...
namespace BangBangFtauDefine {
  ...
  */
  void
  BangBangFtau::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
  ...
  */
  void
  BangBangFtau::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "BangBangFtau::setup_parameters: Missing key `Parameters` in data\n"
    );
  ...
  void
  BangBangFtau::setup_parameters( real_type const Pars[] ) {
  ...
  */
  void
  BangBangFtau::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "BangBangFtau::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("vsTpositive"),
      "in BangBangFtau::setup_classes(gc) missing key: ``vsTpositive''\n"
    );
    vsTpositive.setup( gc("vsTpositive") );

    UTILS_ASSERT0(
      gc.exists("vsBpositive"),
      "in BangBangFtau::setup_classes(gc) missing key: ``vsBpositive''\n"
    );
    vsBpositive.setup( gc("vsBpositive") );

    UTILS_ASSERT0(
      gc.exists("vsTmax"),
      "in BangBangFtau::setup_classes(gc) missing key: ``vsTmax''\n"
    );
  ...
  */
  void
  BangBangFtau::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  BangBangFtau::setup_user_mapped_functions( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("MappedObjects"),
      "BangBangFtau::setup_classes: Missing key `MappedObjects` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("clip"),
      "in BangBangFtau::setup_user_mapped_functions(gc) missing key: ``clip''\n"
    );
    clip.setup( gc("clip") );
  }
  /* --------------------------------------------------------------------------
  ...
  */
  void
  BangBangFtau::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "BangBangFtau::setup_pointers: Missing key `Pointers` in data\n"
    );
  ...
  */
  void
  BangBangFtau::info_classes() const {
    int msg_level = 3;

    m_console->message("\nConstraints LT\n",msg_level);
    m_console->message( vsTpositive.info(),msg_level);
    m_console->message( vsBpositive.info(),msg_level);
    m_console->message( vsTmax.info(),msg_level);
    m_console->message( vsTBInterval_min.info(),msg_level);
    m_console->message( vsTBInterval_max.info(),msg_level);
  ...
  */
  ...
  BangBangFtau::setup( GenericContainer const & gc ) {
    ...
    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("BangBangFtau::setup, Debug");
