...
#include "BangBangFtau_dll_ruby.hh"
#include <map>
...
namespace BangBangFtauDefine {
  ...
  static map< string, BangBangFtau_Problem * > problems;
  ...
  */
  ...
  BangBangFtau_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, BangBangFtau_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  BANGBANGFTAU_API_DLL
  bool
  BangBangFtau_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, BangBangFtau_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "BangBangFtau_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  BangBangFtau_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("BangBangFtau_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  BangBangFtau_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, BangBangFtau_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("BangBangFtau_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
