...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"x1bound",&ProblemStorage::do_x1bound},
  {"x1bound_D",&ProblemStorage::do_x1bound_D},
  {"x2bound",&ProblemStorage::do_x2bound},
  {"x2bound_D",&ProblemStorage::do_x2bound_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "ICLOCS_StirredTank_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
