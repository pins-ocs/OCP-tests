...
DO_COMMAND_REMAP_TO_CLASS_METHOD(x1bound);
DO_COMMAND_REMAP_TO_CLASS_METHOD(x1bound_D);
DO_COMMAND_REMAP_TO_CLASS_METHOD(x2bound);
DO_COMMAND_REMAP_TO_CLASS_METHOD(x2bound_D);
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format( "ICLOCS_StirredTank_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "ICLOCS_StirredTank_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
