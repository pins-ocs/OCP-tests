#-----------------------------------------------------------------------#
#  file: ICLOCS_StirredTank_run.rb                                      #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libICLOCS_StirredTank.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libICLOCS_StirredTank.#{DYLIB_EXT}"
end
...
module ICLOCS_StirredTank
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libICLOCS_StirredTank.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ICLOCS_StirredTank.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ICLOCS_StirredTank.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ICLOCS_StirredTank.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ICLOCS_StirredTank.OCP_write_ocp_solution( @id, fname )
    end
