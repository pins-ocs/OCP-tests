...
namespace ICLOCS_StirredTankDefine {
  ...
  ICLOCS_StirredTank::Hp_eval(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t2   = X__[iX_x1];
    real_type t5   = pow(t2 - ModelPars[iM_x1_f], 2);
    real_type t6   = X__[iX_x2];
    real_type t9   = pow(t6 - ModelPars[iM_x2_f], 2);
    real_type t10  = U__[iU_u];
    real_type t13  = pow(t10 - ModelPars[iM_u_f], 2);
    real_type t17  = 1.0 / ModelPars[iM_theta];
    real_type t24  = exp(-1.0 / t6 * ModelPars[iM_En]);
    real_type t25  = t24 * t2 * ModelPars[iM_k];
    result__[ 0   ] = ModelPars[iM_w_time] + t5 + t9 + t13 + (t17 * (1 - t2) - t25) * L__[iL_lambda1__xo] + (t17 * (ModelPars[iM_Tf] - t6) + t25 - (t6 - ModelPars[iM_Tc]) * t10 * ModelPars[iM_a]) * L__[iL_lambda2__xo];
    if ( m_debug )
  ...
  integer ICLOCS_StirredTank::DHpDp_nnz()     const { return 0; }
