...
namespace AlpRiderDefine {
  ...
  */
  void
  AlpRider::updateContinuation( integer phase, real_type s ) {
    LW_ASSERT(
      s >= 0 && s <= 1,
      "AlpRider::updateContinuation( phase number = {}, s = {}) "
  ...
  */
  ...
  AlpRider::setupParameters( GenericContainer const & gc_data ) {
    ...
    LW_ASSERT0(
      allfound, "in AlpRider::setup not all parameters are set!\n"
    );
  }
  ...
  */
  void
  AlpRider::infoClasses() const {
    int msg_level = 3;
    ostringstream mstr;

    pConsole->message("\nConstraints 1D\n",msg_level);
    mstr.str(""); Ybound.info(mstr);
  ...
  */
  void
  AlpRider::setup( GenericContainer const & gc ) {
  ...
  */
  void
  AlpRider::get_names( GenericContainer & out ) const {
  ...
  */
  void
  AlpRider::diagnostic( GenericContainer const & gc ) {
  ...
  void
  AlpRider::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
