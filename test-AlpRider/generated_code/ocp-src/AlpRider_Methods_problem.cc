...
namespace AlpRiderDefine {
  ...
  AlpRider::H_eval(
    ...
    real_type result__ = (t3 + t5 + t7 + t9) * ModelPars[iM_W] + t13 / 100 + t16 / 100 + (-10 * t2 + t12 + t15) * MU__[0] + (-2 * t4 + t12 + 2 * t15) * MU__[1] + (-3 * t6 + 5 * t8 + t12 - t15) * MU__[2] + (5 * t6 - 3 * t8 + t12 + 3 * t15) * MU__[3];
    if ( m_debug ) {
  ...
  integer AlpRider::DsegmentLinkDxxp_numRows() const { return 0; }
  integer AlpRider::DsegmentLinkDxxp_numCols() const { return 0; }
  integer AlpRider::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  AlpRider::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  AlpRider::DsegmentLinkDxxp_sparse(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
