...
namespace AlpRiderDefine {
  ...
  void
  AlpRider::continuation_step_0( real_type s ) {
    real_type t2   = 1 - s;
  ...
  AlpRider::H_eval(
    ...
    real_type result__ = (t3 + t5 + t7 + t9) * ModelPars[iM_W] + t13 / 100 + t16 / 100 + (-10 * t2 + t12 + t15) * L__[iL_lambda1__xo] + (-2 * t4 + t12 + 2 * t15) * L__[iL_lambda2__xo] + (-3 * t6 + 5 * t8 + t12 - t15) * L__[iL_lambda3__xo] + (5 * t6 - 3 * t8 + t12 + 3 * t15) * L__[iL_lambda4__xo];
    if ( m_debug ) {
  ...
  integer AlpRider::DmayerDxxp_numEqns() const { return 8; }
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer AlpRider::D2mayerD2xxp_numRows() const { return 8; }
  integer AlpRider::D2mayerD2xxp_numCols() const { return 8; }
  integer AlpRider::D2mayerD2xxp_nnz()     const { return 0; }

  void
  AlpRider::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  AlpRider::D2mayerD2xxp_sparse(
    NodeType const     & LEFT__,
    NodeType const     & RIGHT__,
    P_const_pointer_type P__,
    real_type            result__[]
  ...
  integer AlpRider::DlagrangeDxpu_numEqns() const { return 6; }

  void
  AlpRider::DlagrangeDxpu_eval(
    NodeType const     & NODE__,
    U_const_pointer_type U__,
    P_const_pointer_type P__,
    real_type            result__[]
    ...
    result__[ 0   ] = 2 * X__[iX_y1] * t1;
    result__[ 1   ] = 2 * X__[iX_y2] * t1;
    result__[ 2   ] = 2 * X__[iX_y3] * t1;
    result__[ 3   ] = 2 * X__[iX_y4] * t1;
    result__[ 4   ] = U__[iU_u1] / 50;
    result__[ 5   ] = U__[iU_u2] / 50;
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "DlagrangeDxpu_eval", 6, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer AlpRider::D2lagrangeD2xpu_numRows() const { return 6; }
  integer AlpRider::D2lagrangeD2xpu_numCols() const { return 6; }
  integer AlpRider::D2lagrangeD2xpu_nnz()     const { return 6; }

  void
  AlpRider::D2lagrangeD2xpu_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  void
  AlpRider::D2lagrangeD2xpu_sparse(
    NodeType const     & NODE__,
    U_const_pointer_type U__,
    P_const_pointer_type P__,
    real_type            result__[]
    ...
    result__[ 4   ] = 1.0 / 0.50e2;
    result__[ 5   ] = 1.0 / 0.50e2;
    if ( m_debug )
  ...
  integer AlpRider::segmentLink_numEqns() const { return 0; }
  ...
  integer AlpRider::DsegmentLinkDxp_numRows() const { return 0; }
  integer AlpRider::DsegmentLinkDxp_numCols() const { return 0; }
  integer AlpRider::DsegmentLinkDxp_nnz() const { return 0; }
  ...
  integer AlpRider::jump_numEqns() const { return 8; }
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer AlpRider::DjumpDxlxlp_numRows() const { return 8; }
  integer AlpRider::DjumpDxlxlp_numCols() const { return 16; }
  integer AlpRider::DjumpDxlxlp_nnz()     const { return 16; }

  void
  AlpRider::DjumpDxlxlp_pattern( integer iIndex[], integer jIndex[] ) const {
    iIndex[0 ] = 0   ; jIndex[0 ] = 0   ;
  ...
  integer AlpRider::post_numEqns() const { return 3; }
  ...
  integer AlpRider::integrated_post_numEqns() const { return 0; }
