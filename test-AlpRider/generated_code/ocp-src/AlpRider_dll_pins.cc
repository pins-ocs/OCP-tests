...
#define MAP_PROBLEM map< string, AlpRider_Problem * >
...
namespace AlpRiderDefine {
  ...
  ALPRIDER_API_DLL
  mrb_value
  mrb_AlpRider_ocp_setup( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "AlpRider_ocp_setup, missing first parameter or is nota a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "AlpRider_ocp_setup, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      problems[id] = new AlpRider_Problem( n_threads, pConsole );
  ...
  ALPRIDER_API_DLL
  mrb_value
  mrb_AlpRider_ocp_guess( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "AlpRider_ocp_guess, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "AlpRider_ocp_guess, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "AlpRider_ocp_guess, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      Mechatronix::mrb_to_GC( mrb, data_value, gc_data, "mrb_to_GC:" );
      it->second->guess( gc_data( "Guess", "mrb_AlpRider_ocp_guess" ) );
    }
  ...
  ALPRIDER_API_DLL
  mrb_value
  mrb_AlpRider_ocp_solve( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "AlpRider_ocp_solve, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "AlpRider_ocp_solve, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "AlpRider_ocp_solve, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      if ( !ok )
        pConsole->error( "AlpRider_ocp_solve, solution failed!\n" );
  ...
  ALPRIDER_API_DLL
  mrb_value
  mrb_AlpRider_write_ocp_solution( mrb_state *mrb, mrb_value self ) {
    mrb_value a_string;
    ...
    MAP_PROBLEM::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error( "AlpRider_ocp_write_to_file, no solution found!\n" );
    } else {
  ...
  ALPRIDER_API_DLL
  mrb_value
  mrb_AlpRider_ocp_solution_range( mrb_state *mrb, mrb_value self ) {
  ...
  ALPRIDER_API_DLL
  mrb_value
  mrb_AlpRider_ocp_eval_solution( mrb_state *mrb, mrb_value self ) {
  ...
  ALPRIDER_API_DLL
  void
  mrb_libAlpRider_gem_init(mrb_state* mrb) {
    ...
    mrb_define_method(
      ...
      ocp_class,
      "write_ocp_solution",
      mrb_AlpRider_write_ocp_solution,
      MRB_ARGS_REQ(1)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "eval_solution",
      mrb_AlpRider_ocp_eval_solution,
      MRB_ARGS_REQ(2)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "solution_range",
      mrb_AlpRider_ocp_solution_range,
      MRB_ARGS_REQ(1)
    );
  ...
  ALPRIDER_API_DLL
  void
  mrb_libAlpRider_gem_final(mrb_state* mrb)
  {
  }
}

// EOF: AlpRider_dll_pins.cc
