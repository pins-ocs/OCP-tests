...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libAlpRider.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libAlpRider.#{DYLIB_EXT}"
end
...
module AlpRider
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libAlpRider.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = AlpRider.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = AlpRider.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = AlpRider.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      AlpRider.OCP_write_ocp_solution( @id, fname )
    end
