...
main() {
  ...
  try {
    ...
    SingularLuus03   model("SingularLuus03",&TP,&console);
    ...
    gc_data["DumpFile"] = "SingularLuus03_dump";
...
SingularLuus03_data.Mesh["segments"][0]["length"] = Tf;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/SingularLuus03_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/SingularLuus03_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
