...
main() {
  ...
  try {
    ...
    TwoLinkRobotArm  model("TwoLinkRobotArm",&TP,&console);
    ...
    gc_data["DumpFile"] = "TwoLinkRobotArm_dump";
    ...
    data_BoundaryConditions["final_x4"] = SET;
...
TwoLinkRobotArm_data.Mesh["segments"][0]["n"] = 400;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/TwoLinkRobotArm_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/TwoLinkRobotArm_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
