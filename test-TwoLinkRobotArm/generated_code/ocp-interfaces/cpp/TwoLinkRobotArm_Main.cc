...
main() {
  ...
  try {
    TwoLinkRobotArm * m_model = new TwoLinkRobotArm("TwoLinkRobotArm",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/TwoLinkRobotArm_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/TwoLinkRobotArm_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/TwoLinkRobotArm_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
