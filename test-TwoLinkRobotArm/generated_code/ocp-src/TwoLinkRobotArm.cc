...
namespace TwoLinkRobotArmDefine {
  ...
  */
  void
  TwoLinkRobotArm::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "TwoLinkRobotArm::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  TwoLinkRobotArm::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "TwoLinkRobotArm::setup_parameters: Missing key `Parameters` in data\n"
    );
  ...
  void
  TwoLinkRobotArm::setup_parameters( real_type const Pars[] ) {
  ...
  */
  void
  TwoLinkRobotArm::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  TwoLinkRobotArm::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  TwoLinkRobotArm::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  TwoLinkRobotArm::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "TwoLinkRobotArm::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  TwoLinkRobotArm::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "TwoLinkRobotArm::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in TwoLinkRobotArm::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  TwoLinkRobotArm::info_classes() const {
    int msg_level = 3;
    ostringstream mstr;
  ...
  */
  void
  TwoLinkRobotArm::get_names( GenericContainer & out ) const {
  ...
  void
  TwoLinkRobotArm::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
