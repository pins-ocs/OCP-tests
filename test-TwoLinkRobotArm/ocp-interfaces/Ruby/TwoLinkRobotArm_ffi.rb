...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libTwoLinkRobotArm.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libTwoLinkRobotArm.#{DYLIB_EXT}"
end
...
module TwoLinkRobotArm
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libTwoLinkRobotArm.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = TwoLinkRobotArm.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = TwoLinkRobotArm.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = TwoLinkRobotArm.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      TwoLinkRobotArm.OCP_write_ocp_solution( @id, fname )
    end
