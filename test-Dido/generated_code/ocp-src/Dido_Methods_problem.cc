...
namespace DidoDefine {
  ...
  Dido::H_eval(
    ...
    real_type t3   = cos(t2);
    real_type t8   = sin(t2);
    real_type result__ = t3 * MU__[0] - t3 * X__[iX_y] + t8 * MU__[1];
    if ( m_debug ) {
  ...
  Dido::lagrange_target(
    ...
    real_const_ptr X__ = NODE__.x;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t3   = cos(U__[iU_theta]);
    real_type result__ = -t3 * X__[iX_y];
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Dido::D2mayerD2xxp_numRows() const { return 4; }
  integer Dido::D2mayerD2xxp_numCols() const { return 4; }
  integer Dido::D2mayerD2xxp_nnz()     const { return 0; }

  void
  Dido::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  Dido::D2mayerD2xxp_sparse(
    NodeQX const & LEFT__,
    NodeQX const & RIGHT__,
    P_const_p_type P__,
    real_ptr       result__
  ...
  integer Dido::DlagrangeDxpu_numEqns() const { return 3; }

  void
  Dido::DlagrangeDxpu_eval(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
    ...
    result__[ 1   ] = -t2;
    real_type t4   = sin(t1);
    result__[ 2   ] = t4 * X__[iX_y];
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "DlagrangeDxpu_eval", 3, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Dido::D2lagrangeD2xpu_numRows() const { return 3; }
  integer Dido::D2lagrangeD2xpu_numCols() const { return 3; }
  integer Dido::D2lagrangeD2xpu_nnz()     const { return 3; }

  void
  Dido::D2lagrangeD2xpu_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  integer Dido::DsegmentLinkDxxp_numRows() const { return 0; }
  integer Dido::DsegmentLinkDxxp_numCols() const { return 0; }
  integer Dido::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  Dido::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
