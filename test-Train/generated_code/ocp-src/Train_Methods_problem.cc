...
namespace TrainDefine {
  ...
  void
  Train::continuation_step_1( real_type s ) {
    real_type t2   = 1 - s;
    real_type t6   = ModelPars[iM_epsi_min] * s + t2 * ModelPars[iM_epsi_max];
    uaControl.update_epsilon(t6);
    ubControl.update_epsilon(t6);
    real_type t11  = ModelPars[iM_tol_min] * s + t2 * ModelPars[iM_tol_max];
    uaControl.update_tolerance(t11);
    ubControl.update_tolerance(t11);
  }
  ...
  Train::H_eval(
    ...
    real_type result__ = t2 * t1 + t2 * MU__[0] + (t8 + t1 - U__[iU_ub]) * MU__[1];
    if ( m_debug ) {
  ...
  Train::lagrange_target(
    ...
    real_const_ptr X__ = NODE__.x;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type result__ = U__[iU_ua] * X__[iX_v];
    if ( m_debug ) {
  ...
  integer Train::DmayerDxxp_numEqns() const { return 4; }

  void
  Train::DmayerDxxp_eval(
    NodeQX const & LEFT__,
    NodeQX const & RIGHT__,
    P_const_p_type P__,
    real_ptr       result__
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Train::D2mayerD2xxp_numRows() const { return 4; }
  integer Train::D2mayerD2xxp_numCols() const { return 4; }
  integer Train::D2mayerD2xxp_nnz()     const { return 0; }

  void
  Train::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  Train::D2mayerD2xxp_sparse(
    NodeQX const & LEFT__,
    NodeQX const & RIGHT__,
    P_const_p_type P__,
    real_ptr       result__
  ...
  integer Train::DlagrangeDxpu_numEqns() const { return 4; }

  void
  Train::DlagrangeDxpu_eval(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Train::D2lagrangeD2xpu_numRows() const { return 4; }
  integer Train::D2lagrangeD2xpu_numCols() const { return 4; }
  integer Train::D2lagrangeD2xpu_nnz()     const { return 2; }

  void
  Train::D2lagrangeD2xpu_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  integer Train::segmentLink_numEqns() const { return 0; }

  void
  Train::segmentLink_eval(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
  ...
  integer Train::DsegmentLinkDxxp_numRows() const { return 0; }
  integer Train::DsegmentLinkDxxp_numCols() const { return 0; }
  integer Train::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  Train::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  integer Train::jump_numEqns() const { return 4; }
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Train::DjumpDxlxlp_numRows() const { return 4; }
  integer Train::DjumpDxlxlp_numCols() const { return 8; }
  integer Train::DjumpDxlxlp_nnz()     const { return 8; }

  void
  Train::DjumpDxlxlp_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  integer Train::post_numEqns() const { return 3; }
  ...
  integer Train::integrated_post_numEqns() const { return 0; }

  void
  Train::integrated_post_eval(
    NodeQXL const & NODE__,
    P_const_p_type  P__,
    U_const_p_type  U__,
    real_ptr        result__
