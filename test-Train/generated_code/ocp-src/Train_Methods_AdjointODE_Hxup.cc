...
namespace TrainDefine {
  ...
  Train::Hxp_eval(
    ...
    result__[ 1   ] = t7 * t1 + MU__[0] + U__[iU_ua];
    if ( m_debug )
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Train::DHxpDxpuv_numRows() const { return 2; }
  integer Train::DHxpDxpuv_numCols() const { return 6; }
  integer Train::DHxpDxpuv_nnz()     const { return 5; }

  void
  Train::DHxpDxpuv_pattern( integer iIndex[], integer jIndex[] ) const {
