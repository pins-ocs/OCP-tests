...
namespace TrainDefine {
  ...
  void
  Train::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  Train::xlambda_guess_eval(
    integer              i_segment,
    ...
    X__[ iX_x ] = t1 + 0.2083333333e0 * Q__[iQ_zeta] * (ModelPars[iM_x_f] - t1);
    X__[ iX_v ] = 1;
  ...
  bool
  Train::p_check( P_const_pointer_type P__ ) const {
    return true;
  }
  ...
  integer Train::u_guess_numEqns() const { return 2; }
  ...
  Train::u_guess_eval(
    ...
    UGUESS__[ iU_ua ] = ModelPars[iM_uaMax] / 2;
    UGUESS__[ iU_ub ] = ModelPars[iM_ubMax] / 2;
    if ( m_debug )
  ...
  Train::u_check_if_admissible(
    ...
    ok = ok && uaControl.check_range(U__[iU_ua], 0, ModelPars[iM_uaMax]);
    ok = ok && ubControl.check_range(U__[iU_ub], 0, ModelPars[iM_ubMax]);
    return ok;
  }

}

// EOF: Train_Methods_Guess.cc
