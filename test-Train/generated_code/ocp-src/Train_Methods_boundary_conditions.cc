...
namespace TrainDefine {
  ...
  integer Train::boundaryConditions_numEqns() const { return 4; }

  void
  Train::boundaryConditions_eval(
    ...
    result__[ 3   ] = XR__[iX_v] - ModelPars[iM_v_f];
    if ( m_debug )
      Mechatronix::check_in_segment2( result__, "boundaryConditions_eval", 4, i_segment_left, i_segment_right );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Train::DbcDxxp_numRows() const { return 4; }
  integer Train::DbcDxxp_numCols() const { return 4; }
  integer Train::DbcDxxp_nnz()     const { return 4; }

  void
  Train::DbcDxxp_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Train::D2bcD2xxp_numRows() const { return 4; }
  integer Train::D2bcD2xxp_numCols() const { return 4; }
  integer Train::D2bcD2xxp_nnz()     const { return 0; }

  void
  Train::D2bcD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  Train::D2bcD2xxp_sparse(
    NodeType const         & LEFT__,
    NodeType const         & RIGHT__,
    P_const_pointer_type     P__,
  ...
  integer Train::adjointBC_numEqns() const { return 4; }

  void
  Train::adjointBC_eval(
    NodeType const              & LEFT__,
    NodeType const              & RIGHT__,
    P_const_pointer_type          P__,
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Train::DadjointBCDxxp_numRows() const { return 4; }
  integer Train::DadjointBCDxxp_numCols() const { return 4; }
  integer Train::DadjointBCDxxp_nnz()     const { return 0; }

  void
  Train::DadjointBCDxxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  Train::DadjointBCDxxp_sparse(
    NodeType const              & LEFT__,
    NodeType const              & RIGHT__,
    P_const_pointer_type          P__,
    OMEGA_full_const_pointer_type OMEGA__,
