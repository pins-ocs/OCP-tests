...
namespace TrainDefine {
  ...
  Train::g_fun_eval(
    ...
    real_type t5   = uaControl(t1, 0, ModelPars[iM_uaMax]);
    real_type t6   = U__[iU_ub];
    real_type t8   = ubControl(t6, 0, ModelPars[iM_ubMax]);
    real_type t13  = acc(X__[iX_x], t2);
    real_type result__ = t2 * t1 + t5 + t8 + t2 * MU__[0] + (t13 + t1 - t6) * MU__[1];
    if ( m_debug ) {
  ...
  Train::g_eval(
    ...
    real_const_ptr X__ = NODE__.x;
    MeshStd::SegmentClass const & segment = m_pMesh->get_segment_by_index(i_segment);
    real_type t4   = ALIAS_uaControl_D_1(U__[iU_ua], 0, ModelPars[iM_uaMax]);
    real_type t5   = MU__[1];
    result__[ 0   ] = X__[iX_v] + t4 + t5;
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Train::DgDxpm_numRows() const { return 2; }
  integer Train::DgDxpm_numCols() const { return 4; }
  integer Train::DgDxpm_nnz()     const { return 3; }

  void
  Train::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  Train::u_eval_analytic(
    ...
    MeshStd::SegmentClass const & segment = m_pMesh->get_segment_by_index(i_segment);
    real_type t2   = MU__[1];
    U__[ iU_ua ] = ubControl.solve(-X__[iX_v] - t2, 0, ModelPars[iM_uaMax]);
