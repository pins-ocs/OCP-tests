...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"acc",&ProblemStorage::do_acc},
  {"acc_D_1",&ProblemStorage::do_acc_D_1},
  {"acc_D_2",&ProblemStorage::do_acc_D_2},
  {"acc_D_1_1",&ProblemStorage::do_acc_D_1_1},
  {"acc_D_1_2",&ProblemStorage::do_acc_D_1_2},
  {"acc_D_2_2",&ProblemStorage::do_acc_D_2_2},
  {"h",&ProblemStorage::do_h},
  {"h_D",&ProblemStorage::do_h_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "Train_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
