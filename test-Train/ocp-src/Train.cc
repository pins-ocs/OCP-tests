...
namespace TrainDefine {
  ...
  */
  void
  Train::updateContinuation( integer phase, real_type s ) {
    LW_ASSERT(
      s >= 0 && s <= 1,
      "Train::updateContinuation( phase number = {}, s = {}) "
  ...
  */
  ...
  Train::setupParameters( GenericContainer const & gc_data ) {
    ...
    LW_ASSERT0(
      allfound, "in Train::setup not all parameters are set!\n"
    );
  }
  ...
  */
  ...
  Train::infoClasses() const {
    ...
    pConsole->message("\nControls\n",msg_level);
    mstr.str(""); uaControl.info(mstr);
    pConsole->message(mstr.str(),msg_level);
    mstr.str(""); ubControl.info(mstr);
    pConsole->message(mstr.str(),msg_level);
  ...
  */
  void
  Train::setup( GenericContainer const & gc ) {
  ...
  */
  void
  Train::get_names( GenericContainer & out ) const {
  ...
  */
  void
  Train::diagnostic( GenericContainer const & gc ) {
  ...
  void
  Train::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
