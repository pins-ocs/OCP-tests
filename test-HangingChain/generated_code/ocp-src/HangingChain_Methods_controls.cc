...
namespace HangingChainDefine {
  ...
  HangingChain::g_eval(
    ...
    result__[ 0   ] = 1.0 / t5 * (t5 * LM__[0] + (XM__[0] + LM__[1]) * t2);
    if ( m_debug )
  ...
  HangingChain::DgDu_sparse(
    ...
    result__[ 0   ] = t6 * (t2 * t6 * t1 + t10 + t9) - t2 / t5 / t4 * (t5 * t1 + (t9 + t10) * t2);
    if ( m_debug )
  ...
  void
  HangingChain::DuDxlxlp_full_analytic(
    NodeType2 const &          LEFT__,
    NodeType2 const &          RIGHT__,
    P_const_pointer_type       P__,
    U_const_pointer_type       UM__,
  ...
  HangingChain::DmDu_eval(
    ...
    result__[ 0   ] = -2 * V__[0] + 2 * t2 - 2 * t2 / t6 * (V__[1] - t6);
    if ( m_debug )
  ...
  HangingChain::DmDuu_sparse(
    ...
    real_type t3   = t2 + 1;
    real_type t7   = sqrt(t3);
    real_type t8   = V__[1] - t7;
    result__[ 0   ] = 2 + 2 * t2 / t3 + 2 * t2 / t7 / t3 * t8 - 2 / t7 * t8;
    if ( m_debug )
