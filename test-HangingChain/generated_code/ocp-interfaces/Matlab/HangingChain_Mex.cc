...
DO_COMMAND_REMAP_TO_CLASS_METHOD(x_guess);
DO_COMMAND_REMAP_TO_CLASS_METHOD(x_guess_D);
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format( "HangingChain_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "HangingChain_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
