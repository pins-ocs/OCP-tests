...
namespace HangingChainDefine {
  ...
  void
  HangingChain::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  HangingChain::xlambda_guess_eval(
    integer              i_segment,
    ...
    real_type t2   = Q__[0];
    X__[ iX_x ] = t1 + (ModelPars[4] - t1) * t2;
    X__[ iX_z ] = ModelPars[0] * t2;
  ...
  bool
  HangingChain::p_check( P_const_pointer_type P__ ) const {
    bool ok = true;
  ...
  bool
  HangingChain::xlambda_check_node(
    integer              ipos,
  ...
  bool
  HangingChain::xlambda_check_cell(
    integer              icell,
    NodeType2 const    & NODE__,
    P_const_pointer_type P__
  ) const {
