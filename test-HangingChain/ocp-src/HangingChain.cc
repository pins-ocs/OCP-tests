...
namespace HangingChainDefine {
  ...
  */
  void
  HangingChain::updateContinuation( integer phase, real_type s ) {
    UTILS_ASSERT(
      s >= 0 && s <= 1,
      "HangingChain::updateContinuation( phase number = {}, s = {}) "
  ...
  */
  void
  HangingChain::setupParameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "HangingChain::setupParameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in HangingChain::setup not all parameters are set!\n"
    );
  }

  void
  HangingChain::setupParameters( real_type const Pars[] ) {
  ...
  */
  void
  HangingChain::setupClasses( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  HangingChain::setupUserClasses( GenericContainer const & gc ) {
  }
  ...
  */
  void
  HangingChain::setupUserMappedFunctions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  HangingChain::setupPointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "HangingChain::setupPointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in HangingChain::setupPointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  HangingChain::infoClasses() const {
    int msg_level = 3;
    ostringstream mstr;
  ...
  */
  void
  HangingChain::setup( GenericContainer const & gc ) {

    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("HangingChain::setup, Debug");
  ...
  */
  void
  HangingChain::get_names( GenericContainer & out ) const {
  ...
  */
  void
  HangingChain::diagnostic( GenericContainer const & gc ) {
  ...
  void
  HangingChain::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
