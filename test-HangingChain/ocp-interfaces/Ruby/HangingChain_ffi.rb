...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libHangingChain.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libHangingChain.#{DYLIB_EXT}"
end
...
module HangingChain
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libHangingChain.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = HangingChain.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = HangingChain.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = HangingChain.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      HangingChain.OCP_write_ocp_solution( @id, fname )
    end
