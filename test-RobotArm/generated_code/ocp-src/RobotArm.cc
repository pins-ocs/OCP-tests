...
namespace RobotArmDefine {
  ...
  */
  ...
  RobotArm::update_continuation(
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "RobotArm::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  RobotArm::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "RobotArm::setup_parameters: Missing key `Parameters` in data\n"
    );
  ...
  void
  RobotArm::setup_parameters( real_type const Pars[] ) {
  ...
  */
  void
  RobotArm::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  RobotArm::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  RobotArm::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  RobotArm::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "RobotArm::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  RobotArm::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "RobotArm::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in RobotArm::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  RobotArm::info_classes() const {
    int msg_level = 3;

    m_console->message("\nControls\n",msg_level);
    m_console->message( u_rhoControl.info(),msg_level);
    m_console->message( u_thetaControl.info(),msg_level);
    m_console->message( u_phiControl.info(),msg_level);
