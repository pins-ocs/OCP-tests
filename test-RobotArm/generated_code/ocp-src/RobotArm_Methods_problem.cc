...
namespace RobotArmDefine {
  ...
  RobotArm::H_eval(
    ...
    real_type t3   = U__[iU_u_rho];
    real_type t6   = U__[iU_u_theta];
    real_type t9   = U__[iU_u_phi];
    real_type t20  = u_rhoControl(t3, -1, 1);
    real_type t21  = u_thetaControl(t6, -1, 1);
    real_type t22  = u_phiControl(t9, -1, 1);
    real_type result__ = (t3 * L__[iL_lambda1__xo] + t6 * L__[iL_lambda2__xo] + t9 * L__[iL_lambda3__xo] + L__[iL_lambda4__xo] * X__[iX_rho1] + L__[iL_lambda5__xo] * X__[iX_theta1] + L__[iL_lambda6__xo] * X__[iX_phi1] + t20 + t21 + t22) * P__[iP_T];
    return result__;
  }
#else
  real_type
  ...
  RobotArm::control_penalties_eval(
    ...
    real_type result__ = (t3 + t5 + t7) * P__[iP_T];
    if ( m_debug ) {
  ...
  integer
  RobotArm::DmayerDx_numEqns() const
  { return 12; }

  void
  RobotArm::DmayerDx_eval(
    NodeType const     & LEFT__,
    NodeType const     & RIGHT__,
    P_const_pointer_type P__,
    real_ptr             result__
  ...
  integer
  RobotArm::DmayerDp_numEqns() const
  { return 1; }

  void
  RobotArm::DmayerDp_eval(
    NodeType const     & LEFT__,
    NodeType const     & RIGHT__,
    P_const_pointer_type P__,
    real_ptr             result__
  ...
  RobotArm::jump_eval(
    ...
    result__[ 9   ] = -ModelPars[iM_L] * (LL__[iL_lambda1__xo] - LR__[iL_lambda1__xo]);
