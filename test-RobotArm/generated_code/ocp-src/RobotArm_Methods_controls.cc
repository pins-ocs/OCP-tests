...
namespace RobotArmDefine {
  ...
  RobotArm::g_fun_eval(
    ...
    real_type t6   = UM__[2];
    real_type t9   = UM__[0];
    real_type t20  = u_rhoControl(t9, -1, 1);
    real_type t21  = u_thetaControl(t3, -1, 1);
    real_type t22  = u_phiControl(t6, -1, 1);
    real_type result__ = (t3 * LM__[1] + t6 * LM__[2] + t9 * LM__[0] + LM__[3] * XM__[3] + LM__[4] * XM__[4] + LM__[5] * XM__[5] + t20 + t21 + t22) * P__[iP_T];
    if ( m_debug ) {
  ...
  RobotArm::DgDxlxlp_pattern(
    ...
    iIndex[4 ] = 1   ; jIndex[4 ] = 19  ;
    iIndex[5 ] = 1   ; jIndex[5 ] = 24  ;
    iIndex[6 ] = 2   ; jIndex[6 ] = 8   ;
  ...
  RobotArm::DgDxlxlp_sparse(
    ...
    real_type t4   = ALIAS_u_rhoControl_D_1(UM__[0], -1, 1);
    result__[ 2   ] = LM__[0] + t4;
    result__[ 3   ] = result__[1];
    result__[ 4   ] = result__[3];
    real_type t7   = ALIAS_u_thetaControl_D_1(UM__[1], -1, 1);
    result__[ 5   ] = LM__[1] + t7;
    result__[ 6   ] = result__[4];
    result__[ 7   ] = result__[6];
    real_type t10  = ALIAS_u_phiControl_D_1(UM__[2], -1, 1);
    result__[ 8   ] = LM__[2] + t10;
    if ( m_debug )
  ...
  RobotArm::DgDu_sparse(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t1   = P__[iP_T];
    real_type t3   = ALIAS_u_rhoControl_D_1_1(UM__[0], -1, 1);
    result__[ 0   ] = t3 * t1;
    real_type t5   = ALIAS_u_thetaControl_D_1_1(UM__[1], -1, 1);
    result__[ 1   ] = t5 * t1;
    real_type t7   = ALIAS_u_phiControl_D_1_1(UM__[2], -1, 1);
    result__[ 2   ] = t7 * t1;
    if ( m_debug )
  ...
  RobotArm::u_eval_analytic(
    ...
    integer i_segment = LEFT__.i_segment;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    U__[ iU_u_rho   ] = u_phiControl.solve(-LM__[0], -1, 1);
    U__[ iU_u_theta ] = u_phiControl.solve(-LM__[1], -1, 1);
    U__[ iU_u_phi   ] = u_phiControl.solve(-LM__[2], -1, 1);
    if ( m_debug )
  ...
  RobotArm::DuDxlxlp_full_analytic(
    ...
    real_type t2   = u_phiControl.solve_rhs(-LM__[0], -1, 1);
    real_type tmp_0_6 = -0.5e0 * t2;
    real_type tmp_1_6 = 0.0e0;
    real_type tmp_2_6 = 0.0e0;
    real_type tmp_0_7 = 0.0e0;
    real_type t5   = u_phiControl.solve_rhs(-LM__[1], -1, 1);
    real_type tmp_1_7 = -0.5e0 * t5;
    real_type tmp_2_7 = 0.0e0;
    real_type tmp_0_8 = 0.0e0;
    real_type tmp_1_8 = 0.0e0;
    real_type t8   = u_phiControl.solve_rhs(-LM__[2], -1, 1);
    real_type tmp_2_8 = -0.5e0 * t8;
