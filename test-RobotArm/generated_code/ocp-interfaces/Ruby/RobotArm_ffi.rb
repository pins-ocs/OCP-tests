...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libRobotArm.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libRobotArm.#{DYLIB_EXT}"
end
...
module RobotArm
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libRobotArm.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = RobotArm.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = RobotArm.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = RobotArm.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      RobotArm.OCP_write_ocp_solution( @id, fname )
    end
