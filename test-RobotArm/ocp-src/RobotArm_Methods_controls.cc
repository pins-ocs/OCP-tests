...
namespace RobotArmDefine {
  ...
  RobotArm::g_eval(
    ...
    real_type t3   = ALIAS_u_rhoControl_D_1(UM__[0], -1, 1);
    result__[ 0   ] = t3 * t1 + t1 * LM__[0];
    real_type t8   = ALIAS_u_thetaControl_D_1(UM__[1], -1, 1);
    result__[ 1   ] = t8 * t1 + t1 * LM__[1];
    real_type t13  = ALIAS_u_phiControl_D_1(UM__[2], -1, 1);
    result__[ 2   ] = t13 * t1 + t1 * LM__[2];
    if ( m_debug )
  ...
  RobotArm::DgDxlxlp_sparse(
    ...
    real_type t3   = ALIAS_u_rhoControl_D_1(UM__[0], -1, 1);
    result__[ 2   ] = t3 + LM__[0];
    result__[ 3   ] = result__[1];
    result__[ 4   ] = result__[3];
    real_type t6   = ALIAS_u_thetaControl_D_1(UM__[1], -1, 1);
    result__[ 5   ] = t6 + LM__[1];
    result__[ 6   ] = result__[4];
    result__[ 7   ] = result__[6];
    real_type t9   = ALIAS_u_phiControl_D_1(UM__[2], -1, 1);
    result__[ 8   ] = t9 + LM__[2];
    if ( m_debug )
  ...
  RobotArm::u_eval_analytic(
    ...
    integer i_segment = LEFT__.i_segment;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    U__[ iU_u_rho   ] = u_rhoControl.solve(-LM__[0], -1, 1);
    U__[ iU_u_theta ] = u_thetaControl.solve(-LM__[1], -1, 1);
