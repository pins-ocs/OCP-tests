...
main() {
  ...
  try {
    AliChan * m_model = new AliChan("AliChan",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/AliChan_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/AliChan_Data.lua";
    #endif
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/AliChan_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/AliChan_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
