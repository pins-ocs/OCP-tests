...
namespace PathConstrainedDefine {
  ...
  mrb_PathConstrained_ocp_setup( mrb_state *mrb, mrb_value self ) {
    ...
    if ( gc_data.exists("InfoLevel") )
      infoLevel = gc_data("InfoLevel").get_as_int("mrb_PathConstrained_ocp_setup::InfoLevel ");
  ...
  mrb_PathConstrained_ocp_solve(mrb_state *mrb, mrb_value self) {
    ...
    if ( it == problems.end() ) {
      ...
      if ( !ok )
        cerr << "PathConstrained_ocp_solve, solution failed!\n";
  ...
  mrb_PathConstrained_write_ocp_solution( mrb_state *mrb, mrb_value self ) {
    ...
    MAP_PROBLEM::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      cerr << "PathConstrained_ocp_write_to_file, no solution found!\n";
    } else {
