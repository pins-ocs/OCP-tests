...
namespace PathConstrainedDefine {
  ...
  */
  void
  PathConstrained::updateContinuation( indexType phase, valueType s ) {
    ASSERT( s >= 0 && s <= 1.0,
            "PathConstrained::updateContinuation( phase = " << phase <<
  ...
  */
  void
  PathConstrained::setupParameters( GenericContainer const & gc ) {
  }
  ...
  */
  void
  PathConstrained::setupClasses( GenericContainer const & gc ) {

    // Initialize Constraints 1D
    ASSERT( gc.exists("x2bound"), "in PathConstrained::setupClasses(gc) missing key: ``x2bound''" );
  ...
  */
  void
  PathConstrained::setupUserMappedFunctions( GenericContainer const & gc ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  PathConstrained::setupControls( GenericContainer const & gc ) {
    // initialize Controls
  ...
  */
  ...
  PathConstrained::setupPointers( GenericContainer const & gc_data ) {
    ...
    ASSERT( gc_data.exists("Pointers"), "PathConstrained::setup: Missing key `Pointers` in data" );
    GenericContainer const & gc = gc_data("Pointers");

    // Initialize user classes

    ASSERT( gc.exists("pMesh"), "in PathConstrained::setupPointers(gc) cant find key `pMesh' in gc" );
  ...
  */
  void
  PathConstrained::setupParameters( valueConstPointer Pars ) {
  }
  ...
  */
  void
  PathConstrained::infoClasses( basic_ostream<char> & stream ) const {
  ...
  */
  ...
  PathConstrained::setup( GenericContainer const & gc ) {
    ...
    this->setupParameters( gc("Parameters", "PathConstrained::setup: Missing key `Parameters` in data" ) );
    this->setupClasses( gc("Constraints", "PathConstrained::setup: Missing key `Constraints` in data" ) );
    // this->setupUserMappedFunctions( gc("MappedObjects","PathConstrained::setup: Missing key `MappedObjects` in data") ); // no mapped objects
    this->setupControls( gc("Controls", "PathConstrained::setup: Missing key `Controls` in data" ) );
    this->setupUserClasses( gc );
    this->setupPointers( gc );
    this->setupBC( gc("BoundaryConditions", "PathConstrained::setup: Missing key `BoundaryConditions` in data") );

    // setup model
    if ( gc.exists("InfoLevel") ) this->changeInfoLevel(gc("InfoLevel").get_as_int("PathConstrained::setup, InfoLevel "));
    ...
    if ( gc.exists("ControlSolutionMethod") ) {
      this->setControlSolutionMethod(gc("ControlSolutionMethod").get_as_int("PathConstrained::setup, ControlSolutionMethod "));
    }
    if ( gc.exists("ControlSolutionMaxIter") ) {
      this->setControlSolutionMaxIter(gc("ControlSolutionMaxIter").get_as_int("PathConstrained::setup, ControlSolutionMaxIter "));
    }
    ...
    solver.setSolverParameters( gc("Solver", "in PathConstrained::solve") );
    solver.allocate();
  ...
  */
  void
  PathConstrained::guess( GenericContainer const & gc ) {
    ...
    if ( gc.exists("Guess") ) {
      ...
      if ( gc.exists("ParsGuess")  ) {
        ...
        ASSERT( indexType(v_pars.size()) == dim_p,
                "PathConstrained::set_guess, ParsGuess size = " << v_pars.size() <<
      ...
      if ( gc.exists("OmegaGuess")  ) {
        ...
        ASSERT( indexType(o_pars.size()) == this->numOMEGA(),
                "PathConstrained::set_guess, OmegaGuess size = " << o_pars.size() <<
  ...
  PathConstrained::solve() {
    ...
    try {
      ...
      try {
        ...
        lastError = "PathConstrained::solver, fatal: unknown error";
        if ( Indirect_OCP::infoLevel >= 1 )
          (*Indirect_OCP::pCout)
            << "\n\n\nERROR: " << lastError << '\n';
      }
  ...
  */
  void
  PathConstrained::get_solution_base( GenericContainer & gc_solution ) {
    ...
    gc_solution["model_name"]          = "PathConstrained";
  ...
  void
  PathConstrained::get_solution( GenericContainer & gc_solution ) {
