...
#include "ICLOCS2_BatchFermentor_Mex.hh"
...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"mu",&ProblemStorage::do_mu},
  {"mu_D_1",&ProblemStorage::do_mu_D_1},
  {"mu_D_2",&ProblemStorage::do_mu_D_2},
  {"mu_D_1_1",&ProblemStorage::do_mu_D_1_1},
  {"mu_D_1_2",&ProblemStorage::do_mu_D_1_2},
  {"mu_D_2_2",&ProblemStorage::do_mu_D_2_2},
  {"rho",&ProblemStorage::do_rho},
  {"rho_D",&ProblemStorage::do_rho_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "ICLOCS2_BatchFermentor_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
    ...
    mexErrMsgTxt(
      fmt::format( "ICLOCS2_BatchFermentor_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "ICLOCS2_BatchFermentor_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
