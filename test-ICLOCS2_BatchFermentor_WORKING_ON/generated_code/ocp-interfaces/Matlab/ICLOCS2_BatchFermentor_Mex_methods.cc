...
)
{
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format(
        "ICLOCS2_BatchFermentor_Mex('set_guess',...), error: {}",
        exc.what()
      ).c_str()
    );
  }
  catch (...) {
    mexErrMsgTxt( "ICLOCS2_BatchFermentor_Mex('set_guess',...), Unknown error\n" );
  }
  #undef CMD
}
...
)
{
  ...
  try {
    ...
    string msg = fmt::format( "ICLOCS2_BatchFermentor_Mex('eval_F',...) error: {}", exc.what() );
...
// EOF: ICLOCS2_BatchFermentor_Mex.cc
