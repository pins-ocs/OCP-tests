#-----------------------------------------------------------------------#
#  file: ICLOCS2_BatchFermentor_run.rb                                  #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libICLOCS2_BatchFermentor.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libICLOCS2_BatchFermentor.#{DYLIB_EXT}"
end
...
module ICLOCS2_BatchFermentor
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libICLOCS2_BatchFermentor.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ICLOCS2_BatchFermentor.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ICLOCS2_BatchFermentor.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ICLOCS2_BatchFermentor.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ICLOCS2_BatchFermentor.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: ICLOCS2_BatchFermentor_ffi.rb
