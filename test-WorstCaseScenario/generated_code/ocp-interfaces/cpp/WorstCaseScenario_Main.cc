...
main() {
  ...
  try {
    WorstCaseScenario * m_model = new WorstCaseScenario("WorstCaseScenario",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/WorstCaseScenario_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/WorstCaseScenario_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/WorstCaseScenario_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
