...
namespace WorstCaseScenarioDefine {
  ...
  bool
  WorstCaseScenario::penalties_check_cell(
  ...
  WorstCaseScenario::H_eval(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type result__ = L__[iL_lambda1__xo] * (1 - 2 * Q__[iQ_zeta]) * U__[iU_u];
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer WorstCaseScenario::D2lagrangeD2xup_numRows() const { return 2; }
  integer WorstCaseScenario::D2lagrangeD2xup_numCols() const { return 2; }
  integer WorstCaseScenario::D2lagrangeD2xup_nnz()     const { return 0; }

  void
  WorstCaseScenario::D2lagrangeD2xup_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  WorstCaseScenario::D2lagrangeD2xup_sparse(
    NodeType const     & NODE__,
    U_const_pointer_type U__,
    P_const_pointer_type P__,
    real_type            result__[]
  ...
  void
  WorstCaseScenario::DsegmentLinkDxp_sparse(
    NodeType const     & L,
    NodeType const     & R,
    P_const_pointer_type p,
