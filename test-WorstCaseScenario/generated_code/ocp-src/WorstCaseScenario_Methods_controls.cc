...
namespace WorstCaseScenarioDefine {
  ...
  WorstCaseScenario::g_fun_eval(
    ...
    real_type result__ = t2 + t1 * (1 - 2 * Q__[iQ_zeta]) * MU__[0];
    if ( m_debug ) {
  ...
  WorstCaseScenario::g_eval(
    ...
    result__[ 0   ] = t2 + MU__[0] * (1 - 2 * Q__[iQ_zeta]);
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 1, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer WorstCaseScenario::DgDxpm_numRows() const { return 1; }
  integer WorstCaseScenario::DgDxpm_numCols() const { return 2; }
  integer WorstCaseScenario::DgDxpm_nnz()     const { return 1; }

  void
  WorstCaseScenario::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  void
  WorstCaseScenario::DgDxpm_sparse(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__,
  ...
  WorstCaseScenario::u_eval_analytic(
    ...
    integer i_segment = NODE__.i_segment;
    MeshStd::SegmentClass const & segment = m_pMesh->get_segment_by_index(i_segment);
    U__[ iU_u ] = uControl.solve(MU__[0] * (-1 + 2 * Q__[iQ_zeta]), 0, 1);
    if ( m_debug )
