...
namespace WorstCaseScenarioDefine {
  ...
  void
  WorstCaseScenario::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  WorstCaseScenario::xlambda_guess_eval(
    integer              i_segment,
  ...
  bool
  WorstCaseScenario::p_check( P_const_pointer_type P__ ) const {
    return true;
  }
  ...
  bool
  WorstCaseScenario::xlambda_check_node(
    integer              ipos,
  ...
  bool
  WorstCaseScenario::xlambda_check_cell(
    integer              icell,
  ...
  integer WorstCaseScenario::u_guess_numEqns() const { return 1; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  WorstCaseScenario::u_guess_eval(
    NodeType2 const    & LEFT__,
    NodeType2 const    & RIGHT__,
    P_const_pointer_type P__,
    ...
    UGUESS__[ iU_u ] = uControl.solve(QL__[iQ_zeta] * t1 + QR__[iQ_zeta] * t1 - t1, 0, 1);
    if ( m_debug )
  ...
  bool
  WorstCaseScenario::u_check_if_admissible(
    NodeType2 const    & NODE__,
