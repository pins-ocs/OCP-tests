...
namespace WorstCaseScenarioDefine {
  ...
  WorstCaseScenario::g_eval(
    ...
    real_type t7   = ALIAS_uControl_D_1(U__[0], 0, 1);
    result__[ 0   ] = L__[0] * (1 - 2 * Q__[0]) + t7;
  ...
  WorstCaseScenario::u_eval_analytic(
    ...
    real_type const * L__ = NODE__.lambda;
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    U__[ iU_u ] = uControl.solve(L__[0] * (-1 + 2 * Q__[0]), 0, 1);
  }
  ...
  WorstCaseScenario::DuDxlp_full_analytic(
    ...
    real_type const * L__ = NODE__.lambda;
    DuDxlp(0, 0) = 0;
    DuDxlp(0, 1) = uControl.solve_rhs(L__[0] * (-1 + 2 * Q__[0]), 0, 1) * (-1 + 2 * Q__[0]);
  }
  ...
  bool
  WorstCaseScenario::DuDxlp_full_numeric(
    NodeType2 const          & NODE__,
    P_const_pointer_type       P__,
    U_const_pointer_type       U__,
    MatrixWrapper<real_type> & DuDxlp
