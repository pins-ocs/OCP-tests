#-----------------------------------------------------------------------#
#  file: ICLOCS_ContinuousMP_run.rb                                     #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libICLOCS_ContinuousMP.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libICLOCS_ContinuousMP.#{DYLIB_EXT}"
end
...
module ICLOCS_ContinuousMP
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libICLOCS_ContinuousMP.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ICLOCS_ContinuousMP.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ICLOCS_ContinuousMP.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ICLOCS_ContinuousMP.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ICLOCS_ContinuousMP.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: ICLOCS_ContinuousMP_ffi.rb
