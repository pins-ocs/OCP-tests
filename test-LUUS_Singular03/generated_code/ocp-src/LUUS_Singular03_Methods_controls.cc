...
namespace LUUS_Singular03Define {
  ...
  LUUS_Singular03::g_fun_eval(
    ...
    real_type t5   = LM__[0];
    real_type t8   = UM__[0];
    real_type t12  = ModelPars[iM_epsi_x] * ModelPars[iM_epsi_x];
    real_type t14  = uControl(t8, -1, 1);
    real_type t17  = XR__[iX_x1] * XR__[iX_x1];
    real_type t18  = XR__[iX_x2];
    real_type t19  = t18 * t18;
    real_type result__ = t2 + t4 + t3 * t5 + 2 * t8 * LM__[1] + t14 * (t2 + t4 + t12) + t17 + t19 + t18 * t5 + t14 * (t17 + t19 + t12);
    if ( m_debug ) {
  ...
  LUUS_Singular03::g_eval(
    ...
    real_type t6   = XL__[iX_x2] * XL__[iX_x2];
    real_type t8   = ModelPars[iM_epsi_x] * ModelPars[iM_epsi_x];
    real_type t11  = ALIAS_uControl_D_1(UM__[0], -1, 1);
    real_type t14  = XR__[iX_x1] * XR__[iX_x1];
    real_type t16  = XR__[iX_x2] * XR__[iX_x2];
    result__[ 0   ] = 2 * LM__[1] + t11 * (t4 + t6 + t8) + t11 * (t14 + t16 + t8);
    if ( m_debug )
  ...
  void
  LUUS_Singular03::DgDxlxlp_sparse(
    NodeType2 const &    LEFT__,
    NodeType2 const &    RIGHT__,
    U_const_pointer_type UM__,
    P_const_pointer_type P__,
    ...
    result__[ 1   ] = 2 * t3 * XL__[iX_x2];
    result__[ 2   ] = 1.0;
    result__[ 3   ] = 2 * t3 * XR__[iX_x1];
    result__[ 4   ] = 2 * t3 * XR__[iX_x2];
    result__[ 5   ] = 1.0;
    if ( m_debug )
  ...
  LUUS_Singular03::DgDu_sparse(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t2   = XL__[iX_x1] * XL__[iX_x1];
    real_type t4   = XL__[iX_x2] * XL__[iX_x2];
    ...
    real_type t14  = XR__[iX_x2] * XR__[iX_x2];
    result__[ 0   ] = t9 * (t2 + t4 + t6) + t9 * (t12 + t14 + t6);
    if ( m_debug )
  ...
  void
  LUUS_Singular03::u_eval_analytic(
    NodeType2 const &    LEFT__,
    NodeType2 const &    RIGHT__,
    P_const_pointer_type P__,
    U_pointer_type       U__
    ...
    real_type t5   = XL__[iX_x2] * XL__[iX_x2];
    real_type t7   = XR__[iX_x1] * XR__[iX_x1];
    real_type t9   = XR__[iX_x2] * XR__[iX_x2];
    real_type t11  = ModelPars[iM_epsi_x] * ModelPars[iM_epsi_x];
    U__[ iU_u ] = uControl.solve(-2 / (t3 + t5 + t7 + t9 + 2 * t11) * LM__[1], -1, 1);
    if ( m_debug )
  ...
  LUUS_Singular03::DmDu_eval(
    ...
    real_type t6   = ModelPars[iM_epsi_x] * ModelPars[iM_epsi_x];
    real_type t8   = U__[iU_u];
    real_type t9   = ALIAS_uControl_D_1(t8, -1, 1);
    result__[ 0   ] = t9 * (t2 + t4 + t6) - 2 * V__[1] + 2 * t8;
    if ( m_debug )
  ...
  LUUS_Singular03::DmDuu_sparse(
    ...
    real_type t4   = X__[iX_x2] * X__[iX_x2];
    real_type t6   = ModelPars[iM_epsi_x] * ModelPars[iM_epsi_x];
    real_type t9   = ALIAS_uControl_D_1_1(U__[iU_u], -1, 1);
    result__[ 0   ] = t9 * (t2 + t4 + t6) + 2;
    if ( m_debug )
...
// EOF: LUUS_Singular03_Methods_controls.cc
