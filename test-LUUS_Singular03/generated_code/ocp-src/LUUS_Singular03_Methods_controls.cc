...
namespace LUUS_Singular03Define {
  ...
  LUUS_Singular03::g_fun_eval(
    ...
    real_type result__ = t2 + t4 + t9 * (t2 + t4 + t6) + t3 * MU__[0] + t8 * MU__[1];
    if ( m_debug ) {
  ...
  LUUS_Singular03::g_eval(
    ...
    result__[ 0   ] = t9 * (t2 + t4 + t6) + MU__[1];
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 1, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer LUUS_Singular03::DgDxpm_numRows() const { return 1; }
  integer LUUS_Singular03::DgDxpm_numCols() const { return 4; }
  integer LUUS_Singular03::DgDxpm_nnz()     const { return 3; }

  void
  LUUS_Singular03::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  LUUS_Singular03::u_eval_analytic(
    ...
    U__[ iU_u ] = uControl.solve(-1.0 / (t3 + t5 + t7) * MU__[1], -1, 1);
    if ( m_debug )
