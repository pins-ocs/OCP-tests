...
main() {
  ...
  try {
    ...
    LUUS_Singular03  model("LUUS_Singular03",n_threads,&console);
    ...
    gc_data["DumpFile"] = "LUUS_Singular03_dump";
    ...
    // Continuation Parameters
    data_Parameters["epsi_x0"] = epsi_x0;
    data_Parameters["epsi_x1"] = 1e-08;
...
LUUS_Singular03_data.Mesh["segments"][0]["n"] = 1000;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/LUUS_Singular03_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/LUUS_Singular03_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
