...
main() {
  ...
  try {
    ...
    ROSS_Fuller      model("ROSS_Fuller",&console,&TP);
    ...
    gc_data["DumpFile"] = "ROSS_Fuller_dump";
    ...
    // Continuation Parameters
    data_Parameters["u_epsilon_max"] = u_epsilon_max;
    data_Parameters["u_epsilon_min"] = 1e-10;
    data_Parameters["u_tolerance_max"] = u_tolerance_max;
    data_Parameters["u_tolerance_min"] = 1e-10;
...
ROSS_Fuller_data.Mesh["segments"][4]["n"] = 200;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/ROSS_Fuller_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/ROSS_Fuller_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
