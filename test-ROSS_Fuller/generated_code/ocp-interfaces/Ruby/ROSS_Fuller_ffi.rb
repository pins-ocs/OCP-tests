...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libROSS_Fuller.#{DYLIB_EXT}"
...
module ROSS_Fuller
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libROSS_Fuller.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ROSS_Fuller.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ROSS_Fuller.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ROSS_Fuller.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ROSS_Fuller.OCP_write_ocp_solution( @id, fname )
    end
