...
#include "farmer_problem_dll_pins.hh"
#include <map>
...
#define MAP_PROBLEM map< string, farmer_problem_Problem * >
...
namespace farmer_problemDefine {
  ...
  mrb_value
  mrb_farmer_problem_ocp_setup( mrb_state *mrb, mrb_value self ) {
    // ruby parameters
    ...
    if ( gc_data.exists("InfoLevel") )
      infoLevel = gc_data("InfoLevel").get_as_int("mrb_farmer_problem_ocp_setup::InfoLevel ");
  ...
  FARMER_PROBLEM_API_DLL
  mrb_value
  mrb_farmer_problem_ocp_guess( mrb_state *mrb, mrb_value self ) {
    // ruby parameters
    mrb_value id_value   = mrb_iv_get(mrb, self, mrb_intern_cstr(mrb, "@id"   ));
    mrb_value data_value = mrb_iv_get(mrb, self, mrb_intern_cstr(mrb, "@data" ));
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "farmer_problem_ocp_guess, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
  ...
  FARMER_PROBLEM_API_DLL
  mrb_value
  mrb_farmer_problem_ocp_solve(mrb_state *mrb, mrb_value self) {
    // ruby parameters
    mrb_value id_value   = mrb_iv_get(mrb, self, mrb_intern_cstr(mrb, "@id"   ));
    mrb_value data_value = mrb_iv_get(mrb, self, mrb_intern_cstr(mrb, "@data" ));
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "farmer_problem_ocp_solve, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      if ( !ok )
        std::cerr
          << MSG_ERROR( "farmer_problem_ocp_solve, solution failed!" )
          << '\n';
  ...
  FARMER_PROBLEM_API_DLL
  mrb_value
  mrb_farmer_problem_write_ocp_solution( mrb_state *mrb, mrb_value self ) {
    mrb_value a_string;
    // ruby parameters
    mrb_value id_value = mrb_iv_get(mrb, self, mrb_intern_cstr(mrb, "@id" ));
    ...
    if ( it == problems.end() ) {
      std::cerr
        << MSG_ERROR( "farmer_problem_ocp_write_to_file, no solution found!" )
        << '\n';
    } else {
  ...
  FARMER_PROBLEM_API_DLL
  mrb_value
  mrb_farmer_problem_ocp_solution_range( mrb_state *mrb, mrb_value self ) {
  ...
  FARMER_PROBLEM_API_DLL
  mrb_value
  mrb_farmer_problem_ocp_eval_solution( mrb_state *mrb, mrb_value self ) {
  ...
  FARMER_PROBLEM_API_DLL
  void
  mrb_libfarmer_problem_gem_init(mrb_state* mrb) {
    ...
    mrb_define_const(
      ...
      ocp_class,
      "NAME",
      mrb_str_new_cstr(mrb, "farmer_problem")
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "write_ocp_solution",
      mrb_farmer_problem_write_ocp_solution,
      MRB_ARGS_REQ(1)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "eval_solution",
      mrb_farmer_problem_ocp_eval_solution,
      MRB_ARGS_REQ(2)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "solution_range",
      mrb_farmer_problem_ocp_solution_range,
      MRB_ARGS_REQ(1)
    );
  ...
  FARMER_PROBLEM_API_DLL
  void
  mrb_libfarmer_problem_gem_final(mrb_state* mrb)
  {
  }
}

// EOF: farmer_problem_dll_pins.cc
