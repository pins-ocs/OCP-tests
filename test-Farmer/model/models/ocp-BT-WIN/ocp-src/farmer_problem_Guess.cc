...
namespace farmer_problemDefine {
  ...
  void
  farmer_problem::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  farmer_problem::xlambda_guess_eval(
    integer              i_segment,
  ...
  void
  farmer_problem::p_check( P_const_pointer_type P__ ) const {

  }

  void
  farmer_problem::xlambda_check_node(
    integer              ipos,
    integer              i_segment,
    Q_const_pointer_type Q__,
    P_const_pointer_type P__,
    ...
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
      ...
      Xoptima__check__le(0, X__[3], __message_node_check_3);
      Xoptima__check__le(0, X__[0], __message_node_check_0);
      Xoptima__check__le(0, X__[1], __message_node_check_1);
      Xoptima__check__le(0, X__[2], __message_node_check_2);
      Xoptima__check__le(0, X__[4], __message_node_check_4);
  }

  void
  farmer_problem::xlambda_check_cell(
    integer              icell,
    integer              i_segment,
    Q_const_pointer_type Q__,
    P_const_pointer_type P__,
    ...
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
      /* @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      real_type const * U__ = z + U_block(icell-1);

        x1__oControl.check_range(U__[0], -0.1e-2, 100);
        x2__oControl.check_range(U__[1], -0.1e-2, 100);
        x3__oControl.check_range(U__[2], -0.1e-2, 100);
        x4__oControl.check_range(U__[3], -0.1e-2, 100);
      */

  }
}

// EOF: farmer_problem_Guess.cc
