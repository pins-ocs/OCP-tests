...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libfarmer_problem.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libfarmer_problem.#{DYLIB_EXT}"
end
...
module Farmer_problem
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libfarmer_problem.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = Farmer_problem.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = Farmer_problem.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = Farmer_problem.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      Farmer_problem.OCP_write_ocp_solution( @id, fname )
    end
