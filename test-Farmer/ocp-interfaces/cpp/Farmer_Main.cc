...
main() {
  ...
  try {
    Farmer * m_model = new Farmer("Farmer",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/Farmer_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/Farmer_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/Farmer_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
