...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"Ptot",&ProblemStorage::do_Ptot},
  {"Ptot_D",&ProblemStorage::do_Ptot_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "Farmer_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
    ...
    mexErrMsgTxt(
      fmt::format( "Farmer_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "Farmer_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
