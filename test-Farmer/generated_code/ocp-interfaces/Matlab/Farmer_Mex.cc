...
"Farmer_Mex('command', arguments ):\n"
"%===========================================================================%\n"
;

#define MODEL_NAME  "Farmer"
#define MODEL_CLASS Farmer
...
public:
  ...
  )
  {
    ...
    try {
      ...
      mexErrMsgTxt(
        fmt::format(
          "Farmer_Mex('set_guess',...), error: {}",
          exc.what()
        ).c_str()
      );
    }
    catch (...) {
      mexErrMsgTxt( "Farmer_Mex('set_guess',...), Unknown error\n" );
    }
    #undef CMD
  }
  ...
  )
  {
    ...
    try {
      ...
      string msg = fmt::format( "Farmer_Mex('eval_F',...) error: {}", exc.what() );
      mexWarnMsgTxt( msg.c_str() );
      ok = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "Farmer_Mex('eval_F',...) unkown error\n" );
      ok = false;
    }
  ...
  )
  {
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "Farmer_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
      ok_value = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "Farmer_Mex('eval_JF',...) unkown error\n" );
      ok_value = false;
    }
    int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
    UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
...
do_Ptot(
  ...
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_Ptot( nlhs, plhs, nrhs, prhs );
}
...
do_Ptot_D(
  ...
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_Ptot_D( nlhs, plhs, nrhs, prhs );
}
...
do_Ptot_DD(
  ...
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_Ptot_DD( nlhs, plhs, nrhs, prhs );
}
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt( fmt::format( "Farmer_Mex, unknown command: {}", exc.what() ).c_str() );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt( fmt::format( "Farmer_Mex, error: {}", exc.what() ).c_str() );
  }
  catch (...) {
