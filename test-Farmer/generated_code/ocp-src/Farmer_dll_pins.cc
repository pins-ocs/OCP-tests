...
#define MAP_PROBLEM map< string, Farmer_Problem * >
...
namespace FarmerDefine {
  ...
  FARMER_API_DLL
  mrb_value
  mrb_Farmer_ocp_setup( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Farmer_ocp_setup, missing first parameter or is nota a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Farmer_ocp_setup, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      problems[id] = new Farmer_Problem( n_threads, pConsole );
  ...
  FARMER_API_DLL
  mrb_value
  mrb_Farmer_ocp_guess( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Farmer_ocp_guess, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Farmer_ocp_guess, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Farmer_ocp_guess, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      Mechatronix::mrb_to_GC( mrb, data_value, gc_data, "mrb_to_GC:" );
      it->second->guess( gc_data( "Guess", "mrb_Farmer_ocp_guess" ) );
    }
  ...
  FARMER_API_DLL
  mrb_value
  mrb_Farmer_ocp_solve( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Farmer_ocp_solve, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Farmer_ocp_solve, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Farmer_ocp_solve, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      if ( !ok )
        pConsole->error( "Farmer_ocp_solve, solution failed!\n" );
  ...
  FARMER_API_DLL
  mrb_value
  mrb_Farmer_write_ocp_solution( mrb_state *mrb, mrb_value self ) {
    mrb_value a_string;
    ...
    MAP_PROBLEM::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error( "Farmer_ocp_write_to_file, no solution found!\n" );
    } else {
  ...
  FARMER_API_DLL
  mrb_value
  mrb_Farmer_ocp_solution_range( mrb_state *mrb, mrb_value self ) {
  ...
  FARMER_API_DLL
  mrb_value
  mrb_Farmer_ocp_eval_solution( mrb_state *mrb, mrb_value self ) {
  ...
  FARMER_API_DLL
  void
  mrb_libFarmer_gem_init(mrb_state* mrb) {
  ...
  FARMER_API_DLL
  void
  mrb_libFarmer_gem_final(mrb_state* mrb)
  {
  }
}

// EOF: Farmer_dll_pins.cc
