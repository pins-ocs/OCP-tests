...
namespace HangGliderDefine {
  ...
  void
  HangGlider::p_guess_eval( P_p_type P__ ) const {
  ...
  bool
  HangGlider::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  HangGlider::penalties_check_node(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__
  ) const {
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    bool ok = true;
    ok = ok && Tbound.check_range(-P__[iP_T], m_max_penalty_value);
    return ok;
  }
  ...
  integer HangGlider::u_guess_numEqns() const { return 1; }
  ...
  HangGlider::u_check_if_admissible(
    ...
    ok = ok && cLControl.check_range(U__[iU_cL], ModelPars[iM_cL_min], ModelPars[iM_cL_max]);
    return ok;
  }
