...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libHangGlider.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libHangGlider.#{DYLIB_EXT}"
end
...
module HangGlider
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libHangGlider.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = HangGlider.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = HangGlider.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = HangGlider.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      HangGlider.OCP_write_ocp_solution( @id, fname )
    end
