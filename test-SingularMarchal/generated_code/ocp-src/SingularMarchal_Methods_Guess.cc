...
namespace SingularMarchalDefine {
  ...
  void
  SingularMarchal::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  SingularMarchal::xlambda_guess_eval(
    integer              i_segment,
  ...
  bool
  SingularMarchal::p_check( P_const_pointer_type P__ ) const {
    return true;
  }
  ...
  bool
  SingularMarchal::xlambda_check_node(
    integer              ipos,
  ...
  bool
  SingularMarchal::xlambda_check_cell(
    integer              icell,
  ...
  integer SingularMarchal::u_guess_numEqns() const { return 1; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  SingularMarchal::u_guess_eval(
    NodeType2 const    & LEFT__,
    NodeType2 const    & RIGHT__,
    P_const_pointer_type P__,
    ...
    UGUESS__[ iU_u ] = uControl.solve(-4 / (t3 + t5 + 4 * ModelPars[iM_epsilon]) * L__[iL_lambda2__xo], -1, 1);
    if ( m_debug )
  ...
  bool
  SingularMarchal::u_check_if_admissible(
    NodeType2 const    & NODE__,
