...
namespace SingularMarchalDefine {
  ...
  void
  SingularMarchal::p_guess_eval( P_p_type P__ ) const {
  }
  ...
  bool
  SingularMarchal::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  SingularMarchal::penalties_check_node(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__
  ) const {
  ...
  SingularMarchal::u_guess_eval(
    ...
    UGUESS__[ iU_u ] = uControl.solve(-2 / (t3 + 2 * ModelPars[iM_epsilon]) * MU__[1], -1, 1);
    if ( m_debug )
