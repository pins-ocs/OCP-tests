/*-----------------------------------------------------------------------*\
 |  file: SingularMarchal_dll_ruby.cc                                    |
 |                                                                       |
...
#include "SingularMarchal_dll_ruby.hh"
#include <map>
...
namespace SingularMarchalDefine {
  ...
  static map< string, SingularMarchal_Problem * > problems;
  ...
  */
  ...
  SingularMarchal_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, SingularMarchal_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  SINGULARMARCHAL_API_DLL
  bool
  SingularMarchal_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, SingularMarchal_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "SingularMarchal_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  SingularMarchal_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("SingularMarchal_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  SingularMarchal_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, SingularMarchal_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("SingularMarchal_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: SingularMarchal_dll_ruby.cc
