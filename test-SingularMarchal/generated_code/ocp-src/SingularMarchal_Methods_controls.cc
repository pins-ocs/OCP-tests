...
namespace SingularMarchalDefine {
  ...
  SingularMarchal::g_eval(
    ...
    result__[ 0   ] = t7 * (t2 + 2 * ModelPars[iM_epsilon]) / 2 + L__[iL_lambda2__xo];
    if ( m_debug )
  ...
  integer
  SingularMarchal::DgDxlp_numRows() const
  { return 1; }

  integer
  SingularMarchal::DgDxlp_numCols() const
  { return 4; }

  integer
  SingularMarchal::DgDxlp_nnz() const
  { return 2; }

  void
  SingularMarchal::DgDxlp_pattern(
    integer_ptr iIndex,
    integer_ptr jIndex
  ) const {
  ...
  void
  SingularMarchal::u_eval_analytic(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t6   = XL__[iX_x] * XL__[iX_x];
    real_type t8   = XR__[iX_x] * XR__[iX_x];
    U__[ iU_u ] = uControl.solve(1.0 / (t6 + t8 + 4 * ModelPars[iM_epsilon]) * (-2 * LL__[iL_lambda2__xo] - 2 * LR__[iL_lambda2__xo]), -1, 1);
    if ( m_debug )
  ...
  void
  SingularMarchal::DuDxlxlp_full_analytic(
    NodeType2 const &          LEFT__,
    NodeType2 const &          RIGHT__,
    P_const_pointer_type       P__,
    U_const_pointer_type       U__,
    ...
    real_type t6   = t5 * t5;
    real_type t7   = XR__[iX_x];
    real_type t8   = t7 * t7;
    real_type t11  = t6 + t8 + 4 * ModelPars[iM_epsilon];
    real_type t12  = 1.0 / t11;
    real_type t14  = uControl.solve_rhs(2 * t12 * t3, -1, 1);
    real_type t16  = -t3 * t14;
    real_type t17  = t11 * t11;
    real_type t18  = 1.0 / t17;
    real_type tmp_0_0 = 4 * t18 * t5 * t16;
    real_type tmp_0_1 = 0;
    real_type tmp_0_2 = 0;
    real_type tmp_0_3 = -2 * t12 * t14;
    real_type tmp_0_4 = 4 * t18 * t7 * t16;
    real_type tmp_0_5 = 0;
    real_type tmp_0_6 = 0;
  ...
  SingularMarchal::DmDu_eval(
    ...
    result__[ 0   ] = t7 * (t2 + 2 * ModelPars[iM_epsilon]) / 2 + 2 * t6 - 2 * V__[1];
    if ( m_debug )
  ...
  SingularMarchal::DmDuu_sparse(
    ...
    real_type t2   = X__[iX_x] * X__[iX_x];
    real_type t7   = ALIAS_uControl_D_1_1(U__[iU_u], -1, 1);
    result__[ 0   ] = 2 + t7 * (t2 + 2 * ModelPars[iM_epsilon]) / 2;
    if ( m_debug )
...
// EOF: SingularMarchal_Methods_controls.cc
