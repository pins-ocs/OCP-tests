...
namespace SingularMarchalDefine {
  ...
  SingularMarchal::g_fun_eval(
    ...
    real_type result__ = t3 + t7 * (t3 + ModelPars[iM_epsilon]) + MU__[0] * X__[iX_y] + t6 * MU__[1];
    if ( m_debug ) {
  ...
  SingularMarchal::g_eval(
    ...
    result__[ 0   ] = t7 * (t2 / 2 + ModelPars[iM_epsilon]) + MU__[1];
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 1, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer SingularMarchal::DgDxpm_numRows() const { return 1; }
  integer SingularMarchal::DgDxpm_numCols() const { return 4; }
  integer SingularMarchal::DgDxpm_nnz()     const { return 2; }

  void
  SingularMarchal::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  SingularMarchal::u_eval_analytic(
    ...
    U__[ iU_u ] = uControl.solve(-2 / (t3 + 2 * ModelPars[iM_epsilon]) * MU__[1], -1, 1);
    if ( m_debug )
