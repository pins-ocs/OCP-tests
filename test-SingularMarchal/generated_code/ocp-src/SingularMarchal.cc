...
namespace SingularMarchalDefine {
  ...
  */
  SingularMarchal::SingularMarchal(
    string const   & name,
  ...
  */
  void
  SingularMarchal::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "SingularMarchal::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  SingularMarchal::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "SingularMarchal::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in SingularMarchal::setup not all parameters are set!\n"
    );
  }

  void
  SingularMarchal::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  SingularMarchal::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  SingularMarchal::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  SingularMarchal::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  SingularMarchal::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "SingularMarchal::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  SingularMarchal::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "SingularMarchal::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in SingularMarchal::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  SingularMarchal::info_model( integer level ) const {
  ...
  */
  void
  SingularMarchal::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("SingularMarchal::setup, Debug");
  ...
  */
  void
  SingularMarchal::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  SingularMarchal::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
