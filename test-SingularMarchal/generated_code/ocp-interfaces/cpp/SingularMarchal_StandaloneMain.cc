...
main() {
  ...
  try {
    ...
    SingularMarchal  model("SingularMarchal",n_threads,&console);
    ...
    gc_data["DumpFile"] = "SingularMarchal_dump";
    ...
    data_Parameters["x_i"] = 2;
    data_Parameters["y_f"] = 2;
    data_Parameters["y_i"] = -2;
...
SingularMarchal_data.Mesh["segments"][0]["n"] = 1000;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/SingularMarchal_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/SingularMarchal_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
