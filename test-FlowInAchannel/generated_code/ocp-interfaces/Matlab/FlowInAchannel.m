...
classdef FlowInAchannel < handle
  ...
  methods
    ...
    function sol = pack_for_direct( self, X, U, Pars )
      ...
      %
      sol = FlowInAchannel_Mex( 'pack_for_direct', self.objectHandle, X, U, Pars );
    end
    % ---------------------------------------------------------------------
    function [X, U, Pars] = unpack_for_direct( self, sol )
      ...
      %
      [X, U, Pars] = FlowInAchannel_Mex( 'unpack_for_direct', self.objectHandle, sol );
    end
    % ---------------------------------------------------------------------
    function [Lambda,Omega] = estimate_multipliers( self, X, U, Pars, method )
      ...
      %
      [Lambda,Omega] = FlowInAchannel_Mex( 'estimate_multipliers', self.objectHandle, X, U, Pars, method );
    end
    ...
    function U = init_U( self, Z, do_minimization )
      ...
      % Initialize `u`
      %
      U = FlowInAchannel_Mex( 'init_U', self.objectHandle, Z, do_minimization );
    end
    % ---------------------------------------------------------------------
    ...
    function [ac] = eval_ac( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      ac = FlowInAchannel_Mex( 'ac', self.objectHandle, ...
    ...
    function [ DacDxlxlp, DacDu ] = ...
      ...
      %
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      [DacDxlxlp, DacDu] = FlowInAchannel_Mex( ...
    ...
    function [Jh,Jc] = eval_DhcDxlxlop( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      %
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      [Jh,Jc] = FlowInAchannel_Mex( 'DhcDxlxlop', self.objectHandle, ...
        iseg_L,  q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
      );
    end
    % ---------------------------------------------------------------------
    ...
    function u = eval_u( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        u = FlowInAchannel_Mex( 'u', self.objectHandle, ...
          iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
        );
      else
    ...
    function DuDxlxlp = eval_DuDxlxlp( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        DuDxlxlp = FlowInAchannel_Mex( 'DuDxlxlp', self.objectHandle, ...
          iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
        );
      else
    ...
    function rhs = eval_rhs_ode( self, iseg, q, x, u, pars )
      ...
      %
      rhs = FlowInAchannel_Mex( 'rhs_ode', self.objectHandle, ...
        iseg, q, x, u, pars ...
      );
    end
    % ---------------------------------------------------------------------
    ...
    function J = eval_DnuDxp( self, iseg, q, x, V, pars )
      ...
      % respect to `x` and `pars`.
      %
      J = FlowInAchannel_Mex( 'DnuDxp', self.objectHandle, iseg, q, x, V, pars );
    end
    % ---------------------------------------------------------------------
    ...
    function Dfd_odeDxxpu = eval_Dfd_odeDxxpu( self, iseg_L, t_L, x_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        Dfd_odeDxxpu = FlowInAchannel_Mex( ...
    ...
    function D2fd_odeD2xxpu = eval_D2fd_odeD2xxpu( self, iseg_L, t_L, x_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        D2fd_odeD2xxpu = FlowInAchannel_Mex( ...
    ...
    function D2mayerD2xxp = eval_D2mayerD2xxp( self, iseg_L, t_L, x_L, ...
                                                     ...
                                                     pars )
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      D2mayerD2xxp = FlowInAchannel_Mex( ...
    ...
    function J = eval_penalties( self, iseg, q, x, u, pars )
      ...
      %
      J = FlowInAchannel_Mex( 'JP', self.objectHandle, iseg, q, x, u, pars );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    ...
    function target = eval_q( self, i_segment, s )
      target = FlowInAchannel_Mex( 'mesh_functions', self.objectHandle, i_segment, s );
    end
    % ---------------------------------------------------------------------
    ...
    function res = D2bcD2xxp_pattern( self )
      res = FlowInAchannel_Mex('D2bcD2xxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = Drhs_odeDxpu_pattern( self )
      res = FlowInAchannel_Mex('Drhs_odeDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DsegmentLinkDxp_pattern( self )
      res = FlowInAchannel_Mex('DsegmentLinkDxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DHxpDxpu_pattern( self )
      res = FlowInAchannel_Mex('DHxpDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DJPDxpu_pattern( self )
      res = FlowInAchannel_Mex('DJPDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DLTDxpu_pattern( self )
      res = FlowInAchannel_Mex('DLTDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DJUDxpu_pattern( self )
      res = FlowInAchannel_Mex('DJUDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DnuDxp_pattern( self )
      res = FlowInAchannel_Mex('DnuDxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DgDxlxlp_pattern( self )
      res = FlowInAchannel_Mex('DgDxlxlp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DmDuu_pattern( self )
      res = FlowInAchannel_Mex('DmDuu_pattern', self.objectHandle );
    end
