...
main() {
  ...
  try {
    ...
    FlowInAchannel   model("FlowInAchannel",&TP,&console);
    ...
    gc_data["DumpFile"] = "FlowInAchannel_dump";
    ...
    data_BoundaryConditions["initial_u1"] = SET;
    data_BoundaryConditions["final_u"] = SET;
    data_BoundaryConditions["final_u1"] = SET;
...
FlowInAchannel_data.Mesh["segments"][0]["n"] = 100;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/FlowInAchannel_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/FlowInAchannel_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
