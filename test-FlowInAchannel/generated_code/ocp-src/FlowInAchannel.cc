...
namespace FlowInAchannelDefine {
  ...
  */
  ...
  FlowInAchannel::update_continuation(
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "FlowInAchannel::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  FlowInAchannel::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "FlowInAchannel::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in FlowInAchannel::setup not all parameters are set!\n"
    );
  }

  void
  FlowInAchannel::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  FlowInAchannel::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  FlowInAchannel::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  FlowInAchannel::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  FlowInAchannel::setup_controls( GenericContainer const & gc_data ) {
  ...
  */
  void
  FlowInAchannel::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "FlowInAchannel::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in FlowInAchannel::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  FlowInAchannel::info_model( integer level ) const {
  ...
  */
  void
  FlowInAchannel::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("FlowInAchannel::setup, Debug");
  ...
  */
  void
  FlowInAchannel::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  FlowInAchannel::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
