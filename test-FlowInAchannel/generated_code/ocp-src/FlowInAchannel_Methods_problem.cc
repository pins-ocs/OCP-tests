...
namespace FlowInAchannelDefine {
  ...
  void
  FlowInAchannel::continuation_step_1( real_type s ) {
    real_type t1   = ModelPars[iM_R0];
    ModelPars[iM_R] = t1 + (ModelPars[iM_R1] - t1) * s;
  }
  ...
  FlowInAchannel::H_eval(
    ...
    real_type result__ = t2 * MU__[0] + t5 * MU__[1] + t8 * MU__[2] + (t5 * t2 - t8 * X__[iX_u]) * ModelPars[iM_R] * MU__[3];
    if ( m_debug ) {
  ...
  real_type
  FlowInAchannel::lagrange_target(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__
  ) const {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer FlowInAchannel::D2mayerD2xxp_numRows() const { return 8; }
  integer FlowInAchannel::D2mayerD2xxp_numCols() const { return 8; }
  integer FlowInAchannel::D2mayerD2xxp_nnz()     const { return 0; }

  void
  FlowInAchannel::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  FlowInAchannel::D2mayerD2xxp_sparse(
    NodeQX const & LEFT__,
    NodeQX const & RIGHT__,
    P_const_p_type P__,
    real_ptr       result__
  ...
  integer FlowInAchannel::DlagrangeDxpu_numEqns() const { return 4; }

  void
  FlowInAchannel::DlagrangeDxpu_eval(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer FlowInAchannel::D2lagrangeD2xpu_numRows() const { return 4; }
  integer FlowInAchannel::D2lagrangeD2xpu_numCols() const { return 4; }
  integer FlowInAchannel::D2lagrangeD2xpu_nnz()     const { return 0; }

  void
  FlowInAchannel::D2lagrangeD2xpu_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  FlowInAchannel::D2lagrangeD2xpu_sparse(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  integer
  FlowInAchannel::q_numEqns() const
  { return 1; }

  void
  FlowInAchannel::q_eval(
    integer   i_segment,
    real_type s,
    Q_p_type  result__
  ) const {
  ...
  integer FlowInAchannel::DsegmentLinkDxxp_numRows() const { return 0; }
  integer FlowInAchannel::DsegmentLinkDxxp_numCols() const { return 0; }
  integer FlowInAchannel::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  FlowInAchannel::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  FlowInAchannel::DsegmentLinkDxxp_sparse(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
  ...
  FlowInAchannel::jump_eval(
    ...
    MeshStd::SegmentClass const & segmentLeft  = pMesh->get_segment_by_index(i_segment_left);
    MeshStd::SegmentClass const & segmentRight = pMesh->get_segment_by_index(i_segment_right);
    result__[ 0   ] = XR__[iX_u] - XL__[iX_u];
    result__[ 1   ] = XR__[iX_u1] - XL__[iX_u1];
    result__[ 2   ] = XR__[iX_u2] - XL__[iX_u2];
    result__[ 3   ] = XR__[iX_u3] - XL__[iX_u3];
...
// EOF: FlowInAchannel_Methods_problem.cc
