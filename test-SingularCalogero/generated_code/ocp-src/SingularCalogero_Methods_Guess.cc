...
namespace SingularCalogeroDefine {
  ...
  void
  SingularCalogero::p_guess_eval( P_p_type P__ ) const {
  }
  ...
  bool
  SingularCalogero::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  SingularCalogero::penalties_check_node(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__
  ) const {
  ...
  SingularCalogero::u_guess_eval(
    ...
    UGUESS__[ iU_u ] = uControl.solve(-MU__[0], -1, 1);
    if ( m_debug )
