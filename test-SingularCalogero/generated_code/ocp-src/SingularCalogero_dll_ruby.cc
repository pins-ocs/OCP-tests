/*-----------------------------------------------------------------------*\
 |  file: SingularCalogero_dll_ruby.cc                                   |
 |                                                                       |
...
#include "SingularCalogero_dll_ruby.hh"
#include <map>
...
namespace SingularCalogeroDefine {
  ...
  static map< string, SingularCalogero_Problem * > problems;
  ...
  */
  ...
  SingularCalogero_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, SingularCalogero_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  SINGULARCALOGERO_API_DLL
  bool
  SingularCalogero_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, SingularCalogero_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "SingularCalogero_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  SingularCalogero_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("SingularCalogero_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  SingularCalogero_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, SingularCalogero_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("SingularCalogero_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: SingularCalogero_dll_ruby.cc
