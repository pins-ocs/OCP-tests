...
namespace SingularCalogeroDefine {
  ...
  SingularCalogero::g_fun_eval(
    ...
    real_type result__ = t6 * MU__[0] + t5 + t7;
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer SingularCalogero::DgDxpm_numRows() const { return 1; }
  integer SingularCalogero::DgDxpm_numCols() const { return 2; }
  integer SingularCalogero::DgDxpm_nnz()     const { return 1; }

  void
  SingularCalogero::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  void
  SingularCalogero::DgDxpm_sparse(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__,
  ...
  SingularCalogero::u_eval_analytic(
    ...
    integer i_segment = NODE__.i_segment;
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    U__[ iU_u ] = uControl.solve(-MU__[0], -1, 1);
    if ( m_debug )
