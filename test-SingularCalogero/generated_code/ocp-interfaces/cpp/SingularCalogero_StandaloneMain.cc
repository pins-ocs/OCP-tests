...
main() {
  ...
  try {
    ...
    SingularCalogero model("SingularCalogero",n_threads,&console);
    ...
    gc_data["DumpFile"] = "SingularCalogero_dump";
...
SingularCalogero_data.Mesh["s0"] = -1;
SingularCalogero_data.Mesh["segments"][0]["length"] = 2;
SingularCalogero_data.Mesh["segments"][0]["n"] = 100;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/SingularCalogero_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/SingularCalogero_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
