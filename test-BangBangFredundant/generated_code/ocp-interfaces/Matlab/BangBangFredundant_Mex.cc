...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"Flim",&ProblemStorage::do_Flim},
  {"Flim_D",&ProblemStorage::do_Flim_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "BangBangFredundant_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
    ...
    mexErrMsgTxt(
      fmt::format( "BangBangFredundant_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "BangBangFredundant_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
