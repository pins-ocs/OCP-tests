/*-----------------------------------------------------------------------*\
 |  file: BangBangFredundant_Mex.cc                                      |
 |                                                                       |
...
"BangBangFredundant_Mex('command', arguments ):\n"
"%===========================================================================%\n"
;

#define MODEL_NAME  "BangBangFredundant"
#define MODEL_CLASS BangBangFredundant
...
public:
  ...
  )
  {
    ...
    try {
      ...
      mexErrMsgTxt(
        fmt::format(
          "BangBangFredundant_Mex('set_guess',...), error: {}",
          exc.what()
        ).c_str()
      );
    }
    catch (...) {
      mexErrMsgTxt( "BangBangFredundant_Mex('set_guess',...), Unknown error\n" );
    }
    #undef CMD
  }
  ...
  )
  {
    ...
    try {
      ...
      string msg = fmt::format( "BangBangFredundant_Mex('eval_F',...) error: {}", exc.what() );
      mexWarnMsgTxt( msg.c_str() );
      ok = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "BangBangFredundant_Mex('eval_F',...) unkown error\n" );
      ok = false;
    }
  ...
  )
  {
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "BangBangFredundant_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
      ok_value = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "BangBangFredundant_Mex('eval_JF',...) unkown error\n" );
      ok_value = false;
    }
    int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
    UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
  ...
  do_Flim(
    ...
    #define CMD MODEL_NAME "_Mex('Flim', obj, xo___V ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
  ...
  {
    #define CMD MODEL_NAME "_Mex('Flim_D', obj, xo___V ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
    real_const_ptr arg0 = Utils::mex_matrix_pointer( arg_in_2, N0, M0, CMD " xo___V" );
  ...
  do_Flim_DD(
    ...
    #define CMD MODEL_NAME "_Mex('Flim_DD', obj, xo___V ): "
    CHECK_IN_OUT( 3, 1 );
    mwSize N0, M0;
    real_const_ptr arg0 = Utils::mex_matrix_pointer( arg_in_2, N0, M0, CMD " xo___V" );
...
do_Flim(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('Flim',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_Flim( nlhs, plhs, nrhs, prhs );
}
...
do_Flim_D(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('Flim_D',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_Flim_D( nlhs, plhs, nrhs, prhs );
}
...
do_Flim_DD(
  ...
  UTILS_MEX_ASSERT(
    nrhs >= 2,
    MODEL_NAME "_Mex('Flim_DD',...): Expected at least {} argument(s), nrhs = {}\n", nrhs
  );
  Utils::mex_convert_mx_to_ptr<ProblemStorage>(arg_in_1)->do_Flim_DD( nlhs, plhs, nrhs, prhs );
}
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt( fmt::format( "BangBangFredundant_Mex, unknown command: {}", exc.what() ).c_str() );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt( fmt::format( "BangBangFredundant_Mex, error: {}", exc.what() ).c_str() );
  }
  catch (...) {
