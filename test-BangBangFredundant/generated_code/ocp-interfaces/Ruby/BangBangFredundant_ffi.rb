#-----------------------------------------------------------------------#
#  file: BangBangFredundant_run.rb                                      #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libBangBangFredundant.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libBangBangFredundant.#{DYLIB_EXT}"
end
...
module BangBangFredundant
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libBangBangFredundant.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = BangBangFredundant.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = BangBangFredundant.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = BangBangFredundant.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      BangBangFredundant.OCP_write_ocp_solution( @id, fname )
    end
