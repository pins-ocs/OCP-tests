/*-----------------------------------------------------------------------*\
 |  file: BangBangFredundant_dll_ruby.cc                                 |
 |                                                                       |
...
#include "BangBangFredundant_dll_ruby.hh"
#include <map>
...
namespace BangBangFredundantDefine {
  ...
  static map< string, BangBangFredundant_Problem * > problems;
  ...
  */
  ...
  BangBangFredundant_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, BangBangFredundant_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  BANGBANGFREDUNDANT_API_DLL
  bool
  BangBangFredundant_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, BangBangFredundant_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "BangBangFredundant_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  BangBangFredundant_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("BangBangFredundant_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  BangBangFredundant_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, BangBangFredundant_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("BangBangFredundant_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: BangBangFredundant_dll_ruby.cc
