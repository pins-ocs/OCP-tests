/*-----------------------------------------------------------------------*\
 |  file: ICLOCS_ParallelParking.cc                                      |
 |                                                                       |
...
namespace ICLOCS_ParallelParkingDefine {
  ...
  */
  ICLOCS_ParallelParking::ICLOCS_ParallelParking(
    string const &  name,
    ThreadPool *    TP,
  ...
  , C_bound("C_bound")
  , D_bound("D_bound")
  , EXTRA1_bound("EXTRA1_bound")
  , EXTRA2_bound("EXTRA2_bound")
  , vel_bound("vel_bound")
  , acc_bound("acc_bound")
  , phi_bound("phi_bound")
  // Constraints 2D
  ...
  */
  void
  ICLOCS_ParallelParking::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
  ...
  */
  void
  ICLOCS_ParallelParking::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "ICLOCS_ParallelParking::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in ICLOCS_ParallelParking::setup not all parameters are set!\n"
    );
  }

  void
  ICLOCS_ParallelParking::setup_parameters( real_type const Pars[] ) {
  ...
  */
  void
  ICLOCS_ParallelParking::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "ICLOCS_ParallelParking::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("A_bound"),
      "in ICLOCS_ParallelParking::setup_classes(gc) missing key: ``A_bound''\n"
    );
    A_bound.setup( gc("A_bound") );

    UTILS_ASSERT0(
      gc.exists("B_bound"),
      "in ICLOCS_ParallelParking::setup_classes(gc) missing key: ``B_bound''\n"
    );
    B_bound.setup( gc("B_bound") );

    UTILS_ASSERT0(
      gc.exists("C_bound"),
      "in ICLOCS_ParallelParking::setup_classes(gc) missing key: ``C_bound''\n"
    );
    C_bound.setup( gc("C_bound") );

    UTILS_ASSERT0(
      gc.exists("D_bound"),
      "in ICLOCS_ParallelParking::setup_classes(gc) missing key: ``D_bound''\n"
    );
    D_bound.setup( gc("D_bound") );

    UTILS_ASSERT0(
      gc.exists("EXTRA1_bound"),
      "in ICLOCS_ParallelParking::setup_classes(gc) missing key: ``EXTRA1_bound''\n"
    );
    EXTRA1_bound.setup( gc("EXTRA1_bound") );

    UTILS_ASSERT0(
      gc.exists("EXTRA2_bound"),
      "in ICLOCS_ParallelParking::setup_classes(gc) missing key: ``EXTRA2_bound''\n"
    );
    EXTRA2_bound.setup( gc("EXTRA2_bound") );

    UTILS_ASSERT0(
      gc.exists("vel_bound"),
      "in ICLOCS_ParallelParking::setup_classes(gc) missing key: ``vel_bound''\n"
    );
    vel_bound.setup( gc("vel_bound") );

    UTILS_ASSERT0(
      gc.exists("acc_bound"),
      "in ICLOCS_ParallelParking::setup_classes(gc) missing key: ``acc_bound''\n"
    );
    acc_bound.setup( gc("acc_bound") );

    UTILS_ASSERT0(
      gc.exists("phi_bound"),
      "in ICLOCS_ParallelParking::setup_classes(gc) missing key: ``phi_bound''\n"
    );
    phi_bound.setup( gc("phi_bound") );
  ...
  */
  void
  ICLOCS_ParallelParking::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  ICLOCS_ParallelParking::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  ICLOCS_ParallelParking::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "ICLOCS_ParallelParking::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  ICLOCS_ParallelParking::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "ICLOCS_ParallelParking::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in ICLOCS_ParallelParking::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  ICLOCS_ParallelParking::info_classes() const {
    int msg_level = 3;
    ostringstream mstr;
    ...
    C_bound     .info(mstr);
    D_bound     .info(mstr);
    EXTRA1_bound.info(mstr);
    EXTRA2_bound.info(mstr);
    vel_bound   .info(mstr);
    acc_bound   .info(mstr);
    phi_bound   .info(mstr);
    m_console->message(mstr.str(),msg_level);
  ...
  */
  void
  ICLOCS_ParallelParking::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("ICLOCS_ParallelParking::setup, Debug");
  ...
  */
  void
  ICLOCS_ParallelParking::get_names( GenericContainer & out ) const {
  ...
  void
  ICLOCS_ParallelParking::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
