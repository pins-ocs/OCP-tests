...
namespace PathConstrainedDefine {
  ...
  */
  void
  PathConstrained::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "PathConstrained::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  PathConstrained::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "PathConstrained::setup_parameters: Missing key `Parameters` in data\n"
    );
  ...
  void
  PathConstrained::setup_parameters( real_type const Pars[] ) {
  ...
  */
  void
  PathConstrained::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "PathConstrained::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("x2bound"),
      "in PathConstrained::setup_classes(gc) missing key: ``x2bound''\n"
    );
  ...
  */
  void
  PathConstrained::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  PathConstrained::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  PathConstrained::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "PathConstrained::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  PathConstrained::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "PathConstrained::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in PathConstrained::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  PathConstrained::info_classes() const {
    int msg_level = 3;
    ostringstream mstr;
