...
main() {
  ...
  try {
    ...
    PathConstrained  model("PathConstrained",&TP,&console);
    ...
    gc_data["DumpFile"] = "PathConstrained_dump";
    ...
    // Continuation Parameters
    data_Parameters["epsi_ctrl0"] = epsi_ctrl0;
    data_Parameters["epsi_ctrl1"] = 1e-06;
    data_Parameters["tol_ctrl0"] = tol_ctrl0;
    data_Parameters["tol_ctrl1"] = 1e-06;
    ...
    data_x2bound["subType"]   = "PENALTY_REGULAR";
    data_x2bound["epsilon"]   = epsi_ctrl;
    data_x2bound["tolerance"] = tol_ctrl;
    data_x2bound["active"]    = true;
    // Constraint2D: none defined

    // User defined classes initialization
    // User defined classes: M E S H
PathConstrained_data.Mesh["s0"] = 0;
PathConstrained_data.Mesh["segments"][0]["n"] = 400;
PathConstrained_data.Mesh["segments"][0]["length"] = 1;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/PathConstrained_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/PathConstrained_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
