...
main() {
  ...
  try {
    PathConstrained * m_model = new PathConstrained("PathConstrained",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/PathConstrained_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/PathConstrained_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/PathConstrained_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
