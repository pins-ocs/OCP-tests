  ...
  */
  ...
  PathConstrained_ocp_guess( char const id[], GenericContainer & gc_data ) {
    ...
    if ( it == problems.end() ) {
      ...
      it->second->guess( gc_data("Guess", "PathConstrained_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  PathConstrained_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("PathConstrained_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  PathConstrained_write_ocp_solution( char const id[], char const fname[] ) {
    ...
    if ( it == problems.end() ) {
      pConsole->error("PathConstrained_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
