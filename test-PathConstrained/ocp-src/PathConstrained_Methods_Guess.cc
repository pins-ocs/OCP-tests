...
namespace PathConstrainedDefine {
  ...
  void
  PathConstrained::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  PathConstrained::xlambda_guess_eval(
    integer              i_segment,
  ...
  bool
  PathConstrained::p_check( P_const_pointer_type P__ ) const {
    bool ok = true;
  ...
  bool
  PathConstrained::xlambda_check_node(
    integer              ipos,
  ...
  bool
  PathConstrained::xlambda_check_cell(
    integer              icell,
    NodeType2 const    & NODE__,
    P_const_pointer_type P__
  ) const {
  ...
  PathConstrained::u_check_if_admissible(
    ...
    real_type const * L__ = NODE__.lambda;
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    ok = ok && uControl.check_range(U__[0], -20, 20);
    return ok;
  }

}

// EOF: PathConstrained_Guess.cc
