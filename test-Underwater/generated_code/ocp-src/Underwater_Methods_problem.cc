...
namespace UnderwaterDefine {
  ...
  void
  Underwater::continuation_step_0( real_type s ) {
  ...
  void
  Underwater::continuation_step_1( real_type s ) {
  ...
  Underwater::H_eval(
    ...
    real_type result__ = (t6 * t4 + t9 * t8) * t2 * MU__[0] + (-t9 * t4 + t6 * t8) * t2 * MU__[1] + t21 * t2 * MU__[2] + (-t27 * t30 * t21 * t8 + t27 * U__[iU_u1]) * t2 * MU__[3] + (t38 * t26 * t21 * t4 + t38 * U__[iU_u2]) * t2 * MU__[4] + (t49 * U__[iU_u3] + t49 * (t30 - t26) * t8 * t4) * t2 * MU__[5];
    if ( m_debug ) {
  ...
  integer Underwater::DlagrangeDxpu_numEqns() const { return 10; }

  void
  Underwater::DlagrangeDxpu_eval(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Underwater::D2lagrangeD2xpu_numRows() const { return 10; }
  integer Underwater::D2lagrangeD2xpu_numCols() const { return 10; }
  integer Underwater::D2lagrangeD2xpu_nnz()     const { return 0; }

  void
  Underwater::D2lagrangeD2xpu_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  Underwater::D2lagrangeD2xpu_sparse(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  integer Underwater::DsegmentLinkDxxp_numRows() const { return 0; }
  integer Underwater::DsegmentLinkDxxp_numCols() const { return 0; }
  integer Underwater::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  Underwater::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  Underwater::DsegmentLinkDxxp_sparse(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
