...
namespace UnderwaterDefine {
  ...
  void
  Underwater::p_guess_eval( P_p_type P__ ) const {
  ...
  bool
  Underwater::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  Underwater::penalties_check_node(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__
  ) const {
  ...
  integer Underwater::u_guess_numEqns() const { return 3; }
  ...
  Underwater::u_check_if_admissible(
    ...
    ok = ok && u3Control.check_range(U__[iU_u3], -1, 1);
    return ok;
  }
