...
#include "Underwater_dll_ruby.hh"
#include <map>
...
namespace UnderwaterDefine {
  ...
  */
  ...
  UNDERWATER_API_DLL
  bool
  Underwater_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, Underwater_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "Underwater_ocp_guess" ) );
      return true;
    }
  }
  ...
  UNDERWATER_API_DLL
  bool
  Underwater_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("Underwater_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  UNDERWATER_API_DLL
  void
  Underwater_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, Underwater_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("Underwater_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
