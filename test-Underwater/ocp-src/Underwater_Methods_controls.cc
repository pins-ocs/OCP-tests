...
namespace UnderwaterDefine {
  ...
  Underwater::g_eval(
    ...
    result__[ 0   ] = t3 * t1 + 1.0 / ModelPars[iM_m1] * t1 * LM__[3];
    real_type t11  = ALIAS_u2Control_D_1(UM__[1], -1, 1);
    result__[ 1   ] = t11 * t1 + 1.0 / ModelPars[iM_m3] * t1 * LM__[4];
    real_type t19  = ALIAS_u3Control_D_1(UM__[2], -1, 1);
    result__[ 2   ] = t19 * t1 + 1.0 / ModelPars[iM_inertia] * t1 * LM__[5];
    if ( m_debug )
  ...
  Underwater::DgDxlxlp_sparse(
    ...
    result__[ 2   ] = LM__[3] * t3 + t6;
    real_type t10  = 1.0 / ModelPars[iM_m3];
    result__[ 3   ] = 0.5e0 * t10 * t1;
    result__[ 4   ] = result__[3];
    real_type t13  = ALIAS_u2Control_D_1(UM__[1], -1, 1);
    result__[ 5   ] = LM__[4] * t10 + t13;
