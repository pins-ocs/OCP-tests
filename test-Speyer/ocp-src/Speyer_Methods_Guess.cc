...
namespace SpeyerDefine {
  ...
  void
  Speyer::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  Speyer::xlambda_guess_eval(
    integer              i_segment,
    ...
    real_type t3   = 2 * 0.3141592654e1 * Q__[0];
    X__[ iX_x1 ] = sin(t3);
    X__[ iX_x2 ] = cos(t3);
  ...
  bool
  Speyer::p_check( P_const_pointer_type P__ ) const {
    bool ok = true;
