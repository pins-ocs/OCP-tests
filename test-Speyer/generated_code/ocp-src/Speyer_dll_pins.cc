...
#define MAP_PROBLEM map< string, Speyer_Problem * >
...
namespace SpeyerDefine {
  ...
  SPEYER_API_DLL
  mrb_value
  mrb_Speyer_ocp_setup( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Speyer_ocp_setup, missing first parameter or is nota a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Speyer_ocp_setup, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      problems[id] = new Speyer_Problem( pTP, pConsole );
  ...
  SPEYER_API_DLL
  mrb_value
  mrb_Speyer_ocp_guess( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Speyer_ocp_guess, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Speyer_ocp_guess, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Speyer_ocp_guess, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      it->second->guess(
        gc_data(
          "Guess",
          "mrb_Speyer_ocp_guess Missing field `Guess`"
        )
      );
    }
  ...
  SPEYER_API_DLL
  mrb_value
  mrb_Speyer_ocp_solve( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Speyer_ocp_solve, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Speyer_ocp_solve, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Speyer_ocp_solve, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
  ...
  SPEYER_API_DLL
  mrb_value
  mrb_Speyer_write_ocp_solution( mrb_state *mrb, mrb_value self ) {
    mrb_value a_string;
    ...
    MAP_PROBLEM::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error( "Speyer_ocp_write_to_file, no solution found!\n" );
    } else {
  ...
  SPEYER_API_DLL
  mrb_value
  mrb_Speyer_ocp_solution_range( mrb_state *mrb, mrb_value self ) {
  ...
  SPEYER_API_DLL
  mrb_value
  mrb_Speyer_ocp_eval_solution( mrb_state *mrb, mrb_value self ) {
  ...
  SPEYER_API_DLL
  void
  mrb_libSpeyer_gem_init(mrb_state* mrb) {
  ...
  SPEYER_API_DLL
  void
  mrb_libSpeyer_gem_final(mrb_state* mrb)
  {
  }
}

// EOF: Speyer_dll_pins.cc
