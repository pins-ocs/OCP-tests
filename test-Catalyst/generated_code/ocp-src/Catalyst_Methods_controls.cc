...
namespace CatalystDefine {
  ...
  Catalyst::u_eval_analytic(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t1   = LR__[iL_lambda1__xo];
    real_type t3   = LR__[iL_lambda2__xo];
    real_type t8   = LL__[iL_lambda1__xo];
    real_type t9   = LL__[iL_lambda2__xo];
    U__[ iU_u ] = uControl.solve(XR__[iX_x2] * (-10 * t1 + 9 * t3) / 2 + XL__[iX_x1] * (t8 - t9) / 2 + XL__[iX_x2] * (-10 * t8 + 9 * t9) / 2 + XR__[iX_x1] * (t1 - t3) / 2, 0, 1);
    if ( m_debug )
  ...
  Catalyst::DuDxlxlp_full_analytic(
    ...
    real_type t1   = LR__[iL_lambda1__xo];
    real_type t3   = LR__[iL_lambda2__xo];
    real_type t6   = XR__[iX_x2];
    real_type t8   = LL__[iL_lambda1__xo];
    real_type t9   = LL__[iL_lambda2__xo];
    real_type t10  = t8 - t9;
    real_type t11  = XL__[iX_x1];
    real_type t16  = XL__[iX_x2];
    real_type t18  = t1 - t3;
    real_type t19  = XR__[iX_x1];
    real_type t23  = uControl.solve_rhs(t6 * (-10 * t1 + 9 * t3) / 2 + t11 * t10 / 2 + t16 * (-10 * t8 + 9 * t9) / 2 + t19 * t18 / 2, 0, 1);
    real_type tmp_0_0 = t10 * t23 / 2;
    real_type tmp_0_1 = (-5 * t8 + 9.0 / 2.0 * t9) * t23;
    real_type tmp_0_2 = (t11 / 2 - 5 * t16) * t23;
    real_type tmp_0_3 = -(t11 - 9 * t16) * t23 / 2;
    real_type tmp_0_4 = t18 * t23 / 2;
    real_type tmp_0_5 = (-5 * t1 + 9.0 / 2.0 * t3) * t23;
    real_type tmp_0_6 = (t19 / 2 - 5 * t6) * t23;
    real_type tmp_0_7 = -(t19 - 9 * t6) * t23 / 2;
  ...
  Catalyst::m_eval(
    ...
    real_type t10  = pow((t3 - 10 * t4) * t1 + V__[0], 2);
    real_type t16  = pow(t1 * (t3 - 9 * t4) - V__[1] - t4, 2);
    real_type result__ = t2 + t10 + t16;
    if ( m_debug ) {
  ...
  Catalyst::DmDu_eval(
    ...
    result__[ 0   ] = t2 + t6 * (362 * t1 + 18) + t5 * (t10 * (-76 * t1 - 2) - 20 * t12 + 18 * t14) + 4 * t18 * t1 + t10 * (2 * t12 - 2 * t14);
    if ( m_debug )
  ...
  integer
  Catalyst::DmDuu_numRows() const
  { return 1; }

  integer
  Catalyst::DmDuu_numCols() const
  { return 1; }

  integer
  Catalyst::DmDuu_nnz() const
  { return 1; }

  void
  Catalyst::DmDuu_pattern(
    integer_ptr iIndex,
    integer_ptr jIndex
  ) const {
    iIndex[0 ] = 0   ; jIndex[0 ] = 0   ;
  ...
  Catalyst::DmDuu_sparse(
    ...
    real_type t4   = X__[iX_x2];
    real_type t7   = t4 * t4;
    real_type t10  = ALIAS_uControl_D_1_1(U__[iU_u], 0, 1);
    result__[ 0   ] = -76 * t1 * t4 + t10 + 4 * t2 + 362 * t7;
    if ( m_debug )
...
// EOF: Catalyst_Methods_controls.cc
