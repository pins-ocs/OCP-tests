...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libgtocX_2burn.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libgtocX_2burn.#{DYLIB_EXT}"
end
...
module GtocX_2burn
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libgtocX_2burn.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = GtocX_2burn.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = GtocX_2burn.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = GtocX_2burn.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      GtocX_2burn.OCP_write_ocp_solution( @id, fname )
    end
