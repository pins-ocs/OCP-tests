...
)
{
  ...
  try {
    ...
    mexErrMsgTxt(
      fmt::format(
        "gtocX_2burn_Mex('set_guess',...), error: {}",
        exc.what()
      ).c_str()
    );
  }
  catch (...) {
    mexErrMsgTxt( "gtocX_2burn_Mex('set_guess',...), Unknown error\n" );
  }
  #undef CMD
}
...
)
{
  ...
  try {
    ...
    string msg = fmt::format( "gtocX_2burn_Mex('eval_F',...) error: {}", exc.what() );
...
)
{
  ...
  try {
    ...
    mexWarnMsgTxt( fmt::format( "gtocX_2burn_Mex('eval_JF2',...) error: {}", exc.what() ).c_str() );
    ok_value = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "gtocX_2burn_Mex('eval_JF2',...) unkown error\n" );
    ok_value = false;
  }
  int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
  UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
