...
#include "gtocX_2burn_dll_ruby.hh"
#include <map>
...
namespace gtocX_2burnDefine {
  ...
  static map< string, gtocX_2burn_Problem * > problems;
  ...
  */
  ...
  gtocX_2burn_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, gtocX_2burn_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  GTOCX_2BURN_API_DLL
  bool
  gtocX_2burn_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, gtocX_2burn_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "gtocX_2burn_ocp_guess" ) );
      return true;
    }
  }
  ...
  gtocX_2burn_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("gtocX_2burn_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  gtocX_2burn_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, gtocX_2burn_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("gtocX_2burn_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
