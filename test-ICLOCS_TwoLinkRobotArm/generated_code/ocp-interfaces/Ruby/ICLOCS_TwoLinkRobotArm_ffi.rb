#-----------------------------------------------------------------------#
#  file: ICLOCS_TwoLinkRobotArm_run.rb                                  #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libICLOCS_TwoLinkRobotArm.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libICLOCS_TwoLinkRobotArm.#{DYLIB_EXT}"
end
...
module ICLOCS_TwoLinkRobotArm
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libICLOCS_TwoLinkRobotArm.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ICLOCS_TwoLinkRobotArm.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ICLOCS_TwoLinkRobotArm.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ICLOCS_TwoLinkRobotArm.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ICLOCS_TwoLinkRobotArm.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: ICLOCS_TwoLinkRobotArm_ffi.rb
