...
main() {
  ...
  try {
    ...
    ICLOCS2_AlyChan  model("ICLOCS2_AlyChan",&TP,&console);
    ...
    gc_data["DumpFile"] = "ICLOCS2_AlyChan_dump";
...
ICLOCS2_AlyChan_data.Mesh["segments"][0]["length"] = 1/2*Pi;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/ICLOCS2_AlyChan_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/ICLOCS2_AlyChan_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
