...
main() {
  ...
  try {
    ...
    AlyChan          model("AlyChan",&TP,&console);
    ...
    // Continuation Parameters
    data_Parameters["u_epsi_max"] = u_epsi_max;
    data_Parameters["u_epsi_min"] = 1e-10;
    data_Parameters["u_tol_max"] = u_tol_max;
    data_Parameters["u_tol_min"] = 1e-10;
...
AlyChan_data.Mesh["s0"] = 0;
AlyChan_data.Mesh["segments"][0]["n"] = 400;
AlyChan_data.Mesh["segments"][0]["length"] = 1/2*Pi;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/AlyChan_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/AlyChan_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
