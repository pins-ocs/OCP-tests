...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libICLOCS2_AlyChan.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libICLOCS2_AlyChan.#{DYLIB_EXT}"
end
...
module ICLOCS2_AlyChan
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libICLOCS2_AlyChan.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ICLOCS2_AlyChan.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ICLOCS2_AlyChan.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ICLOCS2_AlyChan.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ICLOCS2_AlyChan.OCP_write_ocp_solution( @id, fname )
    end
