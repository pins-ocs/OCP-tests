...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libAlyChan.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libAlyChan.#{DYLIB_EXT}"
end
...
module AlyChan
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libAlyChan.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = AlyChan.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = AlyChan.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = AlyChan.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      AlyChan.OCP_write_ocp_solution( @id, fname )
    end
