...
namespace AlyChanDefine {
  ...
  */
  ...
  AlyChan::update_continuation(
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "AlyChan::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  AlyChan::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "AlyChan::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in AlyChan::setup not all parameters are set!\n"
    );
  }

  void
  AlyChan::setup_parameters( real_type const Pars[] ) {
  ...
  */
  void
  AlyChan::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  AlyChan::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  AlyChan::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  AlyChan::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "AlyChan::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  AlyChan::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "AlyChan::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in AlyChan::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  AlyChan::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("AlyChan::setup, Debug");
  ...
  */
  void
  AlyChan::get_names( GenericContainer & out ) const {
  ...
  void
  AlyChan::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
