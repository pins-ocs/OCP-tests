...
classdef ICLOCS_Catalyst < handle
  ...
  methods
    ...
    function U = guess_U( self, Z )
      ...
      % Initialize `u`
      %
      U = ICLOCS_Catalyst_Mex( 'guess_U', self.objectHandle, Z );
    end
    % ---------------------------------------------------------------------
    ...
    function [JF,ok] = eval_JF2( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      [JF,ok] = ICLOCS_Catalyst_Mex( 'eval_JF2', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    ...
    function [a,b,c] = eval_abc( self, L, R, pars, U )
      ...
      % <<FD1.jpg>>
      %
      [a,b,c] = ICLOCS_Catalyst_Mex( 'abc', self.objectHandle, L, R, pars, U );
    end
    % ---------------------------------------------------------------------
    function DabcDxlxlpu = eval_DabcDxlxlpu( self, L, R, pars, U )
      ...
      % <<FD2.jpg>>
      %
      DabcDxlxlpu = ICLOCS_Catalyst_Mex( 'DabcDxlxlpu', self.objectHandle, L, R, pars, U );
    end
    % ---------------------------------------------------------------------
    ...
    function DuDxlxlp = eval_DuDxlxlp( self, L, R, pars, MU )
      ...
      %
      DuDxlxlp = ICLOCS_Catalyst_Mex( 'DuDxlxlp', self.objectHandle, L, R, pars, MU );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DsegmentLinkDxxp_pattern( self )
      res = ICLOCS_Catalyst_Mex('DsegmentLinkDxxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
