...
classdef ICLOCS_Catalyst < handle
  ...
  methods
    % ---------------------------------------------------------------------
    function self = ICLOCS_Catalyst( name )
      %% Allocate the C++ class instance
      self.objectHandle = ICLOCS_Catalyst_Mex( 'new', name );
    end
    % ---------------------------------------------------------------------
    function delete( self )
      %% Destroy the C++ class instance
      ICLOCS_Catalyst_Mex( 'delete', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function help( ~ )
      %% print help for the class usage
      ICLOCS_Catalyst_Mex('help');
    end
    ...
    function data = read( self, fname )
      ...
      %
      data = ICLOCS_Catalyst_Mex( 'read', self.objectHandle, fname );
    end
    % ---------------------------------------------------------------------
    function setup( self, fname_or_struct )
      ...
      %
      ICLOCS_Catalyst_Mex( 'setup', self.objectHandle, fname_or_struct );
      res = ICLOCS_Catalyst_Mex( 'dims', self.objectHandle );
    ...
    function res = get_dim_Q( self )
      ICLOCS_Catalyst_Mex( 'dim_Q', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'dim_Q', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_X( self )
      ICLOCS_Catalyst_Mex( 'dim_X', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'dim_X', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_Pars( self )
      ICLOCS_Catalyst_Mex( 'dim_Pars', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'dim_Pars', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_BC( self )
      ICLOCS_Catalyst_Mex( 'dim_BC', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'dim_BC', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_Post( self )
      ICLOCS_Catalyst_Mex( 'dim_Post', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'dim_Post', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_Ipost( self )
      ICLOCS_Catalyst_Mex( 'dim_Ipost', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'dim_Ipost', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_num_active_BC( self )
      ICLOCS_Catalyst_Mex( 'num_active_BC', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'num_active_BC', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_num_nodes( self )
      ICLOCS_Catalyst_Mex( 'num_nodes', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'num_nodes', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_num_equations( self )
      ICLOCS_Catalyst_Mex( 'num_equations', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'num_equations', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_num_segments( self )
      ICLOCS_Catalyst_Mex( 'num_segments', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'num_segments', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_full_bc( self )
      ICLOCS_Catalyst_Mex( 'dim_full_bc', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'dim_full_bc', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = get_dim_ineq( self )
      ICLOCS_Catalyst_Mex( 'dim_ineq', self.objectHandle );
      res = ICLOCS_Catalyst_Mex( 'dim_ineq', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function n = names( self )
      ...
      %
      n = ICLOCS_Catalyst_Mex( 'names', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = get_ocp_data( self )
      ...
      % res.Pointers
      %
      res = ICLOCS_Catalyst_Mex( 'get_ocp_data', self.objectHandle );
    end
    ...
    % ---------------------------------------------------------------------
    function info( self )
      ICLOCS_Catalyst_Mex( 'info', self.objectHandle );
    end
    ...
    function set_info_level( self, infoLvl )
      ...
      %
      ICLOCS_Catalyst_Mex( 'set_info_level', self.objectHandle, infoLvl );
    end
    %
    % DEPRECATED
    %
    ...
    function set_max_threads( self, nt )
      ...
      %
      ICLOCS_Catalyst_Mex( 'set_max_threads', self.objectHandle, nt );
    end
    ...
    function remesh( self, new_mesh )
      ...
      %
      ICLOCS_Catalyst_Mex( 'remesh', self.objectHandle, new_mesh );
    end
    ...
    function set_guess( self, varargin )
      ...
      %
      ICLOCS_Catalyst_Mex( 'set_guess', self.objectHandle, varargin{:} );
    end
    %
    % ---------------------------------------------------------------------
    %
    function guess = get_guess( self )
      ...
      %
      guess = ICLOCS_Catalyst_Mex( 'get_guess', self.objectHandle );
    end
    %
    % ---------------------------------------------------------------------
    %
    function guess = get_solution_as_guess( self )
      ...
      %
      guess = ICLOCS_Catalyst_Mex( 'get_solution_as_guess', self.objectHandle );
    end
    ...
    function ok = solve( self, varargin )
      ...
      %
      ok = ICLOCS_Catalyst_Mex( 'solve', self.objectHandle, varargin{:} );
    end
    %
    % ---------------------------------------------------------------------
    %
    function update_continuation( self, n, old_s, s )
      ...
      % the continuation.
      %
      ICLOCS_Catalyst_Mex( 'update_continuation', self.objectHandle, n, old_s, s );
    end
    ...
    function sol = solution( self, varargin )
      ...
      %
      %
      sol = ICLOCS_Catalyst_Mex( 'get_solution', self.objectHandle, varargin{:} );
    end
    % ---------------------------------------------------------------------
    function sol = solution_by_group( self )
      ...
      sol = ICLOCS_Catalyst_Mex( 'get_solution2', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function sol = solution_by_group_and_names( self )
      ...
      % res.data.control_penalties
      %
      sol = ICLOCS_Catalyst_Mex( 'get_solution3', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function sol = pack( self, X, Lambda, Pars, Omega )
      ...
      %
      sol = ICLOCS_Catalyst_Mex( 'pack', self.objectHandle, X, Lambda, Pars, Omega );
    end
    % ---------------------------------------------------------------------
    function [X, Lambda, Pars, Omega ] = unpack( self, sol )
      ...
      %
      [X, Lambda, Pars, Omega] = ICLOCS_Catalyst_Mex( 'unpack', self.objectHandle, sol );
    end
    % ---------------------------------------------------------------------
    function sol = pack_for_direct( self, X, U, Pars )
      ...
      %
      sol = ICLOCS_Catalyst_Mex( 'pack_for_direct', self.objectHandle, X, U, Pars );
    end
    % ---------------------------------------------------------------------
    function [X, U, Pars] = unpack_for_direct( self, sol )
      ...
      %
      [X, U, Pars] = ICLOCS_Catalyst_Mex( 'unpack_for_direct', self.objectHandle, sol );
    end
    % ---------------------------------------------------------------------
    function [Lambda,Omega] = estimate_multipliers( self, X, U, Pars, method )
      ...
      %
      [Lambda,Omega] = ICLOCS_Catalyst_Mex( 'estimate_multipliers', self.objectHandle, X, U, Pars, method );
    end
    ...
    function res = zeta( self )
      res = ICLOCS_Catalyst_Mex( 'get_solution', self.objectHandle, 'zeta' );
    end
    ...
    function res = x1( self )
      ...
      %
      res = ICLOCS_Catalyst_Mex( 'get_solution', self.objectHandle, 'x1' );
    end
    function res = x2( self )
      %
      % Return the solution for the state: x2
      %
      res = ICLOCS_Catalyst_Mex( 'get_solution', self.objectHandle, 'x2' );
    end
    ...
    function res = lambda1( self )
      ...
      %
      res = ICLOCS_Catalyst_Mex( 'get_solution', self.objectHandle, 'lambda1__xo' );
    end
    function res = lambda2( self )
      %
      % Return the solution for the multiplier: lambda2
      %
      res = ICLOCS_Catalyst_Mex( 'get_solution', self.objectHandle, 'lambda2__xo' );
    end
    ...
    function res = u( self )
      ...
      %
      res = ICLOCS_Catalyst_Mex( 'get_solution', self.objectHandle, 'u' );
    end
    ...
    function res = post_processing_uControl( self )
      ...
      %
      res = ICLOCS_Catalyst_Mex( 'get_solution', self.objectHandle, 'uControl' );
    end
    ...
    function U = init_U( self, Z, do_minimization )
      ...
      % Initialize `u`
      %
      U = ICLOCS_Catalyst_Mex( 'init_U', self.objectHandle, Z, do_minimization );
    end
    % ---------------------------------------------------------------------
    function U = eval_U( self, Z, u_guess )
      ...
      %
      U = ICLOCS_Catalyst_Mex( 'eval_U', self.objectHandle, Z, u_guess );
    end
    % ---------------------------------------------------------------------
    function [F,ok] = eval_F( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      [F,ok] = ICLOCS_Catalyst_Mex( 'eval_F', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    function [JF,ok] = eval_JF( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      [JF,ok] = ICLOCS_Catalyst_Mex( 'eval_JF', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    function JF = eval_JF_pattern( self )
      ...
      %
      JF = ICLOCS_Catalyst_Mex( 'eval_JF_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function [Z,U] = get_raw_solution( self )
      ...
      %
      [Z,U] = ICLOCS_Catalyst_Mex( 'get_raw_solution', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function set_raw_solution( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      ICLOCS_Catalyst_Mex( 'set_raw_solution', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    function ok = check_raw_solution( self, Z )
      ...
      % admissible regions.
      %
      ok = ICLOCS_Catalyst_Mex( 'check_raw_solution', self.objectHandle, Z );
    end
    % ---------------------------------------------------------------------
    function check_jacobian( self, Z, U, epsi )
      ...
      %
      ICLOCS_Catalyst_Mex( 'check_jacobian', self.objectHandle, Z, U, epsi );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    ...
    function [ac] = eval_ac( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      ac = ICLOCS_Catalyst_Mex( 'ac', self.objectHandle, ...
    ...
    function [ DacDxlxlp, DacDu ] = ...
      ...
      %
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      [DacDxlxlp, DacDu] = ICLOCS_Catalyst_Mex( ...
    ...
    function [h,c] = eval_hc( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      %
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      [h,c] = ICLOCS_Catalyst_Mex( 'hc', self.objectHandle, ...
    ...
    function [Jh,Jc] = eval_DhcDxlxlop( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      %
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      [Jh,Jc] = ICLOCS_Catalyst_Mex( 'DhcDxlxlop', self.objectHandle, ...
        iseg_L,  q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
      );
    end
    % ---------------------------------------------------------------------
    ...
    function u = eval_u( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        u = ICLOCS_Catalyst_Mex( 'u', self.objectHandle, ...
          iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
        );
      else
    ...
    function DuDxlxlp = eval_DuDxlxlp( self, iseg_L, t_L, x_L, lambda_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        DuDxlxlp = ICLOCS_Catalyst_Mex( 'DuDxlxlp', self.objectHandle, ...
          iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
        );
      else
    ...
    function rhs = eval_rhs_ode( self, iseg, q, x, u, pars )
      ...
      %
      rhs = ICLOCS_Catalyst_Mex( 'rhs_ode', self.objectHandle, ...
        iseg, q, x, u, pars ...
      );
    end
    % ---------------------------------------------------------------------
    ...
    function A = eval_A( self, iseg, q, x, pars )
      ...
      % respect to `pars`.
      %
      A = ICLOCS_Catalyst_Mex( 'A', self.objectHandle, iseg, q, x, pars );
    end
    % ---------------------------------------------------------------------
    ...
    function nu = eval_nu( self, iseg, q, x, V, pars )
      ...
      %
      nu = ICLOCS_Catalyst_Mex( 'nu', self.objectHandle, iseg, q, x, V, pars );
    end
    % ---------------------------------------------------------------------
    function J = eval_DnuDxp( self, iseg, q, x, V, pars )
      ...
      % respect to `x` and `pars`.
      %
      J = ICLOCS_Catalyst_Mex( 'DnuDxp', self.objectHandle, iseg, q, x, V, pars );
    end
    % ---------------------------------------------------------------------
    function bc = eval_bc( self, iseg_L, t_L, x_L, iseg_R, t_R, x_R, pars )
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      bc = ICLOCS_Catalyst_Mex( ...
    ...
    function Dfd_odeDxxpu = eval_Dfd_odeDxxpu( self, iseg_L, t_L, x_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        Dfd_odeDxxpu = ICLOCS_Catalyst_Mex( ...
    ...
    function D2fd_odeD2xxpu = eval_D2fd_odeD2xxpu( self, iseg_L, t_L, x_L, ...
      ...
      if iseg_L == iseg_R
        [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
        D2fd_odeD2xxpu = ICLOCS_Catalyst_Mex( ...
    ...
    function DmayerDxxp = eval_DmayerDxxp( self, iseg_L, t_L, x_L, ...
                                                 ...
                                                 pars )
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      DmayerDxxp = ICLOCS_Catalyst_Mex( ...
    ...
    function D2mayerD2xxp = eval_D2mayerD2xxp( self, iseg_L, t_L, x_L, ...
                                                     ...
                                                     pars )
      [q_L,q_R] = self.eval_q_LR( iseg_L, t_L, iseg_R, t_R );
      D2mayerD2xxp = ICLOCS_Catalyst_Mex( ...
    ...
    function J = eval_penalties( self, iseg, q, x, u, pars )
      ...
      %
      J = ICLOCS_Catalyst_Mex( 'JP', self.objectHandle, iseg, q, x, u, pars );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    ...
    function target = eval_q( self, i_segment, s )
      target = ICLOCS_Catalyst_Mex( 'mesh_functions', self.objectHandle, i_segment, s );
    end
    % ---------------------------------------------------------------------
    function nodes = get_nodes( self )
      nodes = ICLOCS_Catalyst_Mex( 'nodes', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function node_to_segment = get_node_to_segment( self )
      node_to_segment = ICLOCS_Catalyst_Mex( 'node_to_segment', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = A_pattern( self )
      res = ICLOCS_Catalyst_Mex('A_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DbcDxxp_pattern( self )
      res = ICLOCS_Catalyst_Mex('DbcDxxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = D2bcD2xxp_pattern( self )
      res = ICLOCS_Catalyst_Mex('D2bcD2xxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = Drhs_odeDxpu_pattern( self )
      res = ICLOCS_Catalyst_Mex('Drhs_odeDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DsegmentLinkDxp_pattern( self )
      res = ICLOCS_Catalyst_Mex('DsegmentLinkDxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DjumpDxlxlp_pattern( self )
      res = ICLOCS_Catalyst_Mex('DjumpDxlxlp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DHxpDxpu_pattern( self )
      res = ICLOCS_Catalyst_Mex('DHxpDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DJPDxpu_pattern( self )
      res = ICLOCS_Catalyst_Mex('DJPDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DLTDxpu_pattern( self )
      res = ICLOCS_Catalyst_Mex('DLTDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DJUDxpu_pattern( self )
      res = ICLOCS_Catalyst_Mex('DJUDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = D2JPD2xpu_pattern( self )
      res = ICLOCS_Catalyst_Mex('D2JPD2xpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = D2LTD2xpu_pattern( self )
      res = ICLOCS_Catalyst_Mex('D2LTD2xpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = D2JUD2xpu_pattern( self )
      res = ICLOCS_Catalyst_Mex('D2JUD2xpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DLTargsDxpu_pattern( self )
      res = ICLOCS_Catalyst_Mex('DLTargsDxpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = D2LTargsD2xpu_pattern( self )
      res = ICLOCS_Catalyst_Mex('D2LTargsD2xpu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DnuDxp_pattern( self )
      res = ICLOCS_Catalyst_Mex('DnuDxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DetaDxp_pattern( self )
      res = ICLOCS_Catalyst_Mex('DetaDxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DgDxlxlp_pattern( self )
      res = ICLOCS_Catalyst_Mex('DgDxlxlp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DgDu_pattern( self )
      res = ICLOCS_Catalyst_Mex('DgDu_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function res = DmDuu_pattern( self )
      res = ICLOCS_Catalyst_Mex('DmDuu_pattern', self.objectHandle );
    end
