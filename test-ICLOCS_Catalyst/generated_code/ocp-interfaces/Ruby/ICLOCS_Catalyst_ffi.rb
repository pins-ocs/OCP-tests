...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libICLOCS_Catalyst.#{DYLIB_EXT}"
...
module ICLOCS_Catalyst
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libICLOCS_Catalyst.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ICLOCS_Catalyst.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ICLOCS_Catalyst.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ICLOCS_Catalyst.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ICLOCS_Catalyst.OCP_write_ocp_solution( @id, fname )
    end
