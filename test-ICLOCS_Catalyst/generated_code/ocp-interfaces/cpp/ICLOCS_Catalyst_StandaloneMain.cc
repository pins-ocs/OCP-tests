...
main() {
  ...
  try {
    ...
    ICLOCS_Catalyst  model("ICLOCS_Catalyst",n_threads,&console);
    ...
    gc_data["DumpFile"] = "ICLOCS_Catalyst_dump";
...
ICLOCS_Catalyst_data.Mesh["s0"] = 0;
ICLOCS_Catalyst_data.Mesh["segments"][0]["n"] = 400;
ICLOCS_Catalyst_data.Mesh["segments"][0]["length"] = 1;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/ICLOCS_Catalyst_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/ICLOCS_Catalyst_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
