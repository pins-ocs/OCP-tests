...
namespace ICLOCS_CatalystDefine {
  ...
  void
  ICLOCS_Catalyst::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  ICLOCS_Catalyst::xlambda_guess_eval(
    integer              i_segment,
  ...
  bool
  ICLOCS_Catalyst::p_check( P_const_pointer_type P__ ) const {
    return true;
  }
  ...
  bool
  ICLOCS_Catalyst::xlambda_check_node(
    integer              ipos,
  ...
  bool
  ICLOCS_Catalyst::xlambda_check_cell(
    integer              icell,
  ...
  integer ICLOCS_Catalyst::u_guess_numEqns() const { return 1; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  ICLOCS_Catalyst::u_guess_eval(
    NodeType2 const    & LEFT__,
    NodeType2 const    & RIGHT__,
    P_const_pointer_type P__,
    ...
    UGUESS__[ iU_u ] = uControl.solve(-5 * t2 * t1 + t5 * t1 / 2 + t8 * t1 / 2 - 5 * t11 * t1 + 9.0 / 2.0 * t2 * t14 - t5 * t14 / 2 - t8 * t14 / 2 + 9.0 / 2.0 * t11 * t14, 0, 1);
    if ( m_debug )
  ...
  bool
  ICLOCS_Catalyst::u_check_if_admissible(
    NodeType2 const    & NODE__,
