...
"ICLOCS_Speyer_Mex('command', arguments ):\n"
"%===========================================================================%\n"
;

#define MODEL_NAME  "ICLOCS_Speyer"
#define MODEL_CLASS ICLOCS_Speyer
...
public:
...

  {
    ...
    try {
      ...
      mexErrMsgTxt(
        fmt::format(
          "ICLOCS_Speyer_Mex('set_guess',...), error: {}",
          exc.what()
        ).c_str()
      );
    }
    catch (...) {
      mexErrMsgTxt( "ICLOCS_Speyer_Mex('set_guess',...), Unknown error\n" );
    }
    #undef CMD
  }
...

  {
    ...
    try {
      ...
      string msg = fmt::format( "ICLOCS_Speyer_Mex('eval_F',...) error: {}", exc.what() );
      mexWarnMsgTxt( msg.c_str() );
      ok = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "ICLOCS_Speyer_Mex('eval_F',...) unkown error\n" );
      ok = false;
    }
...

  {
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "ICLOCS_Speyer_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
      ok_value = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "ICLOCS_Speyer_Mex('eval_JF',...) unkown error\n" );
      ok_value = false;
    }
    int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
    UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt( fmt::format( "ICLOCS_Speyer_Mex, unknown command: {}", exc.what() ).c_str() );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt( fmt::format( "ICLOCS_Speyer_Mex, error: {}", exc.what() ).c_str() );
  }
  catch (...) {
