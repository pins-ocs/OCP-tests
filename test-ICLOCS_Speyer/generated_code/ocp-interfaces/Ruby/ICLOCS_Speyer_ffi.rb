...
module ICLOCS_Speyer
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libICLOCS_Speyer.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ICLOCS_Speyer.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ICLOCS_Speyer.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ICLOCS_Speyer.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ICLOCS_Speyer.OCP_write_ocp_solution( @id, fname )
    end
