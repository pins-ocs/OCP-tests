...
namespace BangBangFtminPDefine {
  ...
  BangBangFtminP::g_eval(
    ...
    real_type t3   = ALIAS_Fcontrol_D_1(UM__[0], -1, 1);
    result__[ 0   ] = t3 * t1 + t1 * LM__[1];
    if ( m_debug )
  ...
  integer
  BangBangFtminP::DgDxlxlp_numRows() const
  { return 1; }

  integer
  BangBangFtminP::DgDxlxlp_numCols() const
  { return 9; }

  integer
  BangBangFtminP::DgDxlxlp_nnz() const
  { return 3; }

  void
  BangBangFtminP::DgDxlxlp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
  ...
  void
  BangBangFtminP::DgDxlxlp_sparse(
    NodeType2 const &    LEFT__,
    NodeType2 const &    RIGHT__,
    U_const_pointer_type UM__,
    P_const_pointer_type P__,
