...
namespace BangBangFtminPDefine {
  ...
  BangBangFtminP::H_eval(
    ...
    return t8 * t2 * L__[1] + X__[1] * t2 * L__[0] + t10 * t2;
  }
  ...
  integer
  BangBangFtminP::u_guess_numEqns() const
  { return 1; }

  void
  BangBangFtminP::u_guess_eval(
  ...
  bool
  BangBangFtminP::u_check_if_admissible(
    NodeType2 const    & NODE__,
    U_const_pointer_type U__,
    P_const_pointer_type P__
  ) const {
    ...
    real_type const * L__ = NODE__.lambda;
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    ok = ok && Fcontrol.check_range(U__[0], -1, 1);
    return ok;
  }
