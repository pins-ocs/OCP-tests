...
"BangBangFtminP_Mex('command', arguments ):\n"
"%===========================================================================%\n"
;

#define MODEL_NAME  "BangBangFtminP"
#define MODEL_CLASS BangBangFtminP
...
public:
  ...
  do_eval_F(
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "BangBangFtminP_Mex('eval_F',...) error: {}", exc.what() ).c_str() );
      ok = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "BangBangFtminP_Mex('eval_F',...) unkown error\n" );
      ok = false;
    }
  ...
  do_eval_JF(
    ...
    try {
      ...
      mexWarnMsgTxt( fmt::format( "BangBangFtminP_Mex('eval_JF',...) error: {}", exc.what() ).c_str() );
      ok_value = false;
    } catch ( ... ) {
      mexWarnMsgTxt( "BangBangFtminP_Mex('eval_JF',...) unkown error\n" );
      ok_value = false;
    }
    int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
    MEX_ASSERT( ok == 0, CMD "failed the call sparse(...)" );
...
mexFunction(
  ...
  try {
    ...
    mexErrMsgTxt( fmt::format( "BangBangFtminP_Mex, unknown command: {}", exc.what() ).c_str() );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt( fmt::format( "BangBangFtminP_Mex, error: {}", exc.what() ).c_str() );
  }
  catch (...) {
