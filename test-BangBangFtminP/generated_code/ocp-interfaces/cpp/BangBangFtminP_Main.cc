...
main() {
  ...
  try {
    BangBangFtminP * m_model = new BangBangFtminP("BangBangFtminP",&TP,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/BangBangFtminP_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/BangBangFtminP_Data.lua";
    #endif
    ...
    if ( ok ) {
      ...
      file.open( "data/BangBangFtminP_OCP_result_NOT_CONVERGED.ßtxt" );
    }
    file.precision(18);
