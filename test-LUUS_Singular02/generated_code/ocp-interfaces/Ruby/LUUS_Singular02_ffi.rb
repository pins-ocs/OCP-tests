...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libLUUS_Singular02.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libLUUS_Singular02.#{DYLIB_EXT}"
end
...
module LUUS_Singular02
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libLUUS_Singular02.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = LUUS_Singular02.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = LUUS_Singular02.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = LUUS_Singular02.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      LUUS_Singular02.OCP_write_ocp_solution( @id, fname )
    end
