...
main() {
  ...
  try {
    ...
    LUUS_Singular02  model("LUUS_Singular02",n_threads,&console);
    ...
    gc_data["DumpFile"] = "LUUS_Singular02_dump";
    ...
    // Model Parameters
    data_Parameters["epsi_x1"] = epsi_x1;

    // Guess Parameters

    // Boundary Conditions
    data_Parameters["x1_i"] = 0;
    data_Parameters["x2_i"] = 1;
    ...
    data_Parameters["u_epsilon1"] = 1e-06;
    data_Parameters["u_tolerance0"] = u_tolerance0;
    data_Parameters["u_tolerance1"] = 1e-06;
...
LUUS_Singular02_data.Mesh["s0"] = 0;
LUUS_Singular02_data.Mesh["segments"][0]["length"] = Tf;
LUUS_Singular02_data.Mesh["segments"][0]["n"] = 1000;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/LUUS_Singular02_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/LUUS_Singular02_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
