...
namespace LUUS_Singular02Define {
  ...
  */
  LUUS_Singular02::LUUS_Singular02(
    string const   & name,
  ...
  */
  void
  LUUS_Singular02::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "LUUS_Singular02::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  LUUS_Singular02::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "LUUS_Singular02::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in LUUS_Singular02::setup not all parameters are set!\n"
    );
  }

  void
  LUUS_Singular02::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  LUUS_Singular02::setup_classes( GenericContainer const & gc_data ) {
  }
  ...
  */
  void
  LUUS_Singular02::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  LUUS_Singular02::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  LUUS_Singular02::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "LUUS_Singular02::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  LUUS_Singular02::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "LUUS_Singular02::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in LUUS_Singular02::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  LUUS_Singular02::info_model( integer level ) const {
  ...
  */
  void
  LUUS_Singular02::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("LUUS_Singular02::setup, Debug");
  ...
  */
  void
  LUUS_Singular02::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  LUUS_Singular02::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
