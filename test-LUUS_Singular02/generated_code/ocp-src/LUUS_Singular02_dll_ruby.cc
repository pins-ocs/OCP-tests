...
#include "LUUS_Singular02_dll_ruby.hh"
#include <map>
...
namespace LUUS_Singular02Define {
  ...
  */
  ...
  LUUS_SINGULAR02_API_DLL
  bool
  LUUS_Singular02_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, LUUS_Singular02_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "LUUS_Singular02_ocp_guess" ) );
      return true;
    }
  }
  ...
  LUUS_SINGULAR02_API_DLL
  bool
  LUUS_Singular02_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("LUUS_Singular02_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  LUUS_SINGULAR02_API_DLL
  void
  LUUS_Singular02_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, LUUS_Singular02_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("LUUS_Singular02_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: LUUS_Singular02_dll_ruby.cc
