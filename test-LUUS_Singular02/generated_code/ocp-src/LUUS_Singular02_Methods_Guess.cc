...
namespace LUUS_Singular02Define {
  ...
  void
  LUUS_Singular02::p_guess_eval( P_p_type P__ ) const {
  }
  ...
  bool
  LUUS_Singular02::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  LUUS_Singular02::penalties_check_node(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__
  ) const {
  ...
  LUUS_Singular02::u_guess_eval(
    ...
    UGUESS__[ iU_u ] = uControl.solve(-1.0 / (t3 + t5) * MU__[1], -1, 1);
    if ( m_debug )
