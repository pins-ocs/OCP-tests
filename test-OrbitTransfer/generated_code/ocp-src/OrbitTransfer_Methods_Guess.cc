...
namespace OrbitTransferDefine {
  ...
  void
  OrbitTransfer::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  OrbitTransfer::xlambda_guess_eval(
    integer              i_segment,
    ...
    X__[ iX_m     ] = ModelPars[iM_m0];
    real_type t2   = ModelPars[iM_v0];
    real_type t4   = ModelPars[iM_r0];
    X__[ iX_alpha ] = Q__[iQ_zeta] / t4 * t2 * ModelPars[iM_tf];
    X__[ iX_r     ] = t4;
    X__[ iX_u     ] = 0;
    X__[ iX_v     ] = t2;
    L__[ iL_lambda3__xo ] = 0.1e-7;
    if ( m_debug ) {
  ...
  bool
  OrbitTransfer::xlambda_check_node(
    integer              ipos,
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    /* REMOVED */ Xoptima__check__node__lt(0, X__[iX_m], Xoptima__message_node_check_0);
    /* REMOVED */ Xoptima__check__node__lt(0, X__[iX_r], Xoptima__message_node_check_1);
    return true;
  }
  ...
  bool
  OrbitTransfer::xlambda_check_cell(
    integer              icell,
  ...
  integer OrbitTransfer::u_guess_numEqns() const { return 1; }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  void
  OrbitTransfer::u_guess_eval(
    NodeType2 const    & LEFT__,
    NodeType2 const    & RIGHT__,
    P_const_pointer_type P__,
  ...
  bool
  OrbitTransfer::u_check_if_admissible(
    NodeType2 const    & NODE__,
