...
main() {
  ...
  try {
    ...
    OrbitTransfer    model("OrbitTransfer",n_threads,&console);
    ...
    data_BoundaryConditions["v_final"] = SET;
...
OrbitTransfer_data.Mesh["segments"][0]["length"] = 1;
    ...
    if ( ok ) {
      ...
      file.open( "data/OrbitTransfer_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
