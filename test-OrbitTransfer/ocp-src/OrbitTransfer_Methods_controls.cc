...
namespace OrbitTransferDefine {
  ...
  OrbitTransfer::g_eval(
    ...
    real_type t5   = UM__[0];
    real_type t6   = cos(t5);
    real_type t9   = 1.0 / XM__[0];
    real_type t14  = sin(t5);
    real_type t7   = t2 * t4;
    result__[ 0   ] = -LM__[2] * t7 * t9 * t14 + LM__[1] * t7 * t9 * t6;
    if ( m_debug )
  ...
  OrbitTransfer::DgDxlxlp_sparse(
    ...
    real_type t3   = t2 * t4;
    result__[ 0   ] = -0.5e0 * LM__[1] * t3 * t10 * t6 + 0.5e0 * LM__[2] * t3 * t10 * t16;
    real_type t21  = t2 * t4;
    real_type t22  = 1.0 / t8;
    result__[ 1   ] = 0.5e0 * t22 * t6 * t21;
    result__[ 2   ] = -0.5e0 * t22 * t16 * t21;
  ...
  OrbitTransfer::DgDu_sparse(
    ...
    result__[ 0   ] = -t14 * t2 * t4 * t9 * LM__[2] - t2 * t4 * t6 * t9 * LM__[1];
    if ( m_debug )
