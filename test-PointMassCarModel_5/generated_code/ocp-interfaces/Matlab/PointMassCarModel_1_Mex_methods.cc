...
)
{
  ...
  UTILS_MEX_ASSERT0( m_gc_data.exists("Road"), "missing key: ``Road'' in m_gc_data\n" );
  m_road.setup( m_gc_data("Road") );
  ptrs[ "pRoad" ] = &m_road;
...
)
{
  ...
  CHECK_IN_OUT( 3, 0 );
  GenericContainer & gc_mesh = m_gc_data["Road"];
  GenericContainer & ptrs    = m_gc_data["Pointers"];
...
)
{
  ...
  try {
    ...
    mexWarnMsgTxt( fmt::format( "PointMassCarModel_1_Mex('eval_JF2',...) error: {}", exc.what() ).c_str() );
    ok_value = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "PointMassCarModel_1_Mex('eval_JF2',...) unkown error\n" );
    ok_value = false;
  }
  int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
  UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
...
// EOF: PointMassCarModel_1_Mex.cc
