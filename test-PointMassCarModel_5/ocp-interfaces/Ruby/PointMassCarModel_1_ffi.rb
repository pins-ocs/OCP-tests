#-----------------------------------------------------------------------#
#  file: PointMassCarModel_1_run.rb                                     #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libPointMassCarModel_1.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libPointMassCarModel_1.#{DYLIB_EXT}"
end
...
module PointMassCarModel_1
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libPointMassCarModel_1.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = PointMassCarModel_1.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = PointMassCarModel_1.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = PointMassCarModel_1.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      PointMassCarModel_1.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: PointMassCarModel_1_ffi.rb
