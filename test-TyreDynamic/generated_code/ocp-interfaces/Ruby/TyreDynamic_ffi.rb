...
GC_AND_SPLINE_DLL    = MECHATRONIX_LIB_PATH+"/libTyreDynamic.#{DYLIB_EXT}"
...
module TyreDynamic
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libTyreDynamic.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = TyreDynamic.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = TyreDynamic.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = TyreDynamic.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      TyreDynamic.OCP_write_ocp_solution( @id, fname )
    end
