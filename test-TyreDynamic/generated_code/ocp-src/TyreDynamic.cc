...
namespace TyreDynamicDefine {
  ...
  */
  ...
  void
  TyreDynamic::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  TyreDynamic::info_model( integer level ) const {
    ...
    m_console->message( b__oControl.info(),level);
    m_console->message( p__oControl.info(),level);

    // ==================================================================
    m_console->PINS_COLOR_TITLE( fmt::format( "+{:.^71}+\n", "Constraints type LT" ), level);
    m_console->message( OnlyBrakingRear.info(),level);
    m_console->message( OnlyTractionRear.info(),level);
    m_console->message( LongSlipRear_min.info(),level);
    m_console->message( LongSlipRear_max.info(),level);
    m_console->message( v_min.info(),level);
    ...
    m_console->message( fmt::format( "`posPart' mapped with: {}\n",  posPart.info() ), level );
    m_console->message( fmt::format( "`negPart' mapped with: {}\n",  negPart.info() ), level );
    m_console->message( fmt::format( "`clipSup' mapped with: {}\n",  clipSup.info() ), level );
    m_console->message( fmt::format( "`clipInt' mapped with: {}\n",  clipInt.info() ), level );
    m_console->message( fmt::format( "`sign_reg' mapped with: {}\n",  sign_reg.info() ), level );
    m_console->message( fmt::format( "`abs_reg' mapped with: {}\n",  abs_reg.info() ), level );
