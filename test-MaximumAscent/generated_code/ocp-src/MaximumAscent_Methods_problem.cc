...
namespace MaximumAscentDefine {
  ...
  void
  MaximumAscent::continuationStep0( real_type s ) {
  ...
  MaximumAscent::H_eval(
    ...
    real_type t2   = X__[iX_r];
    real_type t3   = t2 * t2;
    real_type t5   = X__[iX_u];
    real_type t7   = X__[iX_v];
    real_type t8   = L__[iL_lambda2__xo];
    real_type t10  = L__[iL_lambda3__xo];
    real_type t18  = tf(ModelPars[iM_days]);
    real_type t24  = -Q__[iQ_zeta] * ModelPars[iM_mdot] * t18 + ModelPars[iM_m0];
    real_type t26  = eta(t18);
    real_type t28  = Tbar(t18);
    real_type t30  = U__[iU_alpha];
    real_type t31  = cos(t30);
    real_type t33  = sin(t30);
    real_type result__ = 1.0 / t24 / t3 * (t26 * t24 * (t5 * t3 * L__[iL_lambda1__xo] + t2 * (-t5 * t10 + t7 * t8 + L__[iL_lambda4__xo]) * t7 - t8) + (t10 * t31 + t8 * t33) * t3 * t28);
    return result__;
  }
#else
  real_type
  ...
  integer
  MaximumAscent::DmayerDx_numEqns() const
  { return 8; }

  void
  MaximumAscent::DmayerDx_eval(
    NodeType const     & LEFT__,
    NodeType const     & RIGHT__,
    P_const_pointer_type P__,
    real_ptr             result__
  ...
  integer
  MaximumAscent::DmayerDp_numEqns() const
  { return 0; }

  void
  MaximumAscent::DmayerDp_eval(
    NodeType const     & LEFT__,
    NodeType const     & RIGHT__,
    P_const_pointer_type P__,
    real_ptr             result__
