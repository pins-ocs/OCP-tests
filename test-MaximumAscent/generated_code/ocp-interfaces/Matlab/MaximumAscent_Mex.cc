...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"eta",&ProblemStorage::do_eta},
  {"eta_D",&ProblemStorage::do_eta_D},
  {"Tbar",&ProblemStorage::do_Tbar},
  {"Tbar_D",&ProblemStorage::do_Tbar_D},
  {"tf",&ProblemStorage::do_tf},
  {"tf_D",&ProblemStorage::do_tf_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "MaximumAscent_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
    ...
    mexErrMsgTxt(
      fmt::format( "MaximumAscent_Mex, unknown command: {}",
      exc.what() ).c_str()
    );
  }
  catch ( std::exception const & exc ) {
    mexErrMsgTxt(
      fmt::format( "MaximumAscent_Mex, error: {}",
      exc.what() ).c_str()
    );
  }
  catch (...) {
