...
namespace PointMassCarModel_2Define {
  ...
  */
  void
  PointMassCarModel_2::updateContinuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "PointMassCarModel_2::updateContinuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  PointMassCarModel_2::setupParameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "PointMassCarModel_2::setupParameters: Missing key `Parameters` in data\n"
    );
  ...
  void
  PointMassCarModel_2::setupParameters( real_type const Pars[] ) {
  ...
  */
  void
  PointMassCarModel_2::setupClasses( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "PointMassCarModel_2::setupClasses: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("AdherenceEllipse"),
      "in PointMassCarModel_2::setupClasses(gc) missing key: ``AdherenceEllipse''\n"
    );
    AdherenceEllipse.setup( gc("AdherenceEllipse") );

    UTILS_ASSERT0(
      gc.exists("RoadLeftBorder"),
      "in PointMassCarModel_2::setupClasses(gc) missing key: ``RoadLeftBorder''\n"
    );
    RoadLeftBorder.setup( gc("RoadLeftBorder") );

    UTILS_ASSERT0(
      gc.exists("RoadRightBorder"),
      "in PointMassCarModel_2::setupClasses(gc) missing key: ``RoadRightBorder''\n"
    );
    RoadRightBorder.setup( gc("RoadRightBorder") );

    UTILS_ASSERT0(
      gc.exists("PowerLimit"),
      "in PointMassCarModel_2::setupClasses(gc) missing key: ``PowerLimit''\n"
    );
    PowerLimit.setup( gc("PowerLimit") );

    UTILS_ASSERT0(
      gc.exists("LimitMinSpeed"),
      "in PointMassCarModel_2::setupClasses(gc) missing key: ``LimitMinSpeed''\n"
    );
  ...
  */
  void
  PointMassCarModel_2::setupUserClasses( GenericContainer const & gc ) {
  }
  ...
  */
  void
  PointMassCarModel_2::setupUserMappedFunctions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  PointMassCarModel_2::setupControls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "PointMassCarModel_2::setupClasses: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  PointMassCarModel_2::setupPointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "PointMassCarModel_2::setupPointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pRoad"),
      "in PointMassCarModel_2::setupPointers(gc) cant find key `pRoad' in gc\n"
    );
  ...
  */
  void
  PointMassCarModel_2::infoClasses() const {
    int msg_level = 3;
    ostringstream mstr;
    ...
    LimitMinSpeed   .info(mstr);
    m_console->message(mstr.str(),msg_level);

    m_console->message("\nUser class (pointer)\n",msg_level);
    mstr.str("");
    mstr << "User function `pRoad`: ";
    pRoad->info(mstr);
    m_console->message(mstr.str(),msg_level);
  ...
  */
  void
  PointMassCarModel_2::diagnostic( GenericContainer const & gc ) {
