...
namespace PointMassCarModel_2Define {
  ...
  PointMassCarModel_2::g_fun_eval(
    ...
    real_type t15  = X__[iX_fx];
    real_type t16  = t15 * t15;
    real_type t18  = ModelPars[iM_mu__x__max] * ModelPars[iM_mu__x__max];
    real_type t22  = ModelPars[iM_g] * ModelPars[iM_g];
    real_type t23  = 1.0 / t22;
    real_type t25  = X__[iX_Omega];
    real_type t26  = t25 * t25;
    real_type t27  = t2 * t2;
    real_type t30  = ModelPars[iM_mu__y__max] * ModelPars[iM_mu__y__max];
    real_type t35  = AdherenceEllipse(t23 / t18 * t16 + t23 / t30 * t27 * t26 - 1);
    real_type t39  = RoadLeftBorder(t4 - Q__[iQ_leftWidth]);
    real_type t43  = RoadRightBorder(-t4 - Q__[iQ_rightWidth]);
    real_type t52  = PowerLimit(ModelPars[iM_m] / ModelPars[iM_Pmax] * t15 * t2 - 1);
    real_type t54  = LimitMinSpeed(-t2);
    real_type t58  = sin(t3);
    real_type result__ = t7 * ModelPars[iM_wT] + t10 * t7 + t13 * t7 + t35 * t7 + t39 * t7 + t43 * t7 + t52 * t7 + t54 * t7 + t58 * t2 * MU__[0] + (-t6 * t5 + t25) * MU__[1] + (-t27 * ModelPars[iM_kD] + t15) * MU__[2] + ModelPars[iM_v__fx__max] * t9 * MU__[3] + ModelPars[iM_v__Omega__max] * t12 * MU__[4];
    if ( m_debug ) {
  ...
  PointMassCarModel_2::g_eval(
    ...
    result__[ 0   ] = t8 * t6 + MU__[3] * ModelPars[iM_v__fx__max];
    real_type t14  = ALIAS_v__OmegaControl_D_1(U__[iU_v__Omega], -1, 1);
    result__[ 1   ] = t14 * t6 + MU__[4] * ModelPars[iM_v__Omega__max];
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 2, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer PointMassCarModel_2::DgDxpm_numRows() const { return 2; }
  integer PointMassCarModel_2::DgDxpm_numCols() const { return 10; }
  integer PointMassCarModel_2::DgDxpm_nnz()     const { return 8; }

  void
  PointMassCarModel_2::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  void
  PointMassCarModel_2::DgDxpm_sparse(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__,
  ...
  PointMassCarModel_2::u_eval_analytic(
    ...
    U__[ iU_v__fx    ] = v__OmegaControl.solve(-t8 * ModelPars[iM_v__fx__max] * MU__[3], -1, 1);
    U__[ iU_v__Omega ] = v__OmegaControl.solve(-t8 * ModelPars[iM_v__Omega__max] * MU__[4], -1, 1);
    if ( m_debug )
