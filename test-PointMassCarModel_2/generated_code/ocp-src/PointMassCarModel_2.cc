...
namespace PointMassCarModel_2Define {
  ...
  */
  PointMassCarModel_2::PointMassCarModel_2(
    string const   & name,
  ...
  */
  void
  PointMassCarModel_2::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "PointMassCarModel_2::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  PointMassCarModel_2::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "PointMassCarModel_2::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in PointMassCarModel_2::setup not all parameters are set!\n"
    );
  }

  void
  PointMassCarModel_2::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  PointMassCarModel_2::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "PointMassCarModel_2::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("AdherenceEllipse"),
      "in PointMassCarModel_2::setup_classes(gc) missing key: ``AdherenceEllipse''\n"
    );
    AdherenceEllipse.setup( gc("AdherenceEllipse") );

    UTILS_ASSERT0(
      gc.exists("RoadLeftBorder"),
      "in PointMassCarModel_2::setup_classes(gc) missing key: ``RoadLeftBorder''\n"
    );
    RoadLeftBorder.setup( gc("RoadLeftBorder") );

    UTILS_ASSERT0(
      gc.exists("RoadRightBorder"),
      "in PointMassCarModel_2::setup_classes(gc) missing key: ``RoadRightBorder''\n"
    );
    RoadRightBorder.setup( gc("RoadRightBorder") );

    UTILS_ASSERT0(
      gc.exists("PowerLimit"),
      "in PointMassCarModel_2::setup_classes(gc) missing key: ``PowerLimit''\n"
    );
    PowerLimit.setup( gc("PowerLimit") );

    UTILS_ASSERT0(
      gc.exists("LimitMinSpeed"),
      "in PointMassCarModel_2::setup_classes(gc) missing key: ``LimitMinSpeed''\n"
    );
    LimitMinSpeed.setup( gc("LimitMinSpeed") );
  ...
  */
  void
  PointMassCarModel_2::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  PointMassCarModel_2::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  PointMassCarModel_2::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "PointMassCarModel_2::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  PointMassCarModel_2::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "PointMassCarModel_2::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pRoad"),
      "in PointMassCarModel_2::setup_pointers(gc) cant find key `pRoad' in gc\n"
    );
  ...
  */
  void
  PointMassCarModel_2::info_model( integer level ) const {
    ...
    m_console->message( v__fxControl.info(),level);
    m_console->message( v__OmegaControl.info(),level);

    // ==================================================================
    m_console->PINS_COLOR_TITLE( fmt::format( "+{:.^71}+\n", "Constraints type LT" ), level);
    m_console->message( AdherenceEllipse.info(),level);
    m_console->message( RoadLeftBorder.info(),level);
    m_console->message( RoadRightBorder.info(),level);
    m_console->message( PowerLimit.info(),level);
    m_console->message( LimitMinSpeed.info(),level);
    ...
    m_console->message( pRoad->info(),level);
  ...
  */
  void
  PointMassCarModel_2::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("PointMassCarModel_2::setup, Debug");
  ...
  */
  void
  PointMassCarModel_2::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  PointMassCarModel_2::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
