...
main() {
  ...
  try {
    ...
    PointMassCarModel_2 model("PointMassCarModel_2",n_threads,&console);
    ...
    gc_data["DumpFile"] = "PointMassCarModel_2_dump";
...
PointMassCarModel_2_data.Road["theta0"] = 0;
PointMassCarModel_2_data.Road["s0"] = 0;
...
PointMassCarModel_2_data.Road["segments"][9]["gridSize"] = 0.1;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/PointMassCarModel_2_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/PointMassCarModel_2_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
