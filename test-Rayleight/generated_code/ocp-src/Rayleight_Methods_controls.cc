...
namespace RayleightDefine {
  ...
  Rayleight::g_eval(
    ...
    real_const_ptr X__ = NODE__.x;
    MeshStd::SegmentClass const & segment = m_pMesh->get_segment_by_index(i_segment);
    result__[ 0   ] = 2 * U__[iU_u] + 4 * MU__[1];
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 1, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Rayleight::DgDxpm_numRows() const { return 1; }
  integer Rayleight::DgDxpm_numCols() const { return 4; }
  integer Rayleight::DgDxpm_nnz()     const { return 1; }

  void
  Rayleight::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
