...
namespace RayleightDefine {
  ...
  Rayleight::u_eval_analytic(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    U__[ iU_u ] = -LL__[iL_lambda2__xo] - LR__[iL_lambda2__xo];
    if ( m_debug )
  ...
  Rayleight::m_eval(
    ...
    real_type t8   = t2 * t2;
    real_type t14  = pow(-V__[1] - X__[iX_x1] + 0.14e1 * t2 - 0.14e0 * t8 * t2 + 4 * U__[iU_u], 2);
  ...
  Rayleight::DmDu_eval(
    ...
    real_type t7   = t5 * t5;
    result__[ 0   ] = -8 * V__[1] - 8 * X__[iX_x1] + 0.112e2 * t5 - 0.112e1 * t7 * t5 + 32 * U__[iU_u];
    if ( m_debug )
