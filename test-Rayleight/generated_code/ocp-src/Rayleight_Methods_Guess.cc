...
namespace RayleightDefine {
  ...
  void
  Rayleight::p_guess_eval( P_pointer_type P__ ) const {
  }

  void
  Rayleight::xlambda_guess_eval(
    integer              i_segment,
  ...
  bool
  Rayleight::p_check( P_const_pointer_type P__ ) const {
    return true;
  }
  ...
  integer Rayleight::u_guess_numEqns() const { return 1; }

  void
  Rayleight::u_guess_eval(
    NodeType2 const    & NODE__,
    P_const_pointer_type P__,
    ...
    UGUESS__[ iU_u ] = -2 * L__[iL_lambda2__xo];
    if ( m_debug )
  ...
  bool
  Rayleight::u_check_if_admissible(
    NodeType2 const    & NODE__,
