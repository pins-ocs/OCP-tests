...
main() {
  ...
  try {
    ...
    ICLOCS_SingularArc model("ICLOCS_SingularArc",&console,&TP);
    ...
    gc_data["DumpFile"] = "ICLOCS_SingularArc_dump";
    ...
    data_Parameters["epsi_ctrl1"] = 0.0001;
    data_Parameters["tol_ctrl0"] = tol_ctrl0;
    data_Parameters["tol_ctrl1"] = 0.0001;
...
ICLOCS_SingularArc_data.Mesh["segments"][0]["length"] = 1;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/ICLOCS_SingularArc_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/ICLOCS_SingularArc_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
