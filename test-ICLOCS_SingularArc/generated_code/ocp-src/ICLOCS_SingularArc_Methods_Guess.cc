...
namespace ICLOCS_SingularArcDefine {
  ...
  void
  ICLOCS_SingularArc::p_guess_eval( P_p_type P__ ) const {
  ...
  void
  ICLOCS_SingularArc::xlambda_guess_eval(
    integer        i_segment,
    Q_const_p_type Q__,
    P_const_p_type P__,
    X_p_type       X__,
  ...
  bool
  ICLOCS_SingularArc::p_check( P_const_p_type P__ ) const {
    return true;
  }
  ...
  bool
  ICLOCS_SingularArc::xlambda_check_node(
    integer         ipos,
  ...
  bool
  ICLOCS_SingularArc::penalties_check(
    NodeQX const & NODE__,
    P_const_p_type P__
  ) const {
    ...
    MeshStd::SegmentClass const & segment = m_pMesh->get_segment_by_index(i_segment);
    bool ok = true;
    ok = ok && tfbound.check_range(-P__[iP_T]);
    return ok;
  }
  ...
  ICLOCS_SingularArc::u_guess_eval(
    ...
    UGUESS__[ iU_u ] = uControl.solve(-MU__[0] * P__[iP_T], -2, 2);
    if ( m_debug )
  ...
  bool
  ICLOCS_SingularArc::u_check_if_admissible(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__
    ...
    ok = ok && uControl.check_range(U__[iU_u], -2, 2);
    return ok;
  }
