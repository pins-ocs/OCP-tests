...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libZermelo.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libZermelo.#{DYLIB_EXT}"
end
...
module Zermelo
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libZermelo.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = Zermelo.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = Zermelo.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = Zermelo.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      Zermelo.OCP_write_ocp_solution( @id, fname )
    end
