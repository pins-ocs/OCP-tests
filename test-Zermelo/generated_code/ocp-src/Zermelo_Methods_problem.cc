...
namespace ZermeloDefine {
  ...
  Zermelo::H_eval(
    ...
    real_type t19  = U__[iU_u];
    real_type t20  = cos(t19);
    real_type t25  = sin(t19);
    real_type result__ = (X__[iX_vx] + t7) * t2 * MU__[0] + (X__[iX_vy] + t13) * t2 * MU__[1] + t20 * t18 * t2 * MU__[2] + t25 * t18 * t2 * MU__[3];
    if ( m_debug ) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Zermelo::D2mayerD2xxp_numRows() const { return 10; }
  integer Zermelo::D2mayerD2xxp_numCols() const { return 10; }
  integer Zermelo::D2mayerD2xxp_nnz()     const { return 0; }

  void
  Zermelo::D2mayerD2xxp_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  Zermelo::D2mayerD2xxp_sparse(
    NodeQX const & LEFT__,
    NodeQX const & RIGHT__,
    P_const_p_type P__,
    real_ptr       result__
  ...
  integer Zermelo::DlagrangeDxpu_numEqns() const { return 6; }

  void
  Zermelo::DlagrangeDxpu_eval(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer Zermelo::D2lagrangeD2xpu_numRows() const { return 6; }
  integer Zermelo::D2lagrangeD2xpu_numCols() const { return 6; }
  integer Zermelo::D2lagrangeD2xpu_nnz()     const { return 0; }

  void
  Zermelo::D2lagrangeD2xpu_pattern( integer iIndex[], integer jIndex[] ) const {
    // EMPTY!
  }


  void
  Zermelo::D2lagrangeD2xpu_sparse(
    NodeQX const & NODE__,
    P_const_p_type P__,
    U_const_p_type U__,
    real_ptr       result__
  ...
  integer Zermelo::DsegmentLinkDxxp_numRows() const { return 0; }
  integer Zermelo::DsegmentLinkDxxp_numCols() const { return 0; }
  integer Zermelo::DsegmentLinkDxxp_nnz() const { return 0; }

  void
  Zermelo::DsegmentLinkDxxp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
   UTILS_ERROR0("NON IMPLEMENTATA\n");
  ...
  void
  Zermelo::DsegmentLinkDxxp_sparse(
    NodeQX const & L,
    NodeQX const & R,
    P_const_p_type p,
  ...
  integer Zermelo::post_numEqns() const { return 1; }
