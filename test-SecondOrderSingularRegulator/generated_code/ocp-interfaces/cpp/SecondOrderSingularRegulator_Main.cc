...
main() {
  ...
  try {
    SecondOrderSingularRegulator * m_model = new SecondOrderSingularRegulator("SecondOrderSingularRegulator",n_threads,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    string fname = "./data/SecondOrderSingularRegulator_Data"; // no .lua or .rb extension
    ...
    if ( ok ) {
      ...
      file.open( "data/SecondOrderSingularRegulator_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
