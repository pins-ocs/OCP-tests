...
namespace SingularLuus04Define {
  ...
  integer
  SingularLuus04::g_numEqns() const
  { return 1; }

  void
  SingularLuus04::g_eval(
    ...
    real_type t7   = ALIAS_uControl_D_1(U__[0], -1, 1);
    result__[ 0   ] = L__[2] + t7 * (t3 + ModelPars[4]);
    if ( m_debug )
  ...
  integer
  SingularLuus04::DgDxlp_numRows() const
  { return 1; }

  integer
  SingularLuus04::DgDxlp_numCols() const
  { return 6; }

  integer
  SingularLuus04::DgDxlp_nnz() const
  { return 2; }

  void
  SingularLuus04::DgDxlp_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
  ...
  integer
  SingularLuus04::DgDu_numRows() const
  { return 1; }

  integer
  SingularLuus04::DgDu_numCols() const
  { return 1; }

  integer
  SingularLuus04::DgDu_nnz() const
  { return 1; }

  void
  SingularLuus04::DgDu_pattern(
    integer iIndex[],
    integer jIndex[]
  ) const {
    iIndex[0 ] = 0   ; jIndex[0 ] = 0   ;
  ...
  SingularLuus04::DgDu_sparse(
    ...
    real_type t6   = ALIAS_uControl_D_1_1(U__[0], -1, 1);
    result__[ 0   ] = t6 * (t2 + ModelPars[4]);
    if ( m_debug )
  ...
  \*/
  integer
  SingularLuus04::u_numEqns() const
  { return 1; }

  void
  SingularLuus04::u_eval_analytic(
    NodeType2 const    & NODE__,
    P_const_pointer_type P__,
    U_pointer_type       U__
  ) const {
    ...
    MeshStd::SegmentClass const & segment = pMesh->getSegmentByIndex(i_segment);
    real_type t3   = X__[0] * X__[0];
    U__[ iU_u ] = uControl.solve(-1.0 / (t3 + ModelPars[4]) * L__[2], -1, 1);
  }
  ...
  \*/
  void
  SingularLuus04::DuDxlp_full_analytic(
    NodeType2 const          & NODE__,
    P_const_pointer_type       P__,
    U_const_pointer_type       U__,
    ...
    real_type const * X__ = NODE__.x;
    real_type const * L__ = NODE__.lambda;
    DuDxlp(0, 0) = 2 * uControl.solve_rhs(-L__[2] / (X__[0] * X__[0] + ModelPars[4]), -1, 1) * L__[2] * pow(X__[0] * X__[0] + ModelPars[4], -2) * X__[0];
    DuDxlp(0, 1) = 0;
    DuDxlp(0, 2) = 0;
    DuDxlp(0, 3) = 0;
    DuDxlp(0, 4) = 0;
    DuDxlp(0, 5) = -uControl.solve_rhs(-L__[2] / (X__[0] * X__[0] + ModelPars[4]), -1, 1) / (X__[0] * X__[0] + ModelPars[4]);
  }
  ...
  bool
  SingularLuus04::DuDxlp_full_numeric(
    NodeType2 const          & NODE__,
    P_const_pointer_type       P__,
    U_const_pointer_type       U__,
    MatrixWrapper<real_type> & DuDxlp
