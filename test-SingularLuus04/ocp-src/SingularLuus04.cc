...
namespace SingularLuus04Define {
  ...
  */
  void
  SingularLuus04::updateContinuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "SingularLuus04::updateContinuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  SingularLuus04::infoClasses() const {
    int msg_level = 3;
    ostringstream mstr;
  ...
  */
  void
  SingularLuus04::get_names( GenericContainer & out ) const {
  ...
  */
  void
  SingularLuus04::diagnostic( GenericContainer const & gc ) {
  ...
  void
  SingularLuus04::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
