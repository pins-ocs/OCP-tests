...
classdef SingularLuus04 < handle
  ...
  methods
    ...
    function set_max_threads( self, nt )
      ...
      %
      SingularLuus04_Mex( 'set_max_threads', self.objectHandle, nt );
    end
    ...
    function remesh( self, new_mesh )
      ...
      %
      SingularLuus04_Mex( 'remesh', self.objectHandle, new_mesh );
    end
    ...
    function ok = solve( self, varargin )
      ...
      %
      ok = SingularLuus04_Mex( 'solve', self.objectHandle, varargin{:} );
    end
    % ---------------------------------------------------------------------
    ...
    function sol = solution_by_group( self )
      ...
      sol = SingularLuus04_Mex( 'get_solution2', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function sol = solution_by_group_and_names( self )
      ...
      % res.data.control_penalties
      %
      sol = SingularLuus04_Mex( 'get_solution3', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    ...
    function res = x( self )
      ...
      %
      res = SingularLuus04_Mex( 'get_solution', self.objectHandle, 'x' );
    end
    function res = y( self )
      %
      % Return the solution for the state: y
      %
      res = SingularLuus04_Mex( 'get_solution', self.objectHandle, 'y' );
    end
    function res = z( self )
      %
      % Return the solution for the state: z
      %
      res = SingularLuus04_Mex( 'get_solution', self.objectHandle, 'z' );
    end
    ...
    function res = lambda1( self )
      ...
      %
      res = SingularLuus04_Mex( 'get_solution', self.objectHandle, 'lambda1__xo' );
    end
    function res = lambda2( self )
      %
      % Return the solution for the multiplier: lambda2
      %
      res = SingularLuus04_Mex( 'get_solution', self.objectHandle, 'lambda2__xo' );
    end
    function res = lambda3( self )
      %
      % Return the solution for the multiplier: lambda3
      %
      res = SingularLuus04_Mex( 'get_solution', self.objectHandle, 'lambda3__xo' );
    end
    ...
    function res = u( self )
      ...
      %
      res = SingularLuus04_Mex( 'get_solution', self.objectHandle, 'u' );
    end
    ...
    function U = init_U( self, x, do_minimization )
      ...
      % Initialize `u`
      %
      U = SingularLuus04_Mex( 'init_U', self.objectHandle, x, do_minimization );
    end
    % ---------------------------------------------------------------------
    function U = eval_U( self, x, u_guess )
      ...
      % Compute `u`
      %
      U = SingularLuus04_Mex( 'eval_U', self.objectHandle, x, u_guess );
    end
    % ---------------------------------------------------------------------
    function [F,ok] = eval_F( self, x, u )
      ...
      %
      [F,ok] = SingularLuus04_Mex( 'eval_F', self.objectHandle, x, u );
    end
    % ---------------------------------------------------------------------
    function [JF,ok] = eval_JF( self, x, u )
      ...
      %
      [JF,ok] = SingularLuus04_Mex( 'eval_JF', self.objectHandle, x, u );
    end
    % ---------------------------------------------------------------------
    ...
    function [z,u] = get_raw_solution( self )
      ...
      %
      [z,u] = SingularLuus04_Mex( 'get_raw_solution', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function set_raw_solution( self, z, u )
      ...
      %
      SingularLuus04_Mex( 'set_raw_solution', self.objectHandle, z, u );
    end
    % ---------------------------------------------------------------------
    function ok = check_raw_solution( self, z )
      ...
      % admissible regions.
      %
      ok = SingularLuus04_Mex( 'check_raw_solution', self.objectHandle, z );
    end
    % ---------------------------------------------------------------------
    function check_jacobian( self, z, u, epsi )
      ...
      %
      SingularLuus04_Mex( 'check_jacobian', self.objectHandle, z, u, epsi );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    ...
    function [a,c] = eval_ac( self, iseg_L, q_L, x_L, lambda_L, ...
      ...
      % <<FD1.jpg>>
      %
      [a,c] = SingularLuus04_Mex( 'ac', self.objectHandle, ...
    ...
    function [Ja,Jc] = eval_DacDxlxlp( self, iseg_L, q_L, x_L, lambda_L, ...
      ...
      % <<FD2.jpg>>
      %
      [Ja,Jc] = SingularLuus04_Mex( 'DacDxlxlp', self.objectHandle, ...
        iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars, U ...
      );
    end
    % ---------------------------------------------------------------------
    function [h,c] = eval_hc( self, iseg_L, q_L, x_L, lambda_L, ...
      ...
      % <<FD3.jpg>>
      %
      [h,c] = SingularLuus04_Mex( 'hc', self.objectHandle, ...
    ...
    function [Jh,Jc] = eval_DhcDxlxlop( self, iseg_L, q_L, x_L, lambda_L, ...
      ...
      % <<FD4.jpg>>
      %
      [Jh,Jc] = SingularLuus04_Mex( 'DhcDxlxlop', self.objectHandle, ...
        iseg_L,  q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
      );
    end
    % ---------------------------------------------------------------------
    ...
    function u = eval_u( self, iseg_L, q_L, x_L, lambda_L, ...
      ...
      %
      u = SingularLuus04_Mex( 'u', self.objectHandle, ...
        iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
      );
    end
    % ---------------------------------------------------------------------
    function DuDxlxlp = eval_DuDxlxlp( self, iseg_L, q_L, x_L, lambda_L, ...
      ...
      %
      DuDxlxlp = SingularLuus04_Mex( 'DuDxlxlp', self.objectHandle, ...
        iseg_L, q_L, x_L, lambda_L, iseg_R, q_R, x_R, lambda_R, pars ...
      );
    end
    % ---------------------------------------------------------------------
    function rhs = eval_rhs_ode( self, iseg, q, x, u, pars )
      ...
      %
      rhs = SingularLuus04_Mex( 'rhs_ode', self.objectHandle, ...
        iseg, q, x, u, pars ...
      );
    end
    % ---------------------------------------------------------------------
    ...
    function A = eval_A( self, iseg, q, x, pars )
      ...
      % respect to `pars`.
      %
      A = SingularLuus04_Mex( 'A', self.objectHandle, iseg, q, x, pars );
    end
    % ---------------------------------------------------------------------
    ...
    function nu = eval_nu( self, iseg, q, x, V, pars )
      ...
      %
      nu = SingularLuus04_Mex( 'nu', self.objectHandle, iseg, q, x, V, pars );
    end
    % ---------------------------------------------------------------------
    function J = eval_DnuDx( self, iseg, q, x, V, pars )
      ...
      % respect to `x`.
      %
      J = SingularLuus04_Mex( 'DnuDx', self.objectHandle, iseg, q, x, V, pars );
    end
    % ---------------------------------------------------------------------
    function J = eval_DnuDp( self, iseg, q, x, V, pars )
      ...
      % respect to `x`.
      %
      J = SingularLuus04_Mex( 'DnuDp', self.objectHandle, iseg, q, x, V, pars );
    end
    % ---------------------------------------------------------------------
    ...
    function target = eval_q( self, i_segment, s )
      target = SingularLuus04_Mex( 'mesh_functions', self.objectHandle, i_segment, s );
    end
    % ---------------------------------------------------------------------
    function nodes = get_nodes( self )
      nodes = SingularLuus04_Mex( 'nodes', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function node_to_segment = get_node_to_segment( self )
      node_to_segment = SingularLuus04_Mex( 'node_to_segment', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    % PLOT SOLUTION
    % ---------------------------------------------------------------------
