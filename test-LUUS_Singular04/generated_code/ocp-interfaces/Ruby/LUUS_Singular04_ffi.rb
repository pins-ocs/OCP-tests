...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libLUUS_Singular04.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libLUUS_Singular04.#{DYLIB_EXT}"
end
...
module LUUS_Singular04
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libLUUS_Singular04.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = LUUS_Singular04.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = LUUS_Singular04.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = LUUS_Singular04.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      LUUS_Singular04.OCP_write_ocp_solution( @id, fname )
    end
