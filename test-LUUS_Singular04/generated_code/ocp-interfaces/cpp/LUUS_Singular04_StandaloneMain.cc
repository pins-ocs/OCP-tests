...
main() {
  ...
  try {
    ...
    LUUS_Singular04  model("LUUS_Singular04",n_threads,&console);
    ...
    gc_data["DumpFile"] = "LUUS_Singular04_dump";
...
LUUS_Singular04_data.Mesh["segments"][0]["length"] = Tf;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/LUUS_Singular04_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/LUUS_Singular04_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
