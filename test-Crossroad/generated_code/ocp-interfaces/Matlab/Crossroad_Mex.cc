...
static std::map<std::string,DO_CMD> cmd_to_fun = {
  ...
  {"kappa",&ProblemStorage::do_kappa},
  {"kappa_D",&ProblemStorage::do_kappa_D},
  {"kappa_DD",&ProblemStorage::do_kappa_DD},
  {"VelBound",&ProblemStorage::do_VelBound},
  {"VelBound_D",&ProblemStorage::do_VelBound_D}
};
...
mexFunction(
  ...
  try {
    ...
    if ( it != cmd_to_fun2.end() ) {
      ...
      UTILS_MEX_ASSERT(
        nrhs >= 2,
        "Crossroad_Mex('{}',obj,...): Expected at least 2 argument(s), nrhs = {}\n",
        cmd, nrhs
      );
