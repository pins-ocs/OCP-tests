...
classdef Crossroad < handle
  ...
  methods
    ...
    function U = guess_U( self, Z )
      ...
      % Initialize `u`
      %
      U = Crossroad_Mex( 'guess_U', self.objectHandle, Z );
    end
    % ---------------------------------------------------------------------
    function U = eval_U( self, Z, u_guess )
      ...
      %
      U = Crossroad_Mex( 'eval_U', self.objectHandle, Z, u_guess );
    end
    % ---------------------------------------------------------------------
    function [F,ok] = eval_F( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      [F,ok] = Crossroad_Mex( 'eval_F', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    function [JF,ok] = eval_JF( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      [JF,ok] = Crossroad_Mex( 'eval_JF', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    ...
    function [JF,ok] = eval_JF2( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      [JF,ok] = Crossroad_Mex( 'eval_JF2', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    ...
    function [Z,U] = get_raw_solution( self )
      ...
      %
      [Z,U] = Crossroad_Mex( 'get_raw_solution', self.objectHandle );
    end
    % ---------------------------------------------------------------------
    function set_raw_solution( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      Crossroad_Mex( 'set_raw_solution', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    ...
    function check_jacobian( self, Z, U, epsi )
      ...
      %
      Crossroad_Mex( 'check_jacobian', self.objectHandle, Z, U, epsi );
    end
    % ---------------------------------------------------------------------
    % ---------------------------------------------------------------------
    ...
    function [a,b,c] = eval_abc( self, L, R, pars, U )
      ...
      % <<FD1.jpg>>
      %
      [a,b,c] = Crossroad_Mex( 'abc', self.objectHandle, L, R, pars, U );
    end
    % ---------------------------------------------------------------------
    function DabcDxlxlpu = eval_DabcDxlxlpu( self, L, R, pars, U )
      ...
      % <<FD2.jpg>>
      %
      DabcDxlxlpu = Crossroad_Mex( 'DabcDxlxlpu', self.objectHandle, L, R, pars, U );
    end
    % ---------------------------------------------------------------------
    ...
    function DuDxlxlp = eval_DuDxlxlp( self, L, R, pars, MU )
      ...
      %
      DuDxlxlp = Crossroad_Mex( 'DuDxlxlp', self.objectHandle, L, R, pars, MU );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DsegmentLinkDxxp_pattern( self )
      res = Crossroad_Mex('DsegmentLinkDxxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
