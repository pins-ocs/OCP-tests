...
namespace CrossroadDefine {
  ...
  */
  ...
  void
  Crossroad::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  Crossroad::info_model( integer level ) const {
    ...
    m_console->message( jerkControl.info(),level);

    // ==================================================================
    m_console->PINS_COLOR_TITLE( fmt::format( "+{:.^71}+\n", "Constraints type LT" ), level);
    m_console->message( Tpositive.info(),level);
    m_console->message( AccBound.info(),level);
    m_console->message( VelBound_min.info(),level);
    m_console->message( VelBound_max.info(),level);
