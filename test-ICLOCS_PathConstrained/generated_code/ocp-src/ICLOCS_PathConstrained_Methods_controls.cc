...
namespace ICLOCS_PathConstrainedDefine {
  ...
  real_type
  ICLOCS_PathConstrained::g_fun_eval(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    ...
    real_type t7   = t6 * t6;
    real_type t8   = uControl(t1, -20, 20);
    real_type t11  = pow(Q__[iQ_zeta] - 0.5e0, 2);
    real_type t14  = x2bound(0.5e0 - 8 * t11 + t6);
    real_type result__ = 0.5e-2 * t2 + t5 + t7 + t8 + t14 + t6 * MU__[0] + (-t6 + t1) * MU__[1];
    if ( m_debug ) {
  ...
  ICLOCS_PathConstrained::g_eval(
    ...
    MeshStd::SegmentClass const & segment = pMesh->get_segment_by_index(i_segment);
    real_type t1   = U__[iU_u];
    real_type t3   = ALIAS_uControl_D_1(t1, -20, 20);
    result__[ 0   ] = 0.10e-1 * t1 + t3 + MU__[1];
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 1, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer ICLOCS_PathConstrained::DgDxpm_numRows() const { return 1; }
  integer ICLOCS_PathConstrained::DgDxpm_numCols() const { return 4; }
  integer ICLOCS_PathConstrained::DgDxpm_nnz()     const { return 1; }

  void
  ICLOCS_PathConstrained::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  void
  ICLOCS_PathConstrained::DgDxpm_sparse(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__,
  ...
  void
  ICLOCS_PathConstrained::DgDu_sparse(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__,
  ...
  void
  ICLOCS_PathConstrained::u_eval_analytic(
...
// EOF: ICLOCS_PathConstrained_Methods_controls.cc
