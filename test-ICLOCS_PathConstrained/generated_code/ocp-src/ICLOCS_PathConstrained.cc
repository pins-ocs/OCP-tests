...
namespace ICLOCS_PathConstrainedDefine {
  ...
  */
  void
  ICLOCS_PathConstrained::update_continuation(
    integer   phase,
    real_type old_s,
    real_type s
  ) {
    ...
    UTILS_ASSERT(
      0 <= old_s && old_s < s && s <= 1,
      "ICLOCS_PathConstrained::update_continuation( phase number={}, old_s={}, s={} ) "
      "must be 0 <= old_s < s <= 1\n",
      phase, old_s, s
    );
    switch ( phase ) {
  ...
  */
  void
  ICLOCS_PathConstrained::setup_parameters( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Parameters"),
      "ICLOCS_PathConstrained::setup_parameters: Missing key `Parameters` in data\n"
    );
    ...
    UTILS_ASSERT0(
      allfound, "in ICLOCS_PathConstrained::setup not all parameters are set!\n"
    );
  }

  void
  ICLOCS_PathConstrained::setup_parameters( real_const_ptr Pars ) {
  ...
  */
  void
  ICLOCS_PathConstrained::setup_classes( GenericContainer const & gc_data ) {
    UTILS_ASSERT0(
      gc_data.exists("Constraints"),
      "ICLOCS_PathConstrained::setup_classes: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    UTILS_ASSERT0(
      gc.exists("x2bound"),
      "in ICLOCS_PathConstrained::setup_classes(gc) missing key: ``x2bound''\n"
    );
  ...
  */
  void
  ICLOCS_PathConstrained::setup_user_classes( GenericContainer const & gc ) {
  }
  ...
  */
  void
  ICLOCS_PathConstrained::setup_user_mapped_functions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  ICLOCS_PathConstrained::setup_controls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    UTILS_ASSERT0(
      gc_data.exists("Controls"),
      "ICLOCS_PathConstrained::setup_classes: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  ICLOCS_PathConstrained::setup_pointers( GenericContainer const & gc_data ) {

    UTILS_ASSERT0(
      gc_data.exists("Pointers"),
      "ICLOCS_PathConstrained::setup_pointers: Missing key `Pointers` in data\n"
    );
    ...
    UTILS_ASSERT0(
      gc.exists("pMesh"),
      "in ICLOCS_PathConstrained::setup_pointers(gc) cant find key `pMesh' in gc\n"
    );
  ...
  */
  void
  ICLOCS_PathConstrained::info_model( integer level ) const {
    ...
    m_console->message( x2bound.info(),level);
  ...
  */
  void
  ICLOCS_PathConstrained::setup( GenericContainer const & gc ) {

    m_debug = false;
    if ( gc.exists("Debug") )
      m_debug = gc("Debug").get_bool("ICLOCS_PathConstrained::setup, Debug");
  ...
  */
  void
  ICLOCS_PathConstrained::get_names( GenericContainer & out ) const {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void
  ICLOCS_PathConstrained::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
