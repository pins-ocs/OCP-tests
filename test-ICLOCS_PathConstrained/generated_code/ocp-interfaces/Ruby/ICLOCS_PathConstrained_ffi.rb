#-----------------------------------------------------------------------#
#  file: ICLOCS_PathConstrained_run.rb                                  #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libICLOCS_PathConstrained.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libICLOCS_PathConstrained.#{DYLIB_EXT}"
end
...
module ICLOCS_PathConstrained

  extend FFI::Library

  LIB_PATH = MECHATRONIX_LIB_PATH+"/libICLOCS_PathConstrained.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = ICLOCS_PathConstrained.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = ICLOCS_PathConstrained.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = ICLOCS_PathConstrained.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      ICLOCS_PathConstrained.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: ICLOCS_PathConstrained_ffi.rb
