...
main() {
  ...
  try {
    ...
    MinimumEnergyProblem model("MinimumEnergyProblem",&TP,&console);
    ...
    gc_data["DumpFile"] = "MinimumEnergyProblem_dump";
...
MinimumEnergyProblem_data.Mesh["segments"][0]["n"] = 20000;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/MinimumEnergyProblem_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/MinimumEnergyProblem_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
