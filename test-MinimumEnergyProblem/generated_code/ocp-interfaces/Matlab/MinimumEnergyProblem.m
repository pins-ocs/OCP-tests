...
classdef MinimumEnergyProblem < handle
  ...
  methods
    ...
    function U = guess_U( self, Z )
      ...
      % Initialize `u`
      %
      U = MinimumEnergyProblem_Mex( 'guess_U', self.objectHandle, Z );
    end
    % ---------------------------------------------------------------------
    ...
    function [JF,ok] = eval_JF2( self, Z, U )
      ...
      % Vector Z can be built as Z = pack( X, Lambda, Pars, Omega );
      %
      [JF,ok] = MinimumEnergyProblem_Mex( 'eval_JF2', self.objectHandle, Z, U );
    end
    % ---------------------------------------------------------------------
    ...
    function [a,b,c] = eval_abc( self, L, R, pars, U )
      ...
      % <<FD1.jpg>>
      %
      [a,b,c] = MinimumEnergyProblem_Mex( 'abc', self.objectHandle, L, R, pars, U );
    end
    % ---------------------------------------------------------------------
    function DabcDxlxlpu = eval_DabcDxlxlpu( self, L, R, pars, U )
      ...
      % <<FD2.jpg>>
      %
      DabcDxlxlpu = MinimumEnergyProblem_Mex( 'DabcDxlxlpu', self.objectHandle, L, R, pars, U );
    end
    % ---------------------------------------------------------------------
    ...
    function DuDxlxlp = eval_DuDxlxlp( self, L, R, pars, MU )
      ...
      %
      DuDxlxlp = MinimumEnergyProblem_Mex( 'DuDxlxlp', self.objectHandle, L, R, pars, MU );
    end
    % ---------------------------------------------------------------------
    ...
    function res = DsegmentLinkDxxp_pattern( self )
      res = MinimumEnergyProblem_Mex('DsegmentLinkDxxp_pattern', self.objectHandle );
    end
    % ---------------------------------------------------------------------
