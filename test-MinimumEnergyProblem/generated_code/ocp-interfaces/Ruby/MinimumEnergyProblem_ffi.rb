#-----------------------------------------------------------------------#
#  file: MinimumEnergyProblem_run.rb                                    #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libMinimumEnergyProblem.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libMinimumEnergyProblem.#{DYLIB_EXT}"
end
...
module MinimumEnergyProblem
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libMinimumEnergyProblem.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = MinimumEnergyProblem.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = MinimumEnergyProblem.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = MinimumEnergyProblem.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      MinimumEnergyProblem.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: MinimumEnergyProblem_ffi.rb
