...
main() {
  ...
  try {
    BertolazziCorsoExample1 * m_model = new BertolazziCorsoExample1("BertolazziCorsoExample1",n_threads,&console);

    // user defined Object instances (external)
    MeshStd          mesh( "mesh" );

    #ifdef MECHATRONIX_USE_RUBY
    string fname = "./data/BertolazziCorsoExample1_Data.rb";
    #endif
    #ifdef MECHATRONIX_USE_LUA
    string fname = "./data/BertolazziCorsoExample1_Data.lua";
    #endif
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/BertolazziCorsoExample1_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/BertolazziCorsoExample1_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
