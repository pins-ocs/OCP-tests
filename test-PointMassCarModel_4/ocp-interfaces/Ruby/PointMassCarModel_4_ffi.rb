#-----------------------------------------------------------------------#
#  file: PointMassCarModel_4_run.rb                                     #
#                                                                       #
...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libPointMassCarModel_4.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libPointMassCarModel_4.#{DYLIB_EXT}"
end
...
module PointMassCarModel_4
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libPointMassCarModel_4.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = PointMassCarModel_4.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = PointMassCarModel_4.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = PointMassCarModel_4.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      PointMassCarModel_4.OCP_write_ocp_solution( @id, fname )
    end
...
# EOF: PointMassCarModel_4_ffi.rb
