/*-----------------------------------------------------------------------*\
 |  file: PointMassCarModel_4_dll_ruby.cc                                |
 |                                                                       |
...
#include "PointMassCarModel_4_dll_ruby.hh"
#include <map>
...
namespace PointMassCarModel_4Define {
  ...
  static map< string, PointMassCarModel_4_Problem * > problems;
  ...
  */
  ...
  PointMassCarModel_4_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, PointMassCarModel_4_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  POINTMASSCARMODEL_4_API_DLL
  bool
  PointMassCarModel_4_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, PointMassCarModel_4_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "PointMassCarModel_4_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  PointMassCarModel_4_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("PointMassCarModel_4_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  PointMassCarModel_4_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, PointMassCarModel_4_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("PointMassCarModel_4_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
...
// EOF: PointMassCarModel_4_dll_ruby.cc
