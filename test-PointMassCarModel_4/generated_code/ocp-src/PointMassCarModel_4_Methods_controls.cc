...
namespace PointMassCarModel_4Define {
  ...
  PointMassCarModel_4::g_fun_eval(
    ...
    real_type t56  = ALIAS_Kappa(t35);
    real_type t57  = zeta__dot(t23, t55, t34, t56);
    real_type t61  = sin(t55);
    real_type result__ = t3 * ModelPars[iM_wT] + t6 * t3 + t9 * t3 + t32 * t3 + t38 * t3 + t42 * t3 + t51 * t3 + t57 * t3 * MU__[0] + t61 * t23 * t3 * MU__[1] - t3 * (t57 * t56 - t21) * MU__[2] + t3 * (-t24 * ModelPars[iM_kD] + t11) * MU__[3] + ModelPars[iM_v__Omega__max] * t8 * t3 * MU__[4] + ModelPars[iM_v__fx__max] * t5 * t3 * MU__[5];
    if ( m_debug ) {
  ...
  PointMassCarModel_4::g_eval(
    ...
    result__[ 0   ] = ModelPars[iM_v__fx__max] * t2 * MU__[5] + t2 * t4;
    real_type t11  = ALIAS_v__OmegaControl_D_1(U__[iU_v__Omega], -1, 1);
    result__[ 1   ] = ModelPars[iM_v__Omega__max] * t2 * MU__[4] + t11 * t2;
    if ( m_debug )
      Mechatronix::check_in_segment( result__, "g_eval", 2, i_segment );
  }

  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  integer PointMassCarModel_4::DgDxpm_numRows() const { return 2; }
  integer PointMassCarModel_4::DgDxpm_numCols() const { return 14; }
  integer PointMassCarModel_4::DgDxpm_nnz()     const { return 4; }

  void
  PointMassCarModel_4::DgDxpm_pattern( integer iIndex[], integer jIndex[] ) const {
  ...
  void
  PointMassCarModel_4::DgDxpm_sparse(
    NodeQX const &  NODE__,
    P_const_p_type  P__,
    MU_const_p_type MU__,
    U_const_p_type  U__,
    ...
    result__[ 0   ] = t8 * t2 * MU__[5] + t4 * t2;
    result__[ 1   ] = t8 * t2;
    real_type t11  = ALIAS_v__OmegaControl_D_1(U__[iU_v__Omega], -1, 1);
    real_type t15  = ModelPars[iM_v__Omega__max];
    result__[ 2   ] = t15 * t2 * MU__[4] + t11 * t2;
    result__[ 3   ] = t15 * t2;
    if ( m_debug )
