...
namespace PointMassCarModel_4Define {
  ...
  PointMassCarModel_4::DgDxlp_sparse(
    ...
    result__[ 0   ] = t8 * t2 * L__[iL_lambda6__xo] + t4 * t2;
    result__[ 1   ] = t8 * t2;
    real_type t11  = ALIAS_v__OmegaControl_D_1(U__[iU_v__Omega], -1, 1);
    real_type t15  = ModelPars[iM_v__Omega__max];
    result__[ 2   ] = t15 * t2 * L__[iL_lambda5__xo] + t11 * t2;
    result__[ 3   ] = t15 * t2;
    if ( m_debug )
  ...
  PointMassCarModel_4::m_eval(
    ...
    real_type t69  = pow(V__[2] + t2 * (t54 * t53 - t13), 2);
    real_type t76  = pow(V__[3] - t2 * (-ModelPars[iM_kD] * t16 + t3), 2);
