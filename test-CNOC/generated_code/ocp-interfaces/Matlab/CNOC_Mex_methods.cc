...
)
{
  ...
  UTILS_MEX_ASSERT0( m_gc_data.exists("ToolPath2D"), "missing key: ``ToolPath2D'' in m_gc_data\n" );
  m_toolPath2D.setup( m_gc_data("ToolPath2D") );
...
)
{
  ...
  CHECK_IN_OUT( 3, 0 );
  GenericContainer & gc_mesh = m_gc_data["ToolPath2D"];
  GenericContainer & ptrs    = m_gc_data["Pointers"];
...
)
{
  ...
  try {
    ...
    mexWarnMsgTxt( fmt::format( "CNOC_Mex('eval_JF2',...) error: {}", exc.what() ).c_str() );
    ok_value = false;
  } catch ( ... ) {
    mexWarnMsgTxt( "CNOC_Mex('eval_JF2',...) unkown error\n" );
    ok_value = false;
  }
  int ok = mexCallMATLAB( 1, &arg_out_0, 5, args, "sparse" );
  UTILS_MEX_ASSERT0( ok == 0, CMD "failed the call sparse(...)" );
