...
main() {
  ...
  try {
    ...
    SingularConstrainedCalogero model("SingularConstrainedCalogero",n_threads,&console);
    ...
    gc_data["DumpFile"] = "SingularConstrainedCalogero_dump";
    ...
    GenericContainer & data_uMaxBound = data_Constraints["uMaxBound"];
    data_uMaxBound["subType"]   = "PENALTY_REGULAR";
    data_uMaxBound["epsilon"]   = epsi_max;
    data_uMaxBound["tolerance"] = tol_max;
    data_uMaxBound["active"]    = true;
...
SingularConstrainedCalogero_data.Mesh["segments"][0]["n"] = 600;
    ...
    std::ofstream file;
    if ( ok ) {
      file.open( "data/SingularConstrainedCalogero_OCP_result.txt" );
    } else {
      cout << gc_solution("solver_message").get_string() << '\n';
      // dump solution to file
      file.open( "data/SingularConstrainedCalogero_OCP_result_NOT_CONVERGED.txt" );
    }
    file.precision(18);
